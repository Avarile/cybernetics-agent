#!/usr/bin/env python3
"""One-shot bootstrap launcher for Cybernetics WebUI."""
...
def _install_env_bridge() -> None:
    """Mirror CYBERNETICS_WEBUI_* ↔ HERMES_WEBUI_* before any reader runs.

    Bootstrap reads several HERMES_WEBUI_* vars at module load (DEFAULT_HOST,
    DEFAULT_PORT, …) so the bridge must run before those lookups happen, even
    though api.__init__ also installs it for downstream modules. Inline import
    keeps bootstrap.py self-contained when api/ isn't on sys.path yet.
    """
    try:
        import importlib.util as _util
        spec = _util.spec_from_file_location("api._env_compat", REPO_ROOT / "api" / "_env_compat.py")
        if spec and spec.loader:
            mod = _util.module_from_spec(spec)
            spec.loader.exec_module(mod)
    except Exception:
        # Bridge is best-effort: a missing helper should never block startup.
        pass
...
def _agent_dir_from_hermes_cli() -> Path | None:
    ...
    The Cybernetics Agent installer drops a `hermes` console-script in the user's
    ...
    """
    cli_path = shutil.which("cybernetics") or shutil.which("hermes")
    if not cli_path:
        return None
    try:
...
def discover_agent_dir() -> Path | None:
    # Parent project migrated ~/.hermes → ~/.cybernetics; the legacy path is
    # retained as a fallback so existing installs that haven't migrated still work.
    home_default = Path.home() / ".cybernetics"
    if not home_default.exists():
    ...
    candidates = [
        os.getenv("CYBERNETICS_WEBUI_AGENT_DIR", "") or os.getenv("HERMES_WEBUI_AGENT_DIR", ""),
        str(home / "cybernetics-agent"),
        str(home / "hermes-agent"),
        str(REPO_ROOT.parent / "cybernetics-agent"),
        str(REPO_ROOT.parent / "hermes-agent"),
        str(Path.home() / ".cybernetics" / "cybernetics-agent"),
        str(Path.home() / ".hermes" / "hermes-agent"),
        str(Path.home() / "cybernetics-agent"),
...
def ensure_python_has_webui_deps(python_exe: str, agent_dir: Path | None = None) -> str:
    """Return a Python executable that can run both WebUI and Cybernetics Agent.

    The WebUI can be launched directly with its local .venv. That venv has the
    WebUI dependencies (for example PyYAML), but may not have Cybernetics Agent on its
    ...
    raise RuntimeError(
        "Python environment cannot import both WebUI dependencies and Cybernetics Agent. "
        "Set HERMES_WEBUI_PYTHON to the Cybernetics Agent venv Python or install the "
...
def install_hermes_agent() -> None:
    ...
    info(f"Cybernetics Agent not found. Attempting install via {INSTALLER_URL}")
    subprocess.run(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Bootstrap Cybernetics WebUI onboarding.")
    ...
    parser.add_argument(
        "--skip-agent-install",
        action="store_true",
        help="Fail instead of attempting the official Cybernetics installer.",
    )
    parser.add_argument(
        "--foreground",
        action="store_true",
...
def main() -> int:
    ...
    if not agent_dir and not hermes_command_exists():
        if args.skip_agent_install:
            raise RuntimeError(
                "Cybernetics Agent was not found and auto-install was disabled."
            )
    ...
    if foreground_reason:
        info(
            f"Starting Cybernetics WebUI on http://{args.host}:{args.port} "
    ...
    info(f"Starting Cybernetics WebUI on http://{args.host}:{args.port}")
