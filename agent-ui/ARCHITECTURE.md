# Cybernetics WebUI: Developer and Architecture Guide

> This document is the canonical reference for anyone (human or agent) working on the
> Cybernetics WebUI. It covers the exact current state of the code, every design decision and
...
The Cybernetics WebUI is a lightweight web application that gives you a browser-based
interface to the Cybernetics agent that is functionally equivalent to the CLI. It is modeled on
...
Cybernetics-level chrome is intentionally consolidated: the sidebar has no dedicated brand header.
Instead, the footer exposes a single "Cybernetics WebUI" launch button that opens one tabbed
...
- The venv has all Cybernetics agent dependencies (run_agent, tools/*, cron/*)
...
- The server imports Cybernetics modules via sys.path.insert(0, parent_dir)
...
Environment variables controlling behavior:
    ...
    CYBERNETICS_WEBUI_PASSWORD          Optional: enable password auth (off by default)
    ...
    HERMES_HOME                    Base directory for Cybernetics state (~/.hermes by default)

Test isolation environment variables (set by conftest.py):

    HERMES_WEBUI_TEST_PORT=...                         Optional pinned test port
    HERMES_WEBUI_TEST_STATE_DIR=/tmp/cybernetics-webui-tests/* Optional pinned test state (default: OS temp dir; must be outside ~/.hermes)
...
The approval system uses the existing Cybernetics gateway module at tools/approval.py.
...
Three-panel layout (in static/index.html):
    ...
    <aside class="sidebar">    Left panel: session list, nav tabs, sidebar-footer Cybernetics WebUI trigger
...
Boot IIFE:
    localStorage key 'cybernetics-webui-session' stores last session_id
...
9.  setBusy(true), setStatus('Cybernetics is thinking...')
...
The api/ modules in turn import Cybernetics internals:
...
1. CYBERNETICS_WEBUI_PASSWORD env var enables auth
2. Login page: minimal dark form, POST /api/auth/login
3. Server sets HttpOnly + SameSite=Strict cookie on successful login
4. All API endpoints check cookie if CYBERNETICS_WEBUI_PASSWORD is set
...
Endpoint that calls Cybernetics Python modules:
...
This section is specifically for agents (Cybernetics instances, subagents, Codex, etc.) that
