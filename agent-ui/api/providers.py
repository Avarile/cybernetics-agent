"""Cybernetics WebUI -- provider management endpoints.
...
# extension — the same technique used throughout the Cybernetics codebase.
...
def _codex_usage_headers(access_token):
    headers = {
        ...
        "User-Agent": "codex_cli_rs/0.0.0 (Cybernetics WebUI)",
...
# through the Cybernetics CLI, not via API keys.  The WebUI cannot set these.
...
def _get_hermes_home() -> Path:
    """Return the active Cybernetics home directory."""
    try:
...
def get_provider_quota(provider_id: str | None = None, *, refresh: bool = False) -> dict[str, Any]:
    ...
    providers reuse Cybernetics Agent's /usage account-limits abstraction so WebUI
...
def _cost_snapshots_dir() -> Path:
    ...
    Uses the Cybernetics home directory (profile-aware) so snapshots are
...
def _append_cost_snapshot(provider: str, usage: int | float | None, limit: int | float | None) -> list[dict[str, Any]]:
    ...
    # deployments that share one Cybernetics home/state directory.
