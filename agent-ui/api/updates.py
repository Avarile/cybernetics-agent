"""
Cybernetics WebUI -- Self-update checker.
...
def _read_agent_source_version(agent_dir: Path) -> str | None:
    """Read Cybernetics Agent's package version from a copied source tree."""
...
def _gateway_health_base_url() -> str:
    """Return the configured/default Cybernetics Agent gateway base URL."""
    raw = (
...
def _version_from_gateway_health_payload(payload: object) -> str | None:
    """Extract a version string from a Cybernetics Agent gateway health payload."""
...
def _detect_agent_version() -> str:
    """Detect the running Cybernetics Agent version for UI display."""
...
def _normalize_remote_url(remote_url):
    ...
    treats the argument as a character set and can truncate ``cybernetics-webui`` to
    ``hermes-webu``.
    """
    if not remote_url:
        return remote_url
...
def _commit_subjects_for_update_with_limit(info: dict, *, limit: int = 24) -> tuple[list[str], bool]:
    ...
    if target not in ('webui', 'agent'):
        target = 'webui' if info.get('repo_url', '').endswith('cybernetics-webui') else target
...
def _fallback_update_bullets(details: list[dict]) -> list[str]:
    bullets = []
    for item in details:
        label = item.get('label') or item.get('name') or 'Cybernetics'
...
def _worth_knowing_bullets(details: list[dict]) -> list[str]:
    ...
    if items:
        return items
    targets = [
        f"{item.get('label') or item.get('name') or 'Cybernetics'} ({item.get('behind') or 0} update{'s' if (item.get('behind') or 0) != 1 else ''})"
        for item in details
        if item.get('behind')
    ]
    if len(targets) > 1:
...
def _update_summary_prompt(details: list[dict]) -> tuple[str, str]:
    system = (
        "You write human-readable release summaries for Cybernetics users. "
    ...
    def _do():
        ...
        with _apply_lock:
            ...
            try:
                ...
                #     (e.g. "/root/cybernetics-webui/server.py"), sys.executable is
