"""
Cybernetics WebUI -- Workspace and file system helpers.
...
def _expanduser_path(path: str | Path) -> Path:
    ...
    if raw.startswith('~'):
        ...
        # Master deliberately does not block ``/root`` (#510/#521 — Cybernetics commonly
...
def _workspace_access_error(candidate: Path, *, missing_label: str = "Path does not exist") -> str | None:
    ...
    try:
        ...
        return (
            ...
            f"Folders permission to the Cybernetics/WebUI app or server process, then try again."
        )
