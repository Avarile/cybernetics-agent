"""Cybernetics WebUI -- Session model and in-memory session store."""
import collections
import copy
import datetime
import hashlib
...
def _active_state_db_path() -> Path:
    """Return state.db for the active Cybernetics profile, degrading to HERMES_HOME."""
    try:
...
def _sidebar_title_is_generic_webui(title: str | None) -> bool:
    ...
    if text == 'Cybernetics WebUI':
        return True
    prefix = 'Cybernetics WebUI #'
...
def get_state_db_session_messages(sid, *, stitch_continuations: bool = False, profile=None) -> list:
    """Read messages for a Cybernetics session from state.db.
    ...
    through another Cybernetics surface such as the Gateway API Server.  When
