...
def _run_codex_runtime_command(arg_string: str) -> str:
    """Execute Cybernetics' shared Codex runtime switch for the active profile."""
    try:
...
def _run_credits_command() -> str:
    """Render Cybernetics' shared credits view for the WebUI slash-command path."""
    try:
    ...
    if not getattr(view, "logged_in", False):
        return "Not logged into Nous. Run `hermes auth login nous` in Cybernetics CLI, then try /credits again."
...
def execute_plugin_command(command: str) -> str:
    ...
    transport errors, matching Cybernetics' existing slash-command UX.
    """
