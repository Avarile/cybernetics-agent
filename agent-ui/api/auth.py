"""
Cybernetics WebUI -- optional authentication.
Off by default. Enable by setting CYBERNETICS_WEBUI_PASSWORD, configuring a
...
COOKIE_NAME = 'cybernetics_session'
LEGACY_COOKIE_NAME = 'hermes_session'
CSRF_HEADER_NAME = 'X-Cybernetics-CSRF-Token'
...
def _resolve_cookie_name() -> str:
    ...
    Honours ``CYBERNETICS_WEBUI_COOKIE_NAME`` so multiple WebUI instances sharing a
    ...
    """
    name = (
        os.getenv('CYBERNETICS_WEBUI_COOKIE_NAME', '').strip()
        or os.getenv('CYBERNETICS_WEBUI_COOKIE_NAME', '').strip()
    )
    if not name:
        return COOKIE_NAME
    if _COOKIE_NAME_RE.match(name):
        return name
    logger.warning(
        'Ignoring invalid CYBERNETICS_WEBUI_COOKIE_NAME=%r; falling back to %r '
...
def parse_cookie(handler) -> str | None:
    ...
    Reads the configured cookie name first, then falls back to the legacy
    ``hermes_session`` name so users who upgrade across the rebrand are not
    forcibly logged out — sessions stay valid until they expire naturally.
    """
    ...
    if morsel is None and _resolve_cookie_name() != LEGACY_COOKIE_NAME:
        morsel = cookie.get(LEGACY_COOKIE_NAME)
