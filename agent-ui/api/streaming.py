"""
Cybernetics WebUI -- SSE streaming engine and agent thread runner.
...
def _resolve_custom_provider_runtime_overrides(
    ...
    Cybernetics Agent treats named custom providers as routing hints around an
...
- Match the normal Cybernetics messaging style, but do not let long tool-running WebUI turns appear silent.
...
def _load_webui_prefill_context(
    ...
    Supports the same bounded JSON-file shape used by Cybernetics Agent.  WebUI also
...
def _cancelled_turn_hint(agent_name: str | None = None) -> str:
    name = str(agent_name or _preferred_agent_display_name()).strip() or 'Cybernetics'
...
def _is_synthetic_max_iteration_summary_request(message) -> bool:
    """Return True for Cybernetics Agent's internal max-iteration summary prompt."""
...
def _stale_completion_max_age_seconds() -> float:
    ...
    contaminate an unrelated later turn. See nesquena/cybernetics-webui#4029.
...
def _drain_webui_process_notifications(session_id: str) -> list[str]:
    ...
    while True:
        ...
        # (nesquena/cybernetics-webui#4029). Drop (consume, do not requeue) any
...
def _build_native_multimodal_message(workspace_ctx: str, msg_text: str, attachments, workspace: str, *, cfg: dict = None):
    ...
    bytes to Cybernetics as OpenAI-style image_url data URLs so vision-capable main
...
def _merge_display_messages_after_agent_result(previous_display, previous_context, result_messages, msg_text):
    ...
    If Cybernetics Agent returns a normal append-only history, append that delta to
...
def _run_agent_streaming(
    ...
    *,
    ...
    # rewritten yet.  See https://github.com/nesquena/cybernetics-webui/issues/1968.
    ...
    try:
        ...
        def _clarify_callback_impl(question, choices, sid, cancel_evt, put_event):
            """Bridge Cybernetics clarify prompts to the WebUI."""
        ...
        try:
            ...
            def on_tool(*cb_args, **cb_kwargs):
                ...
                # Modern Cybernetics Agent builds can call both tool_progress_callback
                ...
                if event_type == 'tool.completed':
                    ...
                    # `result` kwarg from modern Cybernetics builds; fall
            ...
            # provider recovery). Match Cybernetics CLI/gateway semantics:
            ...
            # after the operator raises Cybernetics' global turn budget.
            ...
            try:
                ...
                if _raw_max_iterations is None and isinstance(_cfg, dict):
                    # Back-compat for older Cybernetics config files that used a
            ...
            with _agent_lock:
                ...
                if not ephemeral:
                    ...
                    # every completed turn.  This mirrors Cybernetics CLI where
            ...
            # /goal parity: after a successful assistant turn, run the Cybernetics
