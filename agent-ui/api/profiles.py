"""
Cybernetics WebUI -- Profile state management.
...
def _unwrap_profile_home_to_base(home: Path) -> Path:
    """Return the base Cybernetics home when *home* is already a named profile dir."""
...
#
# Cybernetics Agent allows the root/default profile (~/.hermes itself) to have a
...
def _is_root_profile(name: str) -> bool:
    """True if *name* resolves to the Cybernetics Agent root profile (~/.hermes).
...
def _resolve_profile_home_for_name(name: str) -> Path:
    """Resolve a logical profile name to its Cybernetics home path.
...
def profile_env_for_background_worker(
    ...
    # worker body because several production Cybernetics readers still call
...
# the key must be written to the profile's .env file so that Cybernetics Agent's
