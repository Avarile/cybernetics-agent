"""Default-off Cybernetics Gateway bridge for browser-originated chat turns."""
...
def webui_chat_backend_mode(config_data=None, environ: dict[str, str] | None = None) -> str:
    ...
    values opt browser chat into the Cybernetics API server bridge; generic truthy
...
def _gateway_http_error_event(exc: urllib.error.HTTPError, err_body: str, *, api_key_configured: bool) -> dict:
    ...
    if exc.code == 401:
        return {
            ...
            "hint": (
                "Set HERMES_WEBUI_GATEWAY_API_KEY to the same value as the Cybernetics Gateway "
                "API_SERVER_KEY, or disable HERMES_WEBUI_CHAT_BACKEND=gateway."
                if not api_key_configured
                else "Check that HERMES_WEBUI_GATEWAY_API_KEY matches the Cybernetics Gateway API_SERVER_KEY."
            ),
        }
    return {
...
def _gateway_tool_progress_event(payload: dict) -> tuple[str, dict] | None:
    """Translate Cybernetics Gateway tool-progress SSE payloads to WebUI events."""
    if not isinstance(payload, dict):
        return None
...
def _run_gateway_runs_api_streaming(
    ...
    *, put_gateway_event, cancel_event,
    ...
    headers = {
        ...
        "X-Cybernetics-Session-Id": session_id,
    }
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}"
        headers["X-Cybernetics-Session-Key"] = f"webui:{session_id}"
...
def _run_gateway_chat_streaming(
    ...
    *,
    ...
    """Bridge a WebUI chat turn through Cybernetics Gateway's API server.
    ...
    try:
        ...
        if _use_runs_api:
            ...
            try:
                ...
                put_gateway_event("apperror", {
                    ...
                    "message": str(exc)[:400],
                    "hint": "Check that the Cybernetics Gateway runs API (/v1/runs) is available.",
                })
                return
        ...
        if not assistant_text:
            put_gateway_event("apperror", {
                ...
                "hint": "Check that Cybernetics Gateway API server is running and reachable.",
            })
            return
