"""
Cybernetics WebUI -- Route handlers for GET and POST endpoints.
...
def _active_skills_dir() -> Path:
    """Return the skills directory for the request's active Cybernetics profile.
...
def _active_profile_config_path() -> Path:
    ...
    Cybernetics home, not process-global HERMES_HOME or HERMES_CONFIG_PATH values
    ...
    try:
        ...
        def _persist_success():
            ...
            try:
                ...
                # not expose the newer delivery_error parameter. Real Cybernetics
...
def _should_hide_stale_messaging_session(
    ...
    Cybernetics Gateway keeps the external conversation identity in sessions.json.
...
def _handle_logs(handler, parsed) -> bool:
    """Return a bounded tail window for an active-profile Cybernetics log file."""
...
def _build_llm_wiki_status() -> dict:
    ...
    try:
        ...
        base = {
            ...
            "toggle_reason": "Cybernetics Agent exposes WIKI_PATH/wiki.path for location, but no stable on/off config flag is currently available.",
...
def _handle_insights(handler, parsed) -> bool:
    ...
    # ── Also include CLI sessions from Cybernetics state.db ─────────────────────
    try:
...
def _deep_health_checks(stream_check: dict | None = None) -> tuple[dict, bool]:
    ...
    shared stream map, sidebar/session path, project state, and Cybernetics state.db
...
def _get_plugin_manager_for_visibility():
    """Return Cybernetics Agent's plugin manager for read-only WebUI visibility."""
...
def _plugin_visibility_payload(manager=None) -> dict:
    ...
    The Cybernetics Agent manager stores manifests and callback objects internally.
    ...
    # Cybernetics Agent lifecycle-hook plugins
...
<head>
  ...
  <title>Cybernetics is restarting</title>
</head>
<body style=\"margin:0;padding:2rem;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#111827;color:#e5e7eb;\">
  <main style=\"max-width:40rem;margin:10vh auto;line-height:1.5;\">
    <h1 style=\"font-size:1.5rem;margin:0 0 0.75rem;\">Cybernetics is restarting…</h1>
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path == "/login":
        ...
        _bn = _html.escape(_settings.get("bot_name") or "Cybernetics")
            ...
            return j(
                handler,
                {
                    "webui": {
                        ...
                        "latest_sha": "def5678",
                        "branch": "master",
                        "repo_url": "https://github.com/nesquena/cybernetics-webui",
                        "compare_url": "https://github.com/nesquena/cybernetics-webui/compare/abc1234...def5678",
                    },
                    "agent": {
                        "name": "agent",
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == "/api/settings":
        ...
        if "bot_name" in body:
            body["bot_name"] = (str(body["bot_name"]) or "").strip() or "Cybernetics"
        ...
        # #1560: CYBERNETICS_WEBUI_PASSWORD env var takes precedence in
        ...
        if requested_password or requested_clear_password:
            if os.getenv("HERMES_WEBUI_PASSWORD", "").strip():
                return bad(
                    handler,
                    "CYBERNETICS_WEBUI_PASSWORD env var is set — it overrides the settings password. "
        ...
        def _llm_update_summary(system_prompt: str, user_prompt: str) -> str:
            ...
            with profiles_api.profile_env_for_background_worker(
                ...
                try:
                    ...
                    # discover in the Cybernetics Agent setup/config UI.
...
def _handle_tts(handler, parsed):
    ...
    if engine == "elevenlabs":
        ...
        if not api_key:
            # Fall back to reading from Cybernetics .env file
            try:
        ...
        # Resolve voice_id from Cybernetics config.yaml → env fallback
...
def _handle_media(handler, parsed):
    ...
    # ── #3234: hard-deny Cybernetics's own state + secret/config files ────────────
    # The allowlist above grants the whole Cybernetics home (and base ~/.hermes), so
    ...
    # Cybernetics root. The deny rules target Cybernetics's OWN internal state, which lives
    ...
    # the internal state subdirectories across every Cybernetics root the allowlist
    ...
    # Cybernetics root in its own right, so a sibling/other profile's sensitive subdirs
    ...
    # State-subdir deny set: each DENY_SUBDIR directly under any Cybernetics root
    ...
    # location ($HOME, a Cybernetics root itself, an ANCESTOR of a Cybernetics root, a
    ...
    # Cybernetics root (e.g. STATE_DIR/workspace) is still a legit project workspace
    ...
    # Filename-based denies fire for files under a Cybernetics root, UNLESS the file
...
def _normalize_chat_attachments(raw_attachments):
    ...
    to Cybernetics as native multimodal inputs for the current turn.
    """
    normalized = []
...
def _handle_chat_sync(handler, body):
    ...
    try:
        ...
        with CHAT_LOCK:
            ...
            # Resolve API key via Cybernetics runtime provider (matches gateway behaviour)
            _api_key = None
            try:
                from api.oauth import resolve_runtime_provider_with_anthropic_env_lock
                from hermes_cli.runtime_provider import resolve_runtime_provider
            ...
            agent = AIAgent(
                ...
                base_url=_base_url,
                api_key=_api_key,
                # Identify browser-originated sessions as WebUI so Cybernetics Agent
...
def _handle_file_open_vscode(handler, body):
    ...
    Cybernetics WebUI inside Docker still open files in their local editor.
    """
    try:
        require(body, "session_id", "path")
    except ValueError as e:
...
def _handle_handoff_summary(handler, body):
    ...
    def _agent_text_completion(agent, system_prompt, user_text, max_tokens=700):
        """Use the current Cybernetics Agent transport without mutating conversation history."""
        api_messages = [
            {"role": "system", "content": system_prompt},
...
def _parse_mcp_enabled(value) -> bool:
    """Parse Cybernetics MCP ``enabled`` values without raising on bad config."""
    if value is None:
        return True
...
def _mcp_runtime_status_by_name() -> dict[str, dict]:
    ...
    configuration; it does not probe or spawn MCP subprocesses. If Cybernetics Agent
