"""Cybernetics agent/gateway heartbeat payload helpers (#716, #1879).

The WebUI process is not always paired with a long-running Cybernetics gateway. Some
setups use WebUI only, while self-hosted messaging deployments run a separate
Cybernetics gateway daemon that records runtime metadata in the Cybernetics Agent home.
...
where the WebUI runs separately from ``hermes-agent`` and only a Cybernetics data
...
def _gateway_root_pid_path() -> Path | None:
    """Return the root Cybernetics gateway PID path.
...
def build_agent_health_payload() -> dict[str, Any]:
    """Return `{alive, checked_at, details}` for the Cybernetics gateway/agent.
