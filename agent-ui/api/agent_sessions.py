"""Shared helpers for reading Cybernetics Agent sessions from state.db."""
import logging
import sqlite3
...
def normalize_agent_session_source(raw_source: str | None) -> dict:
    """Return stable source metadata for Cybernetics Agent session rows.
...
def read_importable_agent_session_rows(
    ...
    Cybernetics Agent can create rows in ``state.db.sessions`` before a session has
    ...
    sidebar. This mirrors Cybernetics Agent CLI's session-list behaviour: interactive
    ...
    with closing(sqlite3.connect(str(db_path))) as conn:
        ...
        # Older Cybernetics Agent versions may not have source tracking. Without a
        ...
        if limit is not None:
            ...
            # aggregating every historical Cybernetics state.db session before
...
def read_session_lineage_metadata(db_path: Path, session_ids: list[str] | set[str]) -> dict[str, dict]:
    ...
    WebUI sessions are persisted as JSON files, but Cybernetics Agent also mirrors
