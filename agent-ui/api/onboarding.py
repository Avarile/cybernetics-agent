"""Cybernetics WebUI -- first-run onboarding helpers."""
...
_UNSUPPORTED_PROVIDER_NOTE = (
    ...
    "when your Cybernetics config selects the corresponding provider."
)
...
def probe_provider_endpoint(
    ...
    headers = {
        ...
        "User-Agent": "cybernetics-webui-onboarding-probe",
    }
    if api_key:
...
def _provider_oauth_authenticated(provider: str, hermes_home: "Path") -> bool:
    ...
    requested Cybernetics home. Known OAuth providers may store auth either in the
    legacy providers[provider_id] singleton state or in credential_pool entries
    used by current Cybernetics runtime auth resolution.
    """
    provider = (provider or "").strip().lower()
...
def _status_from_runtime(cfg: dict, imports_ok: bool) -> dict:
    ...
    if not _HERMES_FOUND or not imports_ok:
        ...
        note = (
            "Cybernetics is not fully importable from the Web UI yet. Finish bootstrap or fix the "
            "agent install before provider setup will work."
        )
    elif chat_ready:
        state = "ready"
        provider_name = _PROVIDER_DISPLAY.get(
            provider, provider.title() if provider else "Cybernetics"
        )
        note = f"Cybernetics is minimally configured and ready to chat via {provider_name}."
    elif provider_configured:
        state = "provider_incomplete"
        if provider == "custom" and not base_url:
            note = (
                "Cybernetics has a saved provider/model selection but still needs the "
        ...
        note = "Cybernetics is installed, but you still need to choose a provider and save working credentials."
...
def get_onboarding_status() -> dict:
    ...
    # Auto-complete for existing Cybernetics users: if config.yaml already exists
    # AND the provider is configured (or the system is chat_ready), treat onboarding
    # as done.  These users configured Cybernetics via the CLI before the Web UI existed;
    ...
    return {
        ...
        "settings": {
            ...
            "bot_name": settings.get("bot_name") or "Cybernetics",
        },
        "system": {
...
def apply_onboarding_setup(body: dict) -> dict:
    ...
    if Path(config_path).exists() and not body.get("confirm_overwrite"):
        return {
            "error": "config_exists",
            "message": (
                "Cybernetics is already configured (config.yaml exists). "
