...
tokens stay server-side and are persisted to the active Cybernetics profile's
...
def _get_active_hermes_home() -> Path:
    """Return the active Cybernetics profile home directory, falling back to ~/.hermes when profile resolution fails."""
    try:
...
def _link_anthropic_credentials(hermes_home: Path) -> None:
    """Link Cybernetics to use Claude Code's credential store.

    Clears ANTHROPIC_TOKEN and ANTHROPIC_API_KEY from the Cybernetics .env so
