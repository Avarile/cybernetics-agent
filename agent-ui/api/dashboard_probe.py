"""Safe server-side probe for the official Cybernetics Agent dashboard.
...
def _looks_like_official_dashboard(payload: object) -> bool:
    ...
    # /api/status returns version plus these Cybernetics-specific fields. Requiring at
...
def probe_official_dashboard(
    ...
    try:
        ...
        request = urllib.request.Request(
            f"{base}/api/status",
            headers={"Accept": "application/json", "User-Agent": "cybernetics-webui-dashboard-probe"},
        )
        ...
        logger.debug("official Cybernetics dashboard probe failed", exc_info=True)
        return {"running": False}
