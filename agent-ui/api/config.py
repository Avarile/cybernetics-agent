"""
Cybernetics WebUI -- Shared configuration, constants, and global state.
...
# ── Cybernetics agent directory discovery ─────────────────────────────────────────
def _discover_agent_dir() -> Path:
    """
    Locate the agent checkout using a multi-strategy search.

    Each strategy is tried for both the current ``cybernetics-agent`` name and
    the legacy ``hermes-agent`` name so existing installs keep working.
    """
    candidates = []

    explicit = os.getenv("CYBERNETICS_WEBUI_AGENT_DIR") or os.getenv("HERMES_WEBUI_AGENT_DIR")
    if explicit:
        candidates.append(Path(explicit).expanduser().resolve())

    agent_home = (
        os.getenv("CYBERNETICS_HOME")
        or os.getenv("HERMES_HOME")
        or str(_DEFAULT_HERMES_HOME)
    )
    for name in ("cybernetics-agent", "hermes-agent"):
        candidates.append(Path(agent_home).expanduser() / name)
        candidates.append(REPO_ROOT.parent / name)
        candidates.append(_DEFAULT_HERMES_HOME / name)
    ...
    for name in ("cybernetics-agent", "hermes-agent"):
        candidates.append(xdg_data.expanduser() / name)
        for sys_prefix in ("/opt", "/usr/local", "/usr/local/share"):
            candidates.append(Path(sys_prefix) / name)
...
def _discover_python(agent_dir: Path) -> str:
    """
    Locate a Python executable that has the Cybernetics agent dependencies installed.
...
# ── Inject agent dir into sys.path so Cybernetics modules are importable ──────────
...
# alongside real Cybernetics source files.  Putting _AGENT_DIR at the
...
# and Cybernetics-specific modules (run_agent, hermes/, etc.) still
...
def _save_yaml_config_file(config_path: Path, config_data: dict) -> None:
    try:
        ...
        raise RuntimeError("PyYAML is required to write Cybernetics config.yaml") from exc
...
def print_startup_config() -> None:
    ...
    lines = [
        "",
        "  Cybernetics WebUI -- startup config",
        "  --------------------------------",
    ...
    if not _HERMES_FOUND:
        print(
            f"{err}  Could not find the Cybernetics agent directory.\n"
...
def verify_hermes_imports() -> tuple:
    """
    Attempt to import the key Cybernetics modules.
...
_LEGACY_CLI_TOOLSET_ALIASES = {
    # Older Cybernetics configs used "hermes" as the CLI composite toolset. Modern
    # Cybernetics Agent exposes that split as these two registered composites; keep
...
# Provider display names for known Cybernetics provider IDs
_PROVIDER_DISPLAY = {
    "nous": "Nous Portal",
...
def _provider_is_known_or_configured(
    ...
    """True when ``provider_id`` is a provider Cybernetics recognizes (static registry)
    ...
    This distinguishes a provider Cybernetics knows how to route (e.g. ``ollama-cloud``,
...
def get_effective_default_model(config_data: dict | None = None) -> str:
    """Resolve the effective Cybernetics default model from config, then env overrides."""
...
def _models_dev_reasoning_efforts(model_id: str, provider_id: str) -> list[str] | None:
    """Return reasoning efforts from Cybernetics Agent model metadata when known.
...
def _lmstudio_reasoning_probe_options_fallback(
    ...
    *,
    ...
    headers = {
        ...
        "User-Agent": "cybernetics-webui-reasoning-probe",
    }
    if api_key:
...
def set_hermes_default_model(model_id: str, advanced: dict | None = None) -> dict:
    """Persist the Cybernetics default model in config.yaml and reload runtime config."""
...
def _credential_pool_profile_tag() -> str:
    ...
    The credential pool is per-Cybernetics-profile (it lives in that profile's
...
def _get_auth_store_path() -> Path:
    """Return the auth.json path for the active Cybernetics profile."""
    try:
...
def _read_live_provider_model_ids(provider_id: str) -> list[str]:
    """Return live model IDs from Cybernetics CLI for a provider, or [] on failure.
    ...
    display-facing IDs (for example ``google`` / ``x-ai``) that Cybernetics CLI may
...
def _models_from_live_provider_ids(provider_id: str, live_ids: list[str]) -> list[dict]:
    """Convert Cybernetics CLI model ids into WebUI picker model entries."""
        ...
        if detected_providers:
            ...
            for pid in sorted(detected_providers):
                ...
                if pid == "openrouter":
                    ...
                    # Otherwise, ask Cybernetics CLI first so WebUI tracks the same
...
_SETTINGS_DEFAULTS = {
    ...
    "bot_name": os.getenv(
        "HERMES_WEBUI_BOT_NAME", "Cybernetics"
