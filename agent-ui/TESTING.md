# Cybernetics WebUI: Browser Testing Plan
...
SETUP: Clear localStorage (DevTools > Application > Local Storage > delete cybernetics-webui-session) or open in incognito.
STEPS:
...
EXPECT:
  ...
  - Sidebar footer shows a single "Cybernetics WebUI" control-center button
...
EXPECT:
  ...
  - After a few seconds, Cybernetics responds
...
EXPECT:
  ...
  - Within 10-30 seconds, Cybernetics responds with a three-word greeting
...
EXPECT:
  ...
  - Cybernetics responds describing or reading the file content
FAIL: Upload fails, file badge never appears, Cybernetics does not mention the file.
...
STEPS:
  1. Click the "Cybernetics" button in the sidebar footer
...
4. For T7.x (approval tests): the agent running inside Cybernetics needs to detect a
   dangerous command. Ask Cybernetics to "run: rm -rf /tmp/test_hermes_approval" and watch
...
EXPECT:
  - A small clipboard icon appears in the message header row (right side of "You" or "Cybernetics")
...
EXPECT:
  - "You" and "Cybernetics" labels appear in slightly muted blue/gold (not full-brightness)
...
EXPECT:
  ...
  - Cybernetics streams a fresh response
...
EXPECT:
  ...
  - Cybernetics streams a new response
...
EXPECT:
  - The "Cybernetics" button is visible in the sidebar footer
...
SETUP: Ask Cybernetics something that produces a code response (e.g. "Show me a Python hello world").
EXPECT:
...
STEPS:
  1. While Cybernetics is responding, type a new message and press Enter
EXPECT:
...
- Click the "Cybernetics WebUI" button in the sidebar footer. Control Center overlay opens with vertical section tabs on the left.
