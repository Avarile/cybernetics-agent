...
How to reach a self-hosted Cybernetics WebUI from another machine or your phone.

## Accessing from a remote machine

The server binds to `127.0.0.1` by default (loopback only). If you are running
Cybernetics on a VPS or remote server, use an SSH tunnel from your local machine:
...
The Cybernetics WebUI is fully responsive with a mobile-optimized layout
...
A community report in [#2364](https://github.com/nesquena/cybernetics-webui/issues/2364)
documents Cybernetics Agent + WebUI running on a mid-range ARM64 Android phone inside
