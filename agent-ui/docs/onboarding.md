...
This guide explains what happens the first time Cybernetics WebUI starts, which
...
Cybernetics WebUI is only the browser interface. The actual agent runtime, memory,
skills, config, cron jobs, and provider credentials belong to Cybernetics Agent.
...
[#1952](https://github.com/nesquena/cybernetics-webui/issues/1952), including:
...
your real Cybernetics config, credentials, memory, skills, profiles, sessions, and
cron state.

For a clean local trial, use an isolated Cybernetics home and WebUI state directory:
...
- Cybernetics Agent importability: whether WebUI can import and run `AIAgent`.
...
For API-key providers, the wizard writes the key to the active Cybernetics `.env`
...
first-class built-in Cybernetics provider id. Configure it under the
...
the onboarding flow when your Cybernetics config selects the corresponding provider.
...
The workspace is the filesystem location Cybernetics should use for new sessions.
...
- Active Cybernetics `config.yaml`: provider, default model, and Base URL when
  relevant.
- Active Cybernetics `.env`: provider API keys when you entered one.
...
- Cybernetics Agent state: Windows `%LOCALAPPDATA%\hermes`; POSIX `~/.hermes`
