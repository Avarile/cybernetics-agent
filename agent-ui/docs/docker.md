# Cybernetics WebUI — Docker setup guide
...
| **All-in-one image** (community fork — third-party, not maintained by us) | Podman 3.4 / multi-arch / supervisord-style preference. | [sunnysktsang/hermes-suite](https://github.com/sunnysktsang/hermes-suite) — see [#1399](https://github.com/nesquena/cybernetics-webui/issues/1399) for the original discussion |
...
Cybernetics WebUI assumes one operator controls the container, mounted Cybernetics home, and workspace.
...
them manually from the Tasks panel. In Docker, scheduled jobs require the Cybernetics gateway daemon
...
isolated Cybernetics home and follow
[`docs/onboarding-agent-checklist.md`](onboarding-agent-checklist.md) instead.

> **Linux note**: run Compose as the user who owns the Cybernetics home. The command
...
> your user's real Cybernetics directory. Prefer adding your user to the `docker group`
...
The default Cybernetics WebUI Docker image stays CPU-only. GPU user-space packages
are installed only when you build a custom image with the opt-in build arg:

```bash
docker build --build-arg INSTALL_GPU_LIBS=1 -t cybernetics-webui:gpu .
```
...
```bash
docker run --rm --gpus all cybernetics-webui:gpu nvidia-smi
```
...
and container toolkit setup before debugging Cybernetics WebUI. The container image
...
| `cybernetics-webui` | `ghcr.io/nesquena/cybernetics-webui:latest` | 8787 (chat UI) |
...
services:
  ...
  cybernetics-webui:
    image: ghcr.io/nesquena/cybernetics-webui:latest
    container_name: cybernetics-webui
    depends_on:
      - hermes-agent
    ports:
      - "127.0.0.1:8787:8787"
...
Until the compatibility boundary work in [#1925](https://github.com/nesquena/cybernetics-webui/issues/1925) and [#2491](https://github.com/nesquena/cybernetics-webui/issues/2491) land, the WebUI and Cybernetics Agent deployment should be treated as a release pair: the WebUI release is tested against its matching agent release and should be upgraded/pinned together.
...
- `cybernetics-webui:latest` + fixed `hermes-agent` tag
...
**Fix**:
- Verify: `docker exec cybernetics-webui ls -la /home/hermeswebui/.hermes/config.yaml`
...
**Symptom**: A provider, local model server, webhook, or custom API works on the host at `http://localhost:<port>`, but fails when the same URL is configured in Cybernetics WebUI running in Docker.
...
```
                 ...
                 └─────────────────────────────────┘
                          ...
                          │              │
      ┌──────────────┐    │              │    ┌──────────────┐
      │ hermes-agent │────┘              └────│ cybernetics-webui │
      │  (port 8642) │                        │  (port 8787) │
      └──────────────┘                        └──────────────┘
              │                                       ↑
              │ rw                                    │ ro
...
This is the root cause of [#1416](https://github.com/nesquena/cybernetics-webui/issues/1416): the symptom looked like a missing entrypoint, but the entrypoint was actually present in the new image and hidden behind the stale named volume.
...
- #3006 — `sudo docker compose` can mount `/root/.hermes` instead of the user's Cybernetics home
...
If you hit a new failure mode not covered here, please [open an issue](https://github.com/nesquena/cybernetics-webui/issues/new) with:

1. Which compose file you used
2. The error from `docker logs cybernetics-webui`
3. `docker exec cybernetics-webui id` output
4. `docker exec cybernetics-webui ls -la /home/hermeswebui/.hermes` output
