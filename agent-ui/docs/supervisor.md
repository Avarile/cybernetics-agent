# Running Cybernetics WebUI under a process supervisor
...
**Important (launchd on macOS):** if the ``com.parantoux.cybernetics-webui`` LaunchAgent is enabled, treat launchd as the single source of truth for WebUI lifecycle. Do **not** also run ``./ctl.sh start``, ``bash start.sh``, ``python bootstrap.py``, or ``python server.py`` against the same state dir/port, or you can create a second WebUI instance and trigger port-8787 restart churn.
...
``~/Library/LaunchAgents/com.example.cybernetics-webui.plist``:
...
<dict>
    <key>Label</key>
    <string>com.example.cybernetics-webui</string>

    <key>ProgramArguments</key>
    <array>
        <string>/bin/bash</string>
        <string>/Users/yourname/cybernetics-webui/start.sh</string>
        <string>--foreground</string>
    </array>

    <key>WorkingDirectory</key>
    <string>/Users/yourname/cybernetics-webui</string>
...
```bash
launchctl load ~/Library/LaunchAgents/com.example.cybernetics-webui.plist
launchctl print gui/$(id -u)/com.example.cybernetics-webui   # check state
```

Reload after editing the plist:

```bash
launchctl unload ~/Library/LaunchAgents/com.example.cybernetics-webui.plist
launchctl load   ~/Library/LaunchAgents/com.example.cybernetics-webui.plist
```
...
``~/.config/systemd/user/cybernetics-webui.service``:

```ini
[Unit]
Description=Cybernetics WebUI
After=network.target

[Service]
Type=simple
WorkingDirectory=%h/cybernetics-webui
ExecStart=/bin/bash %h/cybernetics-webui/start.sh --foreground
Restart=on-failure
RestartSec=5
...
systemctl --user enable --now cybernetics-webui.service
journalctl --user -u cybernetics-webui.service -f
```
...
``/etc/supervisor/conf.d/cybernetics-webui.conf``:

```ini
[program:cybernetics-webui]
command=/bin/bash /home/youruser/cybernetics-webui/start.sh --foreground
directory=/home/youruser/cybernetics-webui
user=youruser
autostart=true
autorestart=true
stopsignal=TERM
stopwaitsecs=10
stdout_logfile=/var/log/cybernetics-webui.out.log
stderr_logfile=/var/log/cybernetics-webui.err.log
...
sudo supervisorctl status cybernetics-webui
```
...
Cybernetics WebUI exposes two health levels:
...
- ``/health?deep=1`` — readiness probe that briefly acquires the stream lock,
  reads the sidebar/session path, reads projects state, and touches Cybernetics
...
#!/usr/bin/env bash
set -euo pipefail
LABEL="com.example.cybernetics-webui"
...
``systemctl --user restart cybernetics-webui.service`` on failure.
