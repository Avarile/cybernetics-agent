...
- **Created:** 2026-06-10
- **Tracking issue:** [#3926](https://github.com/nesquena/cybernetics-webui/issues/3926)
- **Parent contract:** [Live-to-Final Assistant Replies](./live-to-final-assistant-replies.md) ([#3400](https://github.com/nesquena/cybernetics-webui/issues/3400))
- **Related RFCs:** [Transparent Stream](./transparent-stream-activity-mode.md), [Cybernetics Run Adapter Contract](./hermes-run-adapter-contract.md), [WebUI Run State Consistency Contract](./webui-run-state-consistency-contract.md), [Turn Journal](./turn-journal.md), [Pending Intent Controls](./webui-pending-intent-controls.md)
...
- Insulate the presentation layer from the in-progress RuntimeAdapter substrate
  ...
  Cybernetics runtime changes the anchor's input source, not the UI.

## Non-goals

- Do not replace the Live-to-Final product model from #3400.
- Do not implement Transparent Stream in this RFC.
- Do not implement a RuntimeAdapter, runner process, sidecar, or Cybernetics runtime
  API.
...
emits now and a runner or Cybernetics runtime will emit later. `run_id` is the durable
...
emitted by the WebUI run journal today and will be emitted by a runner or Cybernetics
...
from the WebUI run path to a runner or Cybernetics runtime changes the anchor's input
