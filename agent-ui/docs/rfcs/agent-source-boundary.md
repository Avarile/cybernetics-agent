...
- **Status:** Proposed
- **Created:** 2026-05-17
- **Tracking issue:** [#2453](https://github.com/nesquena/cybernetics-webui/issues/2453)

## Problem

The WebUI currently depends on Cybernetics Agent Python source being importable at
...
Cybernetics Agent internal module layout and makes the multi-container setup look more
...
|---|---|---|---|
| Browser chat execution | `run_agent.AIAgent` imported by `api/streaming.py` | Run lifecycle API: start, observe, status, cancel, approval, clarify, final usage | Covered by the runtime-adapter migration in [#1925](https://github.com/nesquena/cybernetics-webui/issues/1925), but still source-backed today. |
...
- Do not replace the runtime adapter or Cybernetics Agent API in this document-only
  inventory slice.
