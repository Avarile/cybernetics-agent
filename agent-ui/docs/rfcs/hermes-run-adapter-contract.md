# Cybernetics Run Adapter Contract and Migration Gates
...
- **Created:** 2026-05-11
- **Revised:** 2026-05-23
- **Tracking issue:** [#1925](https://github.com/nesquena/cybernetics-webui/issues/1925)
...
- future Cybernetics runtime APIs cannot be adopted cleanly because WebUI lacks a
...
- Identify future backend mapping: existing Cybernetics runtime API, missing Cybernetics
...
- Do not depend on Cybernetics Agent shipping a WebUI-specific runtime connector before
...
future WebUI-managed runner, or a future Cybernetics `/v1/runs` backend.
...
| queue / continue | append follow-up work according to Cybernetics semantics | runner/runtime control plane |
...
|---|---|---|---|---|---|
| `STREAMS` / `STREAMS_LOCK` | `api.state_sync` process memory | adapter API surface + presentation fan-out | WebUI runner or future Cybernetics run observation API | keep live path; mirror events into journal | Must stop being authoritative for active run existence. |
...
| cached `AIAgent` objects / `AGENT_INSTANCES` | `api/config.py` process memory | runner process | runner-owned Cybernetics integration | unchanged | Moving this is deferred until after journal proof. |
...
| command capability metadata | WebUI command registry + Cybernetics command assumptions | adapter API surface | runtime command/capability metadata | unchanged | Unknown command support should not be guessed by WebUI. |
...
| Multi-profile isolation | profile-specific runs write/read the correct Cybernetics home and memory | protects #2134-family isolation concerns | runner/profile slice |
| Queue/continue | follow-up input during live/resumable work obeys Cybernetics semantics | prevents parallel continuation model | control migration slice |
...
- future Cybernetics CLI/Python/local API or `/v1/runs` backends can be evaluated
  behind the adapter.
...
4. **Restart/reattach success criterion.** Start a long-running run, restart only
   `cybernetics-webui.service`, reload the session, rediscover the active or terminal
...
- no dependency on Cybernetics Agent shipping `/v1/runs` before WebUI can validate the
...
- carry the runtime API gap matrix forward: missing Cybernetics-owned capabilities
...
- no permanent WebUI-owned active-run discovery cache when the missing capability
  belongs in a runner or future Cybernetics Runtime API;
...
Cybernetics runtime families such as `token.delta`, `tool.started`, and `done` before
...
- no permanent WebUI-owned active-run discovery cache that duplicates runner or
  future Cybernetics Runtime API responsibility.
...
1. **Process ownership moved.** A local runner process, not `cybernetics-webui`, owns
...
2. **Restart/reattach with a real runner.** Start a non-trivial `runner-local`
   run, restart only `cybernetics-webui`, reload the session, rediscover the active or
...
- no server-side queue scheduler just for adapter symmetry;
- no broad WebUI product-surface migration;
- no claim that this is the canonical Cybernetics Agent Runtime API; if Cybernetics Agent
...
2. Restart only `cybernetics-webui`.
...
- If Cybernetics Agent later ships a durable `/v1/runs` API, which adapter fields map
