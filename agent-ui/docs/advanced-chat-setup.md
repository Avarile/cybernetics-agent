...
Two optional features for self-hosted Cybernetics WebUI deployments. **Most users need neither** — the defaults (in-process chat, no prefill) work out of the box.
...
Cybernetics Gateway API server while preserving the existing WebUI `/api/chat/start`
...
The bridge is best used by operators who already run Cybernetics Gateway/API Server
