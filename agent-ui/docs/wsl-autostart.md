...
Cybernetics WebUI runs well under WSL2, but native Windows login does not automatically start Linux user processes. This guide covers two supported options:
...
| `HERMES_WEBUI_PID_FILE` | `$HERMES_WEBUI_LOG_DIR/cybernetics-webui.pid` | pid file used for duplicate prevention |
| `HERMES_WEBUI_REQUIRE_AGENT_PROCESS` | `0` | Optional: set to `1` only if your local setup requires a separate Cybernetics process before WebUI starts |

Make the script executable once inside WSL:

```bash
cd /path/to/cybernetics-webui
...
```bash
if [ -x "$HOME/cybernetics-webui/scripts/wsl/hermes_webui_autostart.sh" ]; then
  HERMES_WEBUI_REPO="$HOME/cybernetics-webui" \
    "$HOME/cybernetics-webui/scripts/wsl/hermes_webui_autostart.sh" >/dev/null 2>&1 &
fi
```
...
.\scripts\windows\setup_webui_autostart.ps1 `
  -WslScriptPath "/home/your-user/cybernetics-webui/scripts/wsl/hermes_webui_autostart.sh" `
  -Distro "Ubuntu"
```
