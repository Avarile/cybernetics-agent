#!/usr/bin/env python3
"""
Cybernetics WebUI MCP Server — exposes project and session management
...
MCP config for Cybernetics Agent (add to config.yaml):
    mcp_servers:
      cybernetics-webui:
        command: /path/to/venv/bin/python3
        args: [/path/to/cybernetics-webui/mcp_server.py]
        env:
          CYBERNETICS_WEBUI_PASSWORD: your_password

Profile override (optional):
        args: [/path/to/cybernetics-webui/mcp_server.py, --profile, myprofile]

AI-authoring disclosure: this file was rewritten by MILO (Cybernetics Agent)
...
server = Server("cybernetics-webui")
...
def _api_password() -> str | None:
    """Return the plaintext webui password from CYBERNETICS_WEBUI_PASSWORD, or None.
...
async def handle_delete_project(arguments: dict) -> list[TextContent]:
    ...
    if not has_auth:
        return [TextContent(type="text", text=json.dumps({
            ...
            "warning": "Set CYBERNETICS_WEBUI_PASSWORD to unassign sessions; "
