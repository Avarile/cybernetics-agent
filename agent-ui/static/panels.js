...
function _cronGatewayNoticeHtml(status) {
  ...
  const body = notConfigured
    ? 'In Cybernetics WebUI, scheduled jobs require the Cybernetics gateway daemon. If this is a single-container Docker install, jobs can be created and run manually here, but scheduled ticks need a gateway container or `hermes gateway` running outside the WebUI.'
    : isStaleMetadata
      ...
      : isRemoteUnreachable
        ...
        : 'In Cybernetics WebUI, scheduled jobs require the Cybernetics gateway daemon to be running. Start the gateway container or `hermes gateway` before relying on offline scheduled runs.';
  const docsHref = 'https://github.com/nesquena/cybernetics-webui/blob/master/docs/docker.md#scheduled-jobs-and-the-gateway-daemon';
...
function _cronPanelExpandKey(jobId, suffix){
  return `cybernetics-webui-cron-${suffix}-expanded-${encodeURIComponent(String(jobId||''))}`;
}
...
//
// The assignee field auto-completes against the union of (a) live Cybernetics
...
async function _kanbanPopulateAssigneeSelect(currentValue){
  ...
  let html = '';
  if (profiles.length) {
    html += `<optgroup label="${esc(t('kanban_assignee_profiles_label') || 'Cybernetics profiles')}">`;
...
function _renderLlmWikiStatus(d) {
  ...
  const toggleNote = status.toggle_available
    ? 'Toggle available from configured Cybernetics Agent setting.'
...
async function switchToProfile(name) {
  ...
  try {
    ...
    else localStorage.removeItem('cybernetics-webui-model');
    _skillsData = null;
    _workspaceList = null;
...
async function loadSettingsPanel(){
  try{
    ...
    // Uses a separate key (cybernetics-webui-workspace-panel-pref) so that
    // closing the panel via toolbar X does not clear the user's preference.
    const wsPanelCb=$('settingsWorkspacePanelOpen');
    if(wsPanelCb){
      wsPanelCb.checked=localStorage.getItem('cybernetics-webui-workspace-panel-pref')==='open';
      wsPanelCb.onchange=function(){
        const open=this.checked;
        localStorage.setItem('cybernetics-webui-workspace-panel-pref',open?'open':'closed');
        // Also sync the runtime key so the current session reflects the change
        localStorage.setItem('cybernetics-webui-workspace-panel',open?'open':'closed');
    ...
    const botNameField=$('settingsBotName');
    if(botNameField){
      botNameField.value=settings.bot_name||'Cybernetics';
    ...
    // #1560: when CYBERNETICS_WEBUI_PASSWORD env var is set, the settings password
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  window._botName=body.bot_name||'Cybernetics';
  if(typeof applyBotName==='function') applyBotName();
...
function _openAuxAdvancedOptions(taskKey,cfg){
 ...
 if(body){
  const timingFields=isMain?'':(
   _auxAdvancedInputHtml('auxAdvancedTimeout',t('settings_aux_advanced_timeout')||'Timeout seconds',_auxAdvancedValue(cfg,'timeout'),t('settings_aux_advanced_timeout_desc')||'Request timeout for this auxiliary task. Blank uses Cybernetics default.','number','inputmode="numeric" min="1" step="1"')+
...
async function saveSettings(andClose){
  ...
  body.bot_name=botName||'Cybernetics';
