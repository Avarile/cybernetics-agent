...
async function send(){
  ...
  try{
    ...
    if(startData.effective_model && S.session){
      ...
      localStorage.setItem('cybernetics-webui-model', startData.effective_model);
    ...
    if(e&&e.status===404){
      try{ localStorage.removeItem('cybernetics-webui-session'); }catch(_){ }
      try{
...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _streamFadeNextText(targetText){
    ...
    // appear to accelerate. Cybernetics has a playout buffer, so track incoming word
      ...
      const _finishDone=()=>{
        ...
        if(isActiveSession){
          ...
          if(S.session&&S.session.session_id){
            try{localStorage.setItem('cybernetics-webui-session',S.session.session_id);}catch(_){}
    ...
    source.addEventListener('cancel',e=>{
      ...
      (async()=>{
        try{
          ...
          if(S.session&&S.session.session_id===activeSid){
            clearLiveToolCards();if(!assistantText)removeThinking();
            const cancelAgentName=(assistantDisplayName()+'').trim()||'Cybernetics';
...
function transcript(){
  const lines=[`# Cybernetics session ${S.session?.session_id||''}`,``,
...
function _notificationOptions(body,options={}){
  ...
  return {body:body||'',tag:sid?`hermes-${sid}`:'cybernetics-webui',renotify:false,icon:'static/favicon-192.png',badge:'static/favicon-32.png',data:{url}};
}
