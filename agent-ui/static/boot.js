...
(function(){
  ...
  try{localStorage.removeItem('cybernetics-webui-server-stopped');}catch(_){}
  // Listen for shutdown broadcast from other tabs
  try {
    var _stopChan = new BroadcastChannel('cybernetics-webui-shutdown');
...
function _setWorkspacePanelMode(mode){
  ...
  try{localStorage.setItem('cybernetics-webui-workspace-panel', open ? 'open' : 'closed');}catch(_){}
...
(function(){
  ...
  // Prefer Cybernetics server-side STT (MediaRecorder -> /api/transcribe) only
...
$('modelSelect').onchange=async()=>{
  ...
  else try{localStorage.setItem('cybernetics-webui-model',modelState.model)}catch{}
  if(!S.session){
  ...
  // Warn if selected model belongs to a different provider than what Cybernetics is configured for
...
(async()=>{
  ...
  try{
    ...
    window._botName=s.bot_name||'Cybernetics';
  ...
  const _hydrateBootModelDropdown=()=>populateModelDropdown({preferProfileDefaultOnFreshBoot:true}).then(()=>{
    ...
    const savedState=(typeof _readPersistedModelState==='function')
      ...
      : (localStorage.getItem('cybernetics-webui-model')?{model:localStorage.getItem('cybernetics-webui-model'),model_provider:null}:null);
    ...
    if(savedModel && $('modelSelect')){
      ...
      if(!applied&&sessionModelState&&typeof _ensureModelOptionInDropdown==='function'){
        ...
        if(typeof _clearPersistedModelState==='function') _clearPersistedModelState();
          ...
          localStorage.removeItem('cybernetics-webui-model');
          localStorage.removeItem('cybernetics-webui-model-state');
        }
      }
  ...
  const savedLocal=localStorage.getItem('cybernetics-webui-session');
  ...
  if(saved){
    try{
      ...
      if(S.session && (S.session.message_count||0) === 0 && !_restoredInFlight && !_restoredHasDraft){
        ...
        const _ephPanelPref=localStorage.getItem('cybernetics-webui-workspace-panel-pref')==='open'
          || localStorage.getItem('cybernetics-webui-workspace-panel')==='open';
      ...
      const panelPref=localStorage.getItem('cybernetics-webui-workspace-panel-pref')==='open'
        || localStorage.getItem('cybernetics-webui-workspace-panel')==='open';
    ...
    catch(e){localStorage.removeItem('cybernetics-webui-session');}
  }
  ...
  const _freshPanelPref=localStorage.getItem('cybernetics-webui-workspace-panel-pref')==='open'
    || localStorage.getItem('cybernetics-webui-workspace-panel')==='open';
...
async function shutdownServer() {
  const ok = await showConfirmDialog({
    title: (typeof t === 'function' ? t('settings_shutdown_confirm_title') : 'Stop Cybernetics WebUI'),
    message: (typeof t === 'function' ? t('settings_shutdown_confirm_message') : 'Stop the Cybernetics WebUI server?'),
    confirmLabel: (typeof t === 'function' ? t('settings_shutdown_confirm_btn') : 'Stop'),
    danger: true,
  });
  if (!ok) return;
  localStorage.setItem('cybernetics-webui-server-stopped', '1');
  try { var bc = new BroadcastChannel('cybernetics-webui-shutdown'); bc.postMessage('stop'); bc.close(); } catch(_) {}
  _showServerStopped();
