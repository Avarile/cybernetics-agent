...
function assistantDisplayName(){
  ...
  return window._botName||'Cybernetics';
}
const INFLIGHT={};  // keyed by session_id while request in-flight
const SESSION_QUEUES={};  // keyed by session_id for queued follow-up turns
const MAX_UPLOAD_BYTES=((window.__CYBERNETICS_CONFIG__&&window.__CYBERNETICS_CONFIG__.maxUploadBytes)||(window.__HERMES_CONFIG__&&window.__HERMES_CONFIG__.maxUploadBytes))||20*1024*1024;
...
function openCyberneticsDashboard(event){
...
const MODEL_STATE_KEY='cybernetics-webui-model-state';
const PENDING_SESSION_MODEL_PREFIX='cybernetics-webui-pending-session-model:';
...
function _readPersistedModelState(){
  ...
  const legacy=localStorage.getItem('cybernetics-webui-model');
...
function _writePersistedModelState(model, modelProvider){
  ...
  localStorage.setItem('cybernetics-webui-model', value);
  try{
 ...
 * currently configured in Cybernetics. Returns a warning string if mismatched,
...
const INFLIGHT_KEY = 'cybernetics-webui-inflight'; // localStorage key for in-flight session tracking
const INFLIGHT_STATE_KEY = 'cybernetics-webui-inflight-state'; // localStorage snapshots for mid-stream reload recovery
...
// ── Cybernetics agent/gateway heartbeat alert (#716) ──
...
function _showAgentHealthAlert(payload){
  ...
  if(!banner) return;
  if(title) title.textContent='Cybernetics agent is not responding';
...
function _syncTransparentEventControls(turn){
  ...
  // Wire the Cybernetics chat name tag toggle for the live turn.
...
// ── Transparent turn-level collapse (Cybernetics chat name tag) ───────────────
...
function renderMessages(options){
  ...
  if(!S.busy && (hasMessageToolMetadata||!S.toolCalls||!S.toolCalls.length)){
    ...
    S.messages.forEach((m,rawIdx)=>{
      if(!m) return;
      // OpenAI / Cybernetics CLI format: role=tool with tool_call_id
      if(m.role==='tool'){
  ...
  // Transparent mode per-turn wiring: collapsible Cybernetics chat name tag, old-event
