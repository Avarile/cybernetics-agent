  ...
  deleteBtn.onclick=async()=>{
    ...
    try{
      ...
      if(S.session&&ids.includes(S.session.session_id)){
        S.session=null;S.messages=[];S.entries=[];localStorage.removeItem('cybernetics-webui-session');
...
function _sessionTitleIsDefaultWebUI(rawTitle){
  ...
  return title==='Cybernetics WebUI'||/^Cybernetics WebUI #\d+$/.test(title);
}
...
async function _handleActiveSessionStorageEvent(e){
  if(!e || e.key !== 'cybernetics-webui-session') return;
...
async function deleteSession(sid, beforeDelete=null){
  ...
  if(S.session&&S.session.session_id===sid){
    ...
    if(typeof _hydrateTodosFromSession==='function') _hydrateTodosFromSession(null);
    localStorage.removeItem('cybernetics-webui-session');
