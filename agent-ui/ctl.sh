...
DEFAULT_LAUNCHD_LABEL="${HERMES_WEBUI_LAUNCHD_LABEL:-com.parantoux.cybernetics-webui}"
...
Commands:
  start [bootstrap args...]   Start Cybernetics WebUI as a background daemon
...
start_cmd() {
  ...
  if existing_pid="$(_current_pid 2>/dev/null)"; then
    echo "[ctl] Cybernetics WebUI is already running (PID ${existing_pid})"
    return 0
  fi
  local launchd_pid
  if launchd_pid="$(_launchd_webui_pid 2>/dev/null)"; then
    echo "[ctl] Refusing to start a second Cybernetics WebUI while launchd job ${HERMES_WEBUI_LAUNCHD_LABEL:-${DEFAULT_LAUNCHD_LABEL}} is running (PID ${launchd_pid})." >&2
  ...
  if ! _is_alive "${pid}"; then
    echo "[ctl] Cybernetics WebUI failed to stay running. Log: ${LOG_FILE}" >&2
    rm -f "${PID_FILE}" "${STATE_FILE}"
    return 1
  fi
  echo "[ctl] Started Cybernetics WebUI (PID ${pid})"
...
stop_cmd() {
  ...
  if ! pid="$(_pid_from_file 2>/dev/null)"; then
    echo "[ctl] Cybernetics WebUI is stopped"
    rm -f "${PID_FILE}" "${STATE_FILE}"
    return 0
  fi
  ...
  echo "[ctl] Stopping Cybernetics WebUI (PID ${pid})"
...
status_cmd() {
  ...
  if pid="$(_current_pid 2>/dev/null)"; then
    ...
    echo "● cybernetics-webui — running"
    echo "  PID:     ${pid}"
    ...
    echo "● cybernetics-webui — stopped"
    echo "  PID:     -"
    echo "  Bound:   ${host}:${port}"
    echo "  Log:     ${log_path}"
