...
These tests lock in:
  ...
  2. ``loadSession()`` clears the stale ``cybernetics-webui-session`` key on a 404
...
def test_load_session_clears_saved_stale_404_and_rethrows_to_boot():
    ...
    assert "localStorage.removeItem('cybernetics-webui-session')" in block, (
...
def test_load_session_404_self_heal_gated_to_active_or_boot():
    ...
    clear_idx = arm.find("localStorage.removeItem('cybernetics-webui-session')")
