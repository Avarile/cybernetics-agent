    ...
    try:
        result = run_ctl(
            ...
            env={
                ...
                "HERMES_WEBUI_LAUNCHD_LABEL": "com.parantoux.cybernetics-webui",
            },
        )
        assert result.returncode == 2
        combined = result.stdout + result.stderr
        assert "Refusing to start a second Cybernetics WebUI" in combined
...
def test_start_allows_alternate_port_while_launchd_job_runs_on_default(tmp_path):
    ...
    try:
        ...
        combined = result.stdout + result.stderr
        assert "Refusing to start a second Cybernetics WebUI" not in combined, combined
