"""Regression tests for #1240 — WebUI model catalog should delegate to Cybernetics CLI.

The WebUI picker should not freeze ordinary providers to its static
``_PROVIDER_MODELS`` snapshot when Cybernetics CLI can return a fresher provider
...
def test_generic_provider_uses_hermes_cli_catalog_before_static_snapshot(monkeypatch, tmp_path):
    ...
    this model was invisible even though Cybernetics CLI knew about it.
    """
...
def test_generic_provider_prefixes_live_ids_when_not_active_provider(monkeypatch, tmp_path):
    ...
    # Anthropic is authenticated via Cybernetics CLI, but OpenAI is the active
