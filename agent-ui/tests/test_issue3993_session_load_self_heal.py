...
def test_clear_stuck_session_helper_exists_and_is_wired():
    ...
    assert "localStorage.removeItem('cybernetics-webui-session')" in helper
...
const localStorage = {{ removeItem(k){{ if(k==='cybernetics-webui-session') removed=true; }} }};
...
def test_stale_load_guard_present_before_self_heal():
    ...
    assert block.index(guard) < block.index("localStorage.removeItem('cybernetics-webui-session')"), \
