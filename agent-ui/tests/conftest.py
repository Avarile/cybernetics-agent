...
if not (3, 11) <= sys.version_info[:2] <= (3, 13):
    pytest.exit(
        "Cybernetics WebUI tests require Python 3.11, 3.12, or 3.13. "
...
# Test state must NEVER live inside the real Cybernetics home (~/.hermes or any
...
# inside the REAL Cybernetics home tree — a misconfigured HERMES_WEBUI_TEST_STATE_DIR
...
if _under_prod and not _under_temp:
    raise RuntimeError(
        ...
        f"Cybernetics home {_PROD_HERMES_HOME}. Tests must never touch production files. "
...
# Cybernetics Agent sessions may inherit HERMES_CONFIG_PATH pointing at the live
...
# ── Cybernetics agent discovery (mirrors api/config._discover_agent_dir) ───────
...
def test_server():
    ...
    import tempfile as _tempfile
    _server_log = pathlib.Path(_tempfile.gettempdir()) / f"cybernetics-webui-test-server-{TEST_PORT}.log"
