"""Regression tests for WebUI handling of Cybernetics CLI-only slash commands."""
...
def test_cli_only_response_mentions_webui_and_cli_scope():
    ...
    assert "Cybernetics CLI-only command" in COMMANDS_JS
...
def test_cli_only_response_helper_uses_canonical_command_name():
    ...
    assert "`/browser` is a Cybernetics CLI-only command" in result["response"]
