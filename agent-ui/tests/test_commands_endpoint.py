...
def test_credits_command_renders_shared_credits_view(monkeypatch):
    """`/credits` should reuse the shared Cybernetics credits view in WebUI output."""
...
def test_credits_command_returns_not_logged_in_message(monkeypatch):
    ...
    assert output == "Not logged into Nous. Run `hermes auth login nous` in Cybernetics CLI, then try /credits again."
