...
def test_ephemeral_guard_does_not_remove_session_localstorage_key():
    ...
    localStorage.removeItem('cybernetics-webui-session') — that's exactly what
    ...
    assert "removeItem('cybernetics-webui-session')" not in block, (
        "The empty-session guard must NOT remove 'cybernetics-webui-session' from "
...
def test_ephemeral_guard_still_restores_panel_pref():
    ...
    block_end = BOOT_JS.find("return;", guard_idx)
    block = BOOT_JS[guard_idx:block_end]
    assert "cybernetics-webui-workspace-panel-pref" in block, (
        "Empty-session guard must still read 'cybernetics-webui-workspace-panel-pref' "
