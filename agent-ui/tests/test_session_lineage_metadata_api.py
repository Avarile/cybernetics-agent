...
def test_all_sessions_exposes_state_db_lineage_metadata_for_webui_json_sessions(_isolate):
    ...
    t0 = time.time() - 100
    try:
        _save_webui_session("lineage_api_root", title="Cybernetics WebUI", updated_at=t0)
        _save_webui_session("lineage_api_tip", title="Cybernetics WebUI #2", updated_at=t0 + 10)
        _insert_state_row(
            conn,
            "lineage_api_root",
            started_at=t0,
...
def test_cli_close_parent_preserves_cross_surface_continuation_lineage(_isolate):
    ...
    t0 = time.time() - 100
    try:
        _save_webui_session("lineage_api_cli_parent", title="Cybernetics WebUI #8", updated_at=t0)
        _save_webui_session("lineage_api_webui_child", title="Cybernetics WebUI #8", updated_at=t0 + 10)
        _insert_state_row(
            conn,
...
def test_generic_webui_title_gets_read_only_state_db_display_title(_isolate):
    ...
    t0 = time.time() - 100
    try:
        _save_webui_session("lineage_api_stale_title", title="Cybernetics WebUI #8", updated_at=t0)
        _insert_state_row(
            conn,
            "lineage_api_stale_title",
            title="Cybernetics WebUI #177",
            started_at=t0,
        )

        row = {row["session_id"]: row for row in all_sessions()}["lineage_api_stale_title"]

        assert row["title"] == "Cybernetics WebUI #8"
        assert row["display_title"] == "Cybernetics WebUI #177"
        assert row["_state_db_title"] == "Cybernetics WebUI #177"
    finally:
        conn.close()
