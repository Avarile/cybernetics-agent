...
def _init_repo(path):
    ...
    _git(path, "config", "user.name", "Cybernetics Tests")
    return path
...
def test_git_fetch_pull_and_push_with_upstream(tmp_path):
    ...
    _git(clone, "config", "user.name", "Cybernetics Tests")
...
def test_git_stash_and_checkout_is_explicit(tmp_path):
    ...
    assert result["stash_name"].startswith("cybernetics-webui branch switch")
    assert result["current_branch"] == "target"
    assert git_status(repo)["totals"]["changed"] == 0
    assert "cybernetics-webui branch switch to target" in _git(repo, "stash", "list")
...
def test_git_stash_and_checkout_restores_branch_changes_when_returning(tmp_path):
    ...
    assert "On main: cybernetics-webui branch switch" not in stash_list
    assert "On feature: cybernetics-webui branch switch" in stash_list
...
def test_git_stash_and_checkout_reports_restore_conflicts_without_dropping_stash(tmp_path):
    ...
    assert "On main: cybernetics-webui branch switch" in _git(repo, "stash", "list")
