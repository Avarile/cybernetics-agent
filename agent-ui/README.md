...
[Cybernetics Agent](https://hermes-agent.nousresearch.com/) is a sophisticated autonomous agent that lives on your server, accessed via a terminal or messaging apps, that remembers what it learns and gets more capable the longer it runs.

Cybernetics WebUI is a lightweight, dark-themed web app interface in your browser for [Cybernetics Agent](https://hermes-agent.nousresearch.com/).
...
**Cybernetics Control Center** (launcher at the sidebar bottom).

<img width="2448" height="1748" alt="Cybernetics WebUI — three-panel layout" src="https://github.com/user-attachments/assets/6bf8af4c-209d-441e-8b92-6515d7a0c369" />
...
This gives you nearly **1:1 parity with Cybernetics CLI from a convenient web UI** which you can access securely through an SSH tunnel from your Cybernetics setup. Single command to start this up, and a single command to SSH tunnel for access on your computer. Every single part of the web UI uses your existing Cybernetics agent and existing models, without requiring any additional setup.

---

## Contents

- [Why Cybernetics](#why-hermes) — what it is and how it compares
...
Cybernetics retains context across sessions, runs scheduled jobs while you're offline, and gets
smarter about your environment the longer it runs. It uses your existing Cybernetics agent setup,
...
- **Persistent memory** — user profile, agent notes, and a skills system that saves reusable
  procedures; Cybernetics learns your environment and does not have to relearn it
...
- **Self-improving skills** — Cybernetics writes and saves its own skills automatically from experience;
...
| | OpenClaw | Claude Code | Codex CLI | OpenCode | Cybernetics |
|---|---|---|---|---|---|
...
with memory, cron, and messaging. The key differences: Cybernetics writes and saves its own skills
automatically as a core behavior (OpenClaw's skill system centers on a community marketplace);
Cybernetics is more stable across updates (OpenClaw has documented release regressions and ClawHub
has had security incidents involving malicious skills); and Cybernetics runs natively in the Python
...
```bash
git clone https://github.com/nesquena/cybernetics-webui.git cybernetics-webui
cd cybernetics-webui
...
Two optional, self-hosted-deployment features — attaching dynamic **session-recall prefill** to browser turns (Joplin/Obsidian/Notion/llm-wiki routers), and routing browser chat through a running **Cybernetics Gateway** — are documented in [`docs/advanced-chat-setup.md`](docs/advanced-chat-setup.md). Most users need neither.

The bootstrap will:

1. Detect Cybernetics Agent and, if missing, attempt the official installer (`curl -fsSL https://raw.githubusercontent.com/NousResearch/hermes-agent/main/scripts/install.sh | bash`).
...
A community-maintained native Windows setup is documented at [@markwang2658/hermes-windows-native-guide](https://github.com/markwang2658/hermes-windows-native-guide) (companion setup repo: [@markwang2658/hermes-windows-native](https://github.com/markwang2658/hermes-windows-native)). Notes from the community report in [#1952](https://github.com/nesquena/cybernetics-webui/issues/1952):
...
- Multi-provider model support -- any Cybernetics API provider (OpenAI, Anthropic, Google, DeepSeek, Nous Portal, OpenRouter, MiniMax, Xiaomi MiMo, Z.AI); dynamic model dropdown populated from configured keys
...
- Enable via `CYBERNETICS_WEBUI_PASSWORD` env var or Settings panel
...
- **Cybernetics Control Center** (sidebar launcher button) -- Conversation tab (export/import/clear), Preferences tab (model, send key, theme, language, all toggles), System tab (version, password)
...
|---|---|
| Cybernetics agent dir | `HERMES_WEBUI_AGENT_DIR` env, then `$HERMES_HOME/hermes-agent` (Windows default `%LOCALAPPDATA%\hermes\hermes-agent`, POSIX default `~/.hermes/hermes-agent`), then sibling `../hermes-agent` |
...
| `HERMES_WEBUI_DEFAULT_MODEL` | *(provider default)* | Optional model override; leave unset to use the active Cybernetics provider default |
| `CYBERNETICS_WEBUI_PASSWORD` | *(unset)* | Set to enable password authentication |
...
| `HERMES_HOME` | Windows: `%LOCALAPPDATA%\hermes`; POSIX: `~/.hermes` | Base directory for Cybernetics state (affects all paths) |
| `HERMES_CONFIG_PATH` | `$HERMES_HOME/config.yaml` | Path to Cybernetics config file |
...
```bash
cd /path/to/hermes-agent          # or wherever sys.path can find Cybernetics modules
HERMES_WEBUI_PORT=8787 venv/bin/python /path/to/cybernetics-webui/server.py
```

Note: use the agent venv Python (or any Python environment that has the Cybernetics agent dependencies installed). System Python will be missing `openai`, `httpx`, and other required packages.
...
```bash
git clone https://github.com/nesquena/cybernetics-webui
cd cybernetics-webui
...
Run Compose as the user who owns your Cybernetics home. `sudo docker compose up -d` can make `${HOME}` expand to the root user's home, so Docker mounts the wrong `.hermes` directory instead of your real `~/.hermes` and the WebUI starts with `config.yaml (not found, using defaults)`. Prefer adding your user to the Docker group and running `docker compose up -d`; if you must use sudo, set absolute paths first, for example `HERMES_HOME=/home/you/.hermes HERMES_WORKSPACE=/home/you/workspace sudo -E docker compose up -d`, then verify with `docker compose config`.
...
```bash
docker pull ghcr.io/nesquena/cybernetics-webui:latest
docker run -d \
  ...
  ghcr.io/nesquena/cybernetics-webui:latest
```

### Build locally

```bash
docker build -t cybernetics-webui .
docker run -d \
  -e WANTED_UID=$(id -u) -e WANTED_GID=$(id -g) \
  -v ~/.hermes:/home/hermeswebui/.hermes \
  -e HERMES_WEBUI_STATE_DIR=/home/hermeswebui/.hermes/webui \
...
> and set `CYBERNETICS_WEBUI_PASSWORD` to enable authentication.

---

## Running tests

Tests discover the repo and the Cybernetics agent dynamically -- no hardcoded paths.
...
The WebUI is still coupled to Cybernetics Agent internals for runtime execution, provider/model access, and state/schema usage until the stable agent boundary work in [#1925](https://github.com/nesquena/cybernetics-webui/issues/1925) and [#2491](https://github.com/nesquena/cybernetics-webui/issues/2491) land. In practice, the WebUI imports Agent modules directly (`api/config.py`, `api/providers.py`, `api/streaming.py`) and reads Agent state layout directly, so version skew can cause import or behavior drift.

**Compatibility policy**
- WebUI release branches are tested against the matching Cybernetics Agent release available at that WebUI release time.
...
- Running pinned older/newer combinations is **untested and unsupported** until the stable API boundary work in [#1925](https://github.com/nesquena/cybernetics-webui/issues/1925) / [#2491](https://github.com/nesquena/cybernetics-webui/issues/2491) is in place.
- Record the full `hermes-agent` + `cybernetics-webui` versions in issue reports when upgrade mismatches are suspected.
...
**Start here**
- [`docs/why-hermes.md`](docs/why-hermes.md) — why Cybernetics, the mental model, and a detailed comparison to Claude Code / Codex / OpenCode / Cursor
...
Cybernetics WebUI is built with help from the open-source community. Every PR — whether merged directly, absorbed into a batch release, or salvaged from a larger proposal — shapes the project, and we're grateful to everyone who has taken the time to contribute.
...
```
git@github.com:nesquena/cybernetics-webui.git
```
