...
# WSL-friendly autostart launcher for Cybernetics WebUI.
#
# Safe defaults:
...
HERMES_WEBUI_PID_FILE="${HERMES_WEBUI_PID_FILE:-${HERMES_WEBUI_LOG_DIR}/cybernetics-webui.pid}"
HERMES_WEBUI_LOCK_FILE="${HERMES_WEBUI_LOCK_FILE:-/tmp/cybernetics-webui-autostart.lock}"
...
validate_repo() {
  if [[ ! -d "${HERMES_WEBUI_REPO}" ]]; then
    log "Cybernetics WebUI repo not found: ${HERMES_WEBUI_REPO}"
    exit 1
  fi
...
maybe_require_agent_process() {
  # Cybernetics WebUI usually launches the agent in-process, so this check is opt-in.
  # Set HERMES_WEBUI_REQUIRE_AGENT_PROCESS=1 only if your setup depends on a
  # separately running Cybernetics gateway/agent before WebUI starts.
  if [[ "${HERMES_WEBUI_REQUIRE_AGENT_PROCESS:-0}" != "1" ]]; then
    return 0
  fi
  if ! pgrep -f "hermes" >/dev/null 2>&1; then
    log "HERMES_WEBUI_REQUIRE_AGENT_PROCESS=1 but no Cybernetics process is running; skipping start"
    exit 1
  fi
}
...
start_webui() {
  ...
  if webui_healthy; then
    log "Cybernetics WebUI already running at ${HERMES_WEBUI_HEALTH_URL}"
    exit 0
  fi

  if pid_is_alive; then
    log "Cybernetics WebUI already running with pid $(cat "${HERMES_WEBUI_PID_FILE}")"
    exit 0
  fi

  rm -f "${HERMES_WEBUI_PID_FILE}"
  log "Starting Cybernetics WebUI from ${HERMES_WEBUI_REPO} on ${HERMES_WEBUI_HOST}:${HERMES_WEBUI_PORT}"
  ...
  if webui_healthy; then
    log "Cybernetics WebUI started and passed health check"
    exit 0
  fi

  if pid_is_alive; then
    log "Cybernetics WebUI process started with pid $(cat "${HERMES_WEBUI_PID_FILE}"); health check not ready yet"
    exit 0
  fi

  log "Cybernetics WebUI failed to stay running; see ${WEBUI_LOG}"
  exit 1
}
