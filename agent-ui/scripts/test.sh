...
venv_guidance() {
  ...
  if [[ -n "$base_py" ]]; then
    ...
    echo "Could not create a working .venv for Cybernetics WebUI tests." >&2
  fi
...
select_python() {
  ...
  if [[ -n "$requested" ]]; then
    ...
    if ! is_supported_python "$requested_path"; then
      echo "Unsupported Python for Cybernetics WebUI tests: $requested_path ($(python_version "$requested_path"))" >&2
    ...
    if [[ -z "$base_py" ]]; then
      echo "No supported Python found for Cybernetics WebUI tests." >&2
...
if missing="$(missing_dev_deps "$PYTHON_BIN" 2>/dev/null)"; then
  ...
  echo "Installing missing Cybernetics WebUI test dependencies in $PYTHON_BIN ($(python_version "$PYTHON_BIN"))." >&2
  if [[ -n "${missing:-}" ]]; then
