...
# In two-container setups (hermes-agent + cybernetics-webui), the shared hermes-home
...
echo ""; echo "== Checking required environment variables for cybernetics-webui"
...
echo ""; echo "==================="
echo ""; echo "== Installing uv and creating a new virtual environment for cybernetics-webui"
...
if [ -f /app/venv/.deps_installed ]; then
  ...
  echo ""; echo "== Installing cybernetics-webui dependencies"
  ...
  if [ -n "$_agent_src" ]; then
    ...
    echo "!!   https://github.com/nesquena/cybernetics-webui/blob/master/docker-compose.two-container.yml"
    echo ""
  fi
...
echo ""; echo "== Running cybernetics-webui"
cd /app; python server.py || error_exit "cybernetics-webui failed or exited with an error"
