...
metadata:
  hermes:
    ...
    related_skills: [claude-code, codex, opencode, cybernetics-agent]
---
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `cybernetics-agent` (Hermes subagents via `delegate_task`).
