...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HERMES_HOME", os.path.expanduser("~/.cybernetics")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HERMES_HOME", Path.home() / ".cybernetics")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Cybernetics config paths
# ═══════════════════════════════════════════════════════════════════

HERMES_HOME = Path(os.getenv("HERMES_HOME", Path.home() / ".cybernetics"))
...
def _get_current_model() -> tuple:
    """Read current model and provider from Cybernetics config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.cybernetics/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Cybernetics for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Cybernetics for changes to take effect.")
