...
cybernetics kanban watch --tenant <project-slug>

# Snapshot of the board
cybernetics kanban list --tenant <project-slug>
cybernetics kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
cybernetics kanban stats --tenant <project-slug>

# Visual dashboard (browser)
cybernetics dashboard

# Inspect a specific task (includes comments + events)
cybernetics kanban show <task-id>

# Follow a single task's event stream
cybernetics kanban tail <task-id>
```

Verify available subcommands with `cybernetics kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `cybernetics kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `cybernetics kanban block <id>` then `cybernetics kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `cybernetics kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `cybernetics kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `cybernetics profile list` (profile exists?), `cybernetics status` (gateway/dispatcher up?), and verify tenant. |
...
cybernetics kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
cybernetics kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(cybernetics kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
cybernetics kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
cybernetics kanban block <task-id>
cybernetics kanban archive <task-id>

# Diagnose what happened
cybernetics kanban show <task-id>      # task body, comments, recent events
cybernetics kanban tail <task-id>      # follow the live event stream
cybernetics kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `cybernetics kanban create` for the director
...
while true; do
    clear
    cybernetics kanban list --tenant <slug>
    echo "---"
    cybernetics kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `cybernetics kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `cybernetics profile list` and start
