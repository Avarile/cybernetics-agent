...
the actual `setup.sh` that creates the project workspace, configures Cybernetics
...
2. **Create profiles** — `cybernetics profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.cybernetics/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `cybernetics kanban create` assigned to the director
...
```bash
cybernetics profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.cybernetics/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.cybernetics/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Cybernetics
...
Each profile gets a `SOUL.md` at `~/.cybernetics/profiles/<name>/SOUL.md` that
...
```bash
cybernetics kanban create "Direct production of <video title>" \
    --assignee director \
...
the Cybernetics `.env` (`${HERMES_HOME:-$HOME/.hermes}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _cybernetics_env="${HERMES_HOME:-$HOME/.cybernetics}/.env"
    if grep -q "^${var}=" "$_cybernetics_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_cybernetics_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_cybernetics_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY cybernetics ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY cybernetics OPENROUTER_API_KEY || exit 1
# ...
```
