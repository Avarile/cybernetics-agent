...
Usage (via cron with --no-agent):
    ...
    cybernetics cron create cybernetics-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HERMES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name cybernetics-issues --repo NousResearch/hermes-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Cybernetics .env file
(``${HERMES_HOME:-~/.cybernetics}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Cybernetics-Watcher/1.0",
    }
