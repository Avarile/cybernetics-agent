...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${HERMES_HOME:-~/.cybernetics}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $HERMES_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name cybernetics-issues --repo NousResearch/hermes-agent --scope issues
```
