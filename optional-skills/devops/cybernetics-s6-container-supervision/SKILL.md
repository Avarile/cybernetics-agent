---
name: cybernetics-s6-container-supervision
description: Modify, debug, or extend the s6-overlay supervision tree inside the Cybernetics Agent Docker image — adding new services, debugging profile gateways, understanding the Architecture B main-program pattern.
version: 1.0.0
...
metadata:
  hermes:
    ...
    related_skills: [cybernetics-agent, cybernetics-agent-dev]
---
...
- Adding or removing a static service in the Cybernetics Agent Docker image (something that should be supervised at every container start, like the dashboard)
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `cybernetics gateway start/stop/restart` and routes to s6 when running in a container. |
...
- `cybernetics-tool-quirks`: Specific Cybernetics Agent tool workarounds (sed/grep/etc.) — load when debugging the s6 stack's interaction with cybernetics built-in tools.
