...
export function ArtLines({lines}: { lines: [string, string][] }) {
    return (
        <Box flexDirection="column" height={lines.length} marginTop={1} opaque width={artWidth(lines)}>
            {lines.map(([c, text], i) => (
...
const TAG_FULL = 'The personal agentic assistant'
const TAG_MID = 'The personal agentic assistant'
const TAG_TINY = 'Cybernetics'
const HIDE_BELOW = 34
    ...
    const skillsBody = () => {
        ...
        return (
            <>
                ...
                {overflow > 0 && (
                    <Text color={t.color.muted}>(and {overflow} more…)</Text>
                )}
            </>
        )
    }
        ...
        <>
            {(info.mcp_servers ?? []).map(s => (
                <Text key={s.name} wrap="truncate">
                    <Text color={t.color.muted}>  {s.name} [{s.transport}]: </Text>
                    {s.connected ? (
                        <Text color={t.color.text}>{s.tools} tool{s.tools === 1 ? '' : 's'}</Text>
        ...
        <Box borderColor={t.color.border} borderStyle="round" marginBottom={1} paddingX={2} paddingY={1}>
            <Box flexDirection="column" width={w}>
                ...
                <Box flexDirection="row" marginBottom={1}>
                    ...
                    {info.version && <Text color={t.color.muted}> v{info.version}</Text>}
                    <Text color={t.color.muted}> · </Text>
                    <Text color={t.color.accent}>{model}</Text>
                    <Text color={t.color.muted}> · </Text>
                    {sid && (
                        <Text wrap="truncate-end">
                            <Text color={t.color.sessionLabel}>Session ID: </Text>
                ...
                <Box flexDirection="column" marginTop={1}>
                    {/* ── Context ── */}
                    <Text color={t.color.muted} wrap="truncate-end">{info.cwd || process.cwd()}</Text>
                    {/* ── Summary ── */}
                    ...
                    {/*        {'! '}{info.update_behind} {info.update_behind === 1 ? 'commit' : 'commits'} behind — run{' '}*/}
                    {/*        <Text bold color={t.color.warn}>{info.update_command || 'Cybernetics update'}</Text>*/}
                    {/*    </Text>*/}
                    {/*)}*/}
                </Box>
...
// ── IntroLayout ──────────────────────────────────────────────────────
//
// Places the Banner (logo art) on the left and the SessionPanel on the
// right when the terminal is wide enough; falls back to a stacked column
// layout on narrow terminals so neither piece gets squeezed.

// Banner needs `LOGO_WIDTH + 2` of headroom to render the full art (see
// the `cols >= logoW + 2` check above); anything tighter drops it to the
// compact text fallback. The gap doubles as that slack column.
const INTRO_GAP = 2
const INTRO_LOGO_BUDGET = LOGO_WIDTH + INTRO_GAP
const INTRO_MIN_SESSION_WIDTH = 44

export function IntroLayout({cols, info, sid, t}: IntroLayoutProps) {
    const sideBySide = !!info && cols >= INTRO_LOGO_BUDGET + INTRO_MIN_SESSION_WIDTH

    if (sideBySide && info) {
        const sessionW = cols - INTRO_LOGO_BUDGET

        return (
            <Box flexDirection="row" paddingTop={1}>
                <Box marginRight={INTRO_GAP}>
                    <Banner maxWidth={INTRO_LOGO_BUDGET} t={t}/>
                </Box>
                <SessionPanel info={info} maxWidth={sessionW} sid={sid} t={t}/>
            </Box>
        )
    }
    ...
    return (
        <Box flexDirection="column" paddingTop={1}>
            <Banner maxWidth={cols} t={t}/>
            {info && <SessionPanel info={info} maxWidth={cols} sid={sid} t={t}/>}
        </Box>
    )
}
