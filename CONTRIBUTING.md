...
Thank you for contributing to Cybernetics Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Cybernetics to work everywhere.
...
If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `cybernetics skills browse` (labeled "official") and install it with `cybernetics skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `cybernetics skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.cybernetics/plugins/` (or via a pip entry point).
...
- Integrate with `cybernetics memory setup` via `post_setup()` — no need to touch core code
...
```bash
mkdir -p ~/.cybernetics/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.cybernetics/config.yaml
touch ~/.cybernetics/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.cybernetics/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/cybernetics" ~/.local/bin/cybernetics
...
### User configuration (stored in `~/.cybernetics/`)

| Path | Purpose |
|------|---------|
| `~/.cybernetics/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.cybernetics/.env` | API keys and secrets |
| `~/.cybernetics/auth.json` | OAuth credentials (Nous Portal) |
| `~/.cybernetics/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.cybernetics/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.cybernetics/state.db` | SQLite session database |
| `~/.cybernetics/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.cybernetics/cron/` | Scheduled job data |
| `~/.cybernetics/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.cybernetics/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
The user may skip setup and keep loading the skill. Cybernetics only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `cybernetics setup` or update `~/.cybernetics/.env` locally.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Cybernetics Agent" is the secondary collaborator. If the contributor's commit shows "Cybernetics Agent" as author because they used Cybernetics to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Cybernetics tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `cybernetics --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Cybernetics uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.cybernetics/skins/<name>.yaml`:
...
2. **Test manually**: Run `cybernetics` and exercise the code path you changed
...
- Include: OS, Python version, Cybernetics version (`cybernetics version`), full error traceback
