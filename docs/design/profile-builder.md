...
Supersedes: PR #31781 (prompt_toolkit `cybernetics profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `cybernetics profile wizard` in the terminal.
...
A profile = a full `~/.cybernetics/profiles/<name>/` directory with its own:
...
5. spawn `cybernetics -p <profile> skills install <id>` per hub skill (async, returns PIDs)
