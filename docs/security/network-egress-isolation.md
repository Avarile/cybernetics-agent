...
When running Cybernetics inside Docker, the default `network_mode: host` gives the
...
The Cybernetics [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
│                                             │
│   ┌──────────────┐   ┌──────────────────┐   │
│   │ cybernetics-agent │   │ cybernetics-dashboard │   │
│   └──────┬───────┘   └────────┬─────────┘   │
│          │                    │              │
│          ▼                    │              │
│   ┌──────────────┐            │              │
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=cybernetics,cybernetics-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://cybernetics-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
...
- [SECURITY.md](../../SECURITY.md) — Cybernetics trust model and vulnerability reporting
