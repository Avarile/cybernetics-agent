# Cybernetics Middleware

Cybernetics middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Cybernetics'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Cybernetics calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Cybernetics retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Cybernetics guardrails,
...
Cybernetics stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Cybernetics runs
them as a nested chain in registration order. Middleware failures are fail-open:
Cybernetics logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Cybernetics applies middleware in this order:
...
For each tool call, Cybernetics applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Cybernetics pre-execution path against the effective arguments:
...
```bash
cybernetics plugins enable <plugin-name>
```
...
```bash
export HERMES_HOME=/tmp/cybernetics-middleware-test
mkdir -p "$HERMES_HOME"
cybernetics plugins enable <plugin-name>
cybernetics chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run cybernetics plugins enable <plugin-name>
uv run cybernetics chat --query 'Reply exactly ok'
```
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/cybernetics-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Cybernetics expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Cybernetics treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Cybernetics preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Cybernetics does not convert downstream
