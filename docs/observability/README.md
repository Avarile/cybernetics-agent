# Cybernetics Observer Hooks

Cybernetics observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Cybernetics' planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. Cybernetics catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Cybernetics only builds
