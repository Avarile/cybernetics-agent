...
Outputs a compact, plain-text summary of the user's Cybernetics setup
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.cybernetics_build_sha`` by the Dockerfile's
...
def _redact(value: str) -> str:
    ...
    ``cybernetics dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # A credential added via `cybernetics auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `cybernetics auth list` shows it (#42130).
