"""Slash command definitions and autocomplete for the Cybernetics CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Cybernetics config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Cybernetics works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Cybernetics is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.cybernetics/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Cybernetics Agent to the latest version", "Info"),
    CommandDef("version", "Show Cybernetics Agent version", "Info", aliases=("v",)),
...
Telegram only displays a small BotCommand menu in practice.  The full Cybernetics
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``cybernetics skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``cybernetics skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``cybernetics skills list`` and
...
# listed here still degrades gracefully (reachable via /cybernetics <command>).
...
# native slot, the alias spelling stays reachable via /cybernetics reset).
...
# ``/cybernetics <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/hermes" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /cybernetics credits on Slack.
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/cybernetics <verb>`` subcommand.
    ...
    can still reach them via ``/cybernetics <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/hermes`` is always reserved as the first entry so the
    legacy ``/cybernetics <subcommand>`` form keeps working for anything that
    ...
    # Reserve /cybernetics as the catch-all top-level command.
    entries.append(("cybernetics", "Talk to Cybernetics or run a subcommand", "[subcommand] [args]"))
    seen.add("cybernetics")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_VIA_HERMES_ONLY:
            # Intentionally Slack-via-/cybernetics only (see _SLACK_VIA_HERMES_ONLY).
            return
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /cybernetics handler.

    Maps both canonical names and aliases so /cybernetics bg do stuff works
    the same as /cybernetics background do stuff.

    Plugin-registered slash commands are included so ``/cybernetics <plugin-cmd>``
