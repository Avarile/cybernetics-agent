"""
Cybernetics Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``cybernetics desktop`` / ``cybernetics gui``)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Cybernetics.app`` or ``~/Applications/Cybernetics.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Cybernetics`` (NSIS per-user)
...
the app name ("Cybernetics"), separate from ``$HERMES_HOME``:
  - macOS:   ``~/Library/Application Support/Cybernetics``
  - Windows: ``%APPDATA%\\Cybernetics``
  - Linux:   ``$XDG_CONFIG_HOME/Cybernetics`` (default ``~/.config/Cybernetics``)
...
``cybernetics uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def desktop_userdata_dir() -> Path:
    ...
    Mirrors Electron's ``app.getPath('userData')`` for an app named "Cybernetics"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Cybernetics"
...
def source_built_gui_artifacts(hermes_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``cybernetics desktop`` inside the checkout.
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Cybernetics" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Cybernetics.app"),
            home / "Applications" / "Cybernetics.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Cybernetics).
            local_base / "Programs" / "Cybernetics",
        ...
        if program_files:
            ...
            paths.append(Path(program_files) / "Cybernetics")
    else:
        ...
        paths += [
            ...
            data_base / "applications" / "Cybernetics.desktop",
        ]
    return paths
