...
``<root>`` is the **shared Cybernetics root** (the parent of any active
...
``cybernetics -p <profile>`` joins the same board as the dispatcher that
...
the profile's Cybernetics home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``cybernetics kanban boards
...
def kanban_home() -> Path:
    """Return the shared Cybernetics root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``cybernetics kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``cybernetics kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``cybernetics kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``cybernetics kanban log`` unambiguous even when multiple
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Cybernetics treats as user-controlled-but-trusted on the
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``cybernetics kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``cybernetics --skills ...`` alongside the built-in
    ...
    # `cybernetics --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``cybernetics kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `cybernetics kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``cybernetics kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``cybernetics kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``cybernetics kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``cybernetics kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Cybernetics' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Cybernetics-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``cybernetics kanban show <id>`` and the dashboard.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Cybernetics
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``cybernetics kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Cybernetics profile.
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Cybernetics profile.
        # `_default_spawn` invokes ``cybernetics -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Cybernetics
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `cybernetics kanban tail` — without
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Cybernetics CLI invocation."""
...
def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Cybernetics shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Cybernetics executable path.
...
def _resolve_hermes_argv() -> list[str]:
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Cybernetics is launched from a venv and the ``hermes`` shim is not on
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    the worker activates a profile (``cybernetics -p <name>``), its ``SKILLS_DIR``
    ...
    # home (``~/.cybernetics``), which ships the bundled skill.
    base = _Path(hermes_home) if hermes_home else (_Path.home() / ".cybernetics")
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``cybernetics -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `cybernetics -p <name>` the
    ...
    # that even when the worker activates a profile (`cybernetics -p <name>`
    ...
    # `cybernetics -p <assignee>` activates the profile, but the env var is
    ...
    # `cybernetics kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Install Cybernetics Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``cybernetics kanban daemon`` is systemd-friendly.
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Cybernetics root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``cybernetics kanban assignees`` for the terminal.
