"""
Baked-in build metadata for Cybernetics Agent.
...
To make ``cybernetics dump`` and the startup banner identify the exact commit the
image was built from, the Docker build writes the build-time ``$HERMES_GIT_SHA``
arg into ``<project_root>/.cybernetics_build_sha``.  This module is the single
...
_BUILD_SHA_FILE = Path(__file__).parent.parent / ".cybernetics_build_sha"


def get_build_sha(short: int = 8) -> Optional[str]:
    """Return the baked-in build SHA, truncated to ``short`` chars, or None.

    Reads ``<project_root>/.cybernetics_build_sha`` if present.  The file is
