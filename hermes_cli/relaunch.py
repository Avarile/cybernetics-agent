"""
Unified self-relaunch for Cybernetics CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``cybernetics sessions browse`` or post-setup relaunch
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the cybernetics entry point.
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh cybernetics invocation.
    ...
    relaunch contract wants: "run cybernetics again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    That's functionally equivalent — the user sees "cybernetics exited, then
    new cybernetics started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                f"\nCybernetics relaunch failed: {exc}\n"
