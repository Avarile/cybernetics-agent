"""cybernetics hooks — inspect and manage shell-script hooks.

Usage::

    cybernetics hooks list
    cybernetics hooks test <event> [--for-tool X] [--payload-file F]
...
def hooks_command(args) -> None:
    """Entry point for ``cybernetics hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: cybernetics hooks {list|test|revoke|doctor}")
        print("Run 'cybernetics hooks --help' for details.")
        return
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `cybernetics hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `cybernetics hooks test` and `cybernetics hooks doctor`
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `cybernetics hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `cybernetics hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `cybernetics hooks doctor`.")
