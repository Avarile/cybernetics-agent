...
entries via ``cybernetics mcp catalog`` or the interactive ``cybernetics mcp picker``,
and install them with ``cybernetics mcp install <name>`` (or by toggling in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``cybernetics mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.cybernetics/.env`` (the
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Cybernetics install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Cybernetics understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Cybernetics is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Cybernetics
        understands. Update Cybernetics to install this entry.
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.cybernetics/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.cybernetics/.env via save_env_value()."""
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``cybernetics mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `cybernetics mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `cybernetics mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `cybernetics mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `cybernetics mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `cybernetics auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`cybernetics auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Cybernetics session to load its tools.",
        Colors.GREEN,
    ))
