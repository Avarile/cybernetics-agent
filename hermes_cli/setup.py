"""
Interactive setup wizard for Cybernetics Agent.
...
# display_hermes_home imported lazily at call sites (stale-module safety during cybernetics update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Cybernetics Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Cybernetics using environment variables or config commands:")
    print_info("  cybernetics config set model.provider custom")
    print_info("  cybernetics config set model.base_url http://localhost:8080/v1")
    print_info("  cybernetics config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'cybernetics setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'cybernetics setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'cybernetics setup' to configure"))
    ...
    # Video generation — opt-in via `cybernetics tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'cybernetics setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'cybernetics setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'cybernetics setup terminal'"))
    ...
    # Spotify (OAuth via cybernetics auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'cybernetics setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('cybernetics setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('cybernetics setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('cybernetics setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('cybernetics setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('cybernetics setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('cybernetics config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('cybernetics config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('cybernetics config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('cybernetics', Colors.GREEN)}         Start chatting")
    print(f"   {color('cybernetics gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('cybernetics doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `cybernetics tools` and `cybernetics setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``cybernetics model``)
    ...
    provider added to ``cybernetics model`` is automatically available here.
    ...
    # Delegate to the shared cybernetics model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: cybernetics model")
    ...
    # on demand via `cybernetics auth add`, `cybernetics setup` vision, and
    # `cybernetics setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``cybernetics model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.cybernetics/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Cybernetics auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via cybernetics setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'cybernetics setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Cybernetics runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `cybernetics setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `cybernetics setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `cybernetics setup agent` later to customize.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Cybernetics delivers cron job results,")
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `cybernetics update`)",
                True,
            ):
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Cybernetics",
            bot_description="Your Cybernetics agent on Slack",
        )
        ...
        print_info(
            "   Re-run `cybernetics slack manifest --write` anytime to refresh after "
            "Cybernetics adds new commands."
        )
        ...
        print_info(
            ...
            "cybernetics slack manifest --write"
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Cybernetics delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Cybernetics to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  cybernetics config edit")
    print_info("   Open config in your editor:  cybernetics config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Cybernetics from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'cybernetics setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     cybernetics config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd:
                    print_info("  Or as a boot-time service: sudo cybernetics gateway install --system")
                print_info("  Or run in foreground:  cybernetics gateway")
        else:
            from hermes_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   cybernetics gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   cybernetics gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `cybernetics setup tools` and `cybernetics tools` use the same flow:
...
# Config values may have different semantics between OpenClaw and Cybernetics.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Cybernetics to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Cybernetics at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Cybernetics at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Cybernetics at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Cybernetics at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Cybernetics equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Cybernetics config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Cybernetics.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Cybernetics's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Cybernetics can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: cybernetics claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: cybernetics claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Cybernetics configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Cybernetics config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Cybernetics (use cybernetics claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``cybernetics setup --portal`` and ``cybernetics portal``. This is the
    ...
    Cybernetics session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``cybernetics setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``cybernetics model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``cybernetics portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Cybernetics Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `cybernetics portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `cybernetics portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `cybernetics portal`: %s", exc)
        print()
    ...
    print_info("  Run `cybernetics portal info` to inspect routing.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      cybernetics setup           — full or quick (auto-detected)
      cybernetics setup model     — just model/provider
      cybernetics setup tts       — just text-to-speech
      cybernetics setup terminal  — just terminal backend
      cybernetics setup gateway   — just messaging platforms
      cybernetics setup tools     — just tool configuration
      cybernetics setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Cybernetics Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Cybernetics Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Cybernetics Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Cybernetics configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'cybernetics setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Cybernetics?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'cybernetics config edit'")
    ...
    # Tune later with `cybernetics setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``cybernetics setup <section>``
    or switch providers with ``cybernetics model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'cybernetics setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    cybernetics setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  cybernetics setup gateway")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'cybernetics setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Cybernetics to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'cybernetics setup gateway'.")
