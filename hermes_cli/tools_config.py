"""
Unified tool configuration for Cybernetics Agent.

`cybernetics tools` and `cybernetics setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.cybernetics/config.yaml under
...
# who want it opt in via `cybernetics tools` → Video Generation, which walks
...
# `cybernetics tools` → X (Twitter) Search setup walks users through credential
...
# Platform-scoped toolsets: only appear in the `cybernetics tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``cybernetics tools`` → "reconfigure existing" would
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``cybernetics tools`` checklist actually offers
    for ``platform``.
    ...
    so ``cybernetics tools`` never claims to add or remove a toolset the user was
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Cybernetics routes X searches through xAI's built-in x_search "
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``cybernetics update`` and
      by ``cybernetics computer-use install --upgrade``.
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `cybernetics update` calls this for every
    ...
    try:
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                _print_info("      System Settings > Privacy & Security > Accessibility")
                _print_info("      System Settings > Privacy & Security > Screen Recording")
                _print_info("    Both must allow the terminal / Cybernetics process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.cybernetics/config.yaml")
        ...
        # Run the full `cybernetics auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.cybernetics/.env), then continues straight into PKCE. If they
        ...
        try:
            ...
            _print_info("    Run manually: cybernetics auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `cybernetics auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: cybernetics auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run manually: cybernetics plugins enable observability/langfuse")
        _print_info("    Restart Cybernetics for tracing to take effect.")
        _print_info("    Verify: cybernetics plugins list")
        ...
        try:
            ...
            _print_info("    Run later: cybernetics auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `cybernetics auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: cybernetics auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: cybernetics auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``cybernetics tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``cybernetics tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: cybernetics tools post-setup <key>")
        return 2
...
def _get_platform_tools(
    ...
    *,
    ...
    if has_explicit_config:
        ...
        # through ``cybernetics tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `cybernetics tools` (which flips has_explicit_config
    ...
    # shipped as a bundled plugin but user must opt in via `cybernetics tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `cybernetics tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by cybernetics tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Opening `cybernetics tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `cybernetics tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `cybernetics model`).
    if managed_feature:
        from hermes_cli.nous_subscription import (
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `cybernetics tools` and `cybernetics setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Cybernetics Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print(color("  Changes take effect on next 'cybernetics' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, cybernetics mcp
