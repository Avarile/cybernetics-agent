"""cybernetics webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.cybernetics/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: cybernetics gateway run
"""
...
def webhook_command(args):
    """Entry point for 'cybernetics webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: cybernetics webhook {subscribe|list|remove|test}")
        print("Run 'cybernetics webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (cybernetics gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: cybernetics webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from cybernetics webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (cybernetics gateway run)")
