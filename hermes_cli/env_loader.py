"""Helpers for loading Cybernetics .env files consistently across entrypoints."""
...
# Used by setup / `cybernetics model` flows to label detected credentials so
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `cybernetics setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    already knows all valid Cybernetics env-var names and can split
...
def load_hermes_dotenv(
    *,
    ...
    """Load Cybernetics environment files with user config taking precedence.

    Behavior:
    - `~/.cybernetics/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(hermes_home or os.getenv("HERMES_HOME", Path.home() / ".cybernetics"))
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    locate the access token) but BEFORE the rest of Cybernetics reads
    ...
    (tests, future ``cybernetics secrets bitwarden sync`` from a long-running
    process).
    """
    ...
    if result.applied:
        ...
        # edited .env (see #6843).
        _sanitize_loaded_credentials()
        # Remember where these came from so the setup / `cybernetics model`
