...
Usage examples::
    ...
    cybernetics logs                    # last 50 lines of agent.log
    cybernetics logs -f                 # follow agent.log in real time
    cybernetics logs errors             # last 50 lines of errors.log
    cybernetics logs gateway -n 100    # last 100 lines of gateway.log
    cybernetics logs gui -f            # follow gui.log (dashboard/pty/ws)
    cybernetics logs desktop -f        # follow desktop.log (Electron app boot/backend)
    cybernetics logs --level WARNING    # only WARNING+ lines
    cybernetics logs --session abc123   # filter by session ID substring
    cybernetics logs --component tools  # only tool-related lines
    cybernetics logs --since 1h         # lines from the last hour
    cybernetics logs --since 30m -f     # follow, starting 30 min ago
"""
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'cybernetics chat' to generate logs)")
