...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `cybernetics --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``cybernetics auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`cybernetics auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Cybernetics reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `cybernetics auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `cybernetics auth` is called bare."""
