...
Pure logic: walks a Cybernetics config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `cybernetics doctor` and a future `cybernetics migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Cybernetics config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Cybernetics config and return retirement issues.
