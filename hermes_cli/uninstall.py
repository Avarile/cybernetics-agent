"""
Cybernetics Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.cybernetics/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Cybernetics PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the "# Cybernetics Agent" comment and following line
                if '# Cybernetics Agent' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
def remove_wrapper_script():
    """Remove the cybernetics wrapper script if it exists."""
    wrapper_paths = [
...
def remove_node_symlinks(hermes_home: Path) -> list:
    ...
    the FHS fix).  Only symlinks that resolve into this Cybernetics home's ``node``
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``cybernetics gateway run`` processes
...
#   4. On the ``cybernetics dashboard`` + gateway paths, drops files into
...
def _hermes_path_markers(hermes_home: Path) -> list[str]:
    """Path-entry substrings that identify Cybernetics-owned User-PATH entries."""
    ...
    # backslash-cybernetics as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(hermes_home: Path) -> list[str]:
    """Strip Cybernetics-owned entries from User-scope PATH in the registry.
...
def _uninstall_profile(profile) -> None:
    ...
    We shell out to ``cybernetics -p <name> gateway stop|uninstall`` because
...
def run_gui_uninstall(args):
    ...
    Mirrors ``cybernetics uninstall --gui``. Removes the desktop app's built
    ...
    print(color("│         ⚕ Cybernetics Chat GUI Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No Cybernetics Chat GUI installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Cybernetics agent stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The Cybernetics agent at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Cybernetics agent is still installed. Run 'cybernetics' to use the CLI,")
    print("or 'cybernetics uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.cybernetics/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.cybernetics/ for future reinstall
    """
    ...
    # full wipe (code + ~/.cybernetics data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ Cybernetics Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Cybernetics data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Cybernetics code but keep your configuration and data.")
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No Cybernetics-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No Cybernetics-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing cybernetics command...")
    ...
    #     (only when they still point into this Cybernetics home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Cybernetics-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No Cybernetics-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.cybernetics/, just remove the hermes-agent subdir
    ...
    # 5. Optionally remove ~/.cybernetics/ data directory (and named profiles)
    if full_uninstall:
    ...
    print()
    print("Thank you for using Cybernetics Agent! ⚕")
    print()
