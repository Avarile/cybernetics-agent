"""
Single source of truth for provider identity in Cybernetics Agent.
...
2. **Cybernetics overlays** — transport type, auth patterns, aggregator flags,
...
# -- Cybernetics overlay ----------------------------------------------------------
# Cybernetics-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Cybernetics-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Cybernetics overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Cybernetics overlay
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + cybernetics extra
    ...
    if overlay is not None:
        # Cybernetics-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
