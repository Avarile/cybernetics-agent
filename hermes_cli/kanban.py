...
def _check_dispatcher_presence() -> tuple[bool, str]:
    ...
    Used by ``cybernetics kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`cybernetics kanban daemon --force`)."
        )
    return (
        False,
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `cybernetics kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `cybernetics kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `cybernetics gateway start`.",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``cybernetics kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: cybernetics kanban <action> [options]\n"
                "Run 'cybernetics kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `cybernetics kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
...
# ---------------------------------------------------------------------------
# Boards management (cybernetics kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``cybernetics kanban boards <action>``.
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `cybernetics kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `cybernetics kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `cybernetics kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `cybernetics kanban boards delete <slug>` (alias), the
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    # the kanban dispatcher can use them without a separate `cybernetics profile
    ...
    print("  cybernetics gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `cybernetics -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`cybernetics kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``cybernetics kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "cybernetics kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    cybernetics gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`cybernetics kanban list --status ready` / "
                    f"`cybernetics kanban list --status blocked` for recent "
