"""``cybernetics dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.cybernetics/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.cybernetics/auth.json``), refreshing it if needed. Fails fast with a
     "run `cybernetics setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.cybernetics/.env`` idempotently.
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``cybernetics dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `cybernetics auth login nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    print(
        ...
        "  `cybernetics dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    cybernetics dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `cybernetics dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `cybernetics setup` (or `cybernetics auth login nous`) first, then retry.")
        else:
