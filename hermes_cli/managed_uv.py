...
Cybernetics owns its own uv binary at ``$HERMES_HOME/bin/uv`` (or ``uv.exe`` on
...
def managed_uv_path() -> Path:
    """Return the path where Cybernetics keeps *its* uv binary.
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``cybernetics update`` runs
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``cybernetics update`` so the managed copy stays current.
