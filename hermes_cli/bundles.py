"""Implementation of the ``cybernetics bundles`` CLI subcommand.
...
def _console() -> Console:
    # Bind to stderr so piping `cybernetics bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  cybernetics bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``cybernetics bundles`` argparse tree.
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `cybernetics bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``cybernetics bundles <subcommand>`` to the right handler."""
