"""
cybernetics fallback — manage the fallback provider chain.
...
Subcommands:
  cybernetics fallback [list]   Show the current fallback chain (default when no subcommand)
  cybernetics fallback add      Pick provider + model via the same picker as `cybernetics model`,
                           then append the selection to the chain
  cybernetics fallback remove   Pick an entry to delete from the chain
  cybernetics fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.cybernetics/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `cybernetics model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  cybernetics fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `cybernetics model`, then append the selection to the chain."""
    ...
    print("  `cybernetics model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `cybernetics fallback list` to view, or `cybernetics fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``cybernetics fallback [subcommand]``."""
