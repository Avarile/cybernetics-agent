"""Persistent session goals — the Ralph loop for Cybernetics.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Cybernetics feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.CyberneticsCLI`` or the gateway
...
def run_kanban_goal_loop(
    *,
    ...
    (``cybernetics -p <profile> chat -q "work kanban task <id>"``). The worker's
