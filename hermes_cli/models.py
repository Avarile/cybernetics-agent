...
Add, remove, or reorder entries here — both `cybernetics setup` and
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `cybernetics model`
...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Cybernetics refreshes the cache from
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Cybernetics that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Cybernetics that doesn't ship
...
# this list:  cybernetics model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `cybernetics model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `cybernetics model` TUI
...
#
# Some vendors expose several Cybernetics provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `cybernetics model` / setup wizard /
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``cybernetics model``, the
...
# path. The *interactive* default (GUI onboarding / ``cybernetics model``) uses the
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``cybernetics auth add openai-codex`` without
    ``cybernetics model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``cybernetics model`` picker offers first. For metered aggregators
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``cybernetics model``, ``/model``, etc.).
    """
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Cybernetics' canonical provider ids.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Cybernetics should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``cybernetics auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.cybernetics/.env``.
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a Cybernetics release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Cybernetics release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        # added Portal models still surface without a Cybernetics release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``cybernetics model`` shows.
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Cybernetics' default Claude IDs elsewhere use
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Cybernetics will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Cybernetics cannot verify the model name."
