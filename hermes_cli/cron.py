...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'cybernetics cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: cybernetics gateway install", Colors.DIM))
        print(color("                    sudo cybernetics gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    cybernetics gateway install    # Install as a user service")
        print("    sudo cybernetics gateway install --system  # Linux servers: boot-time system service")
        print("    cybernetics gateway            # Or run in foreground")
...
def cron_create(args):
    ...
    if _contains_gateway_lifecycle_command(combined):
        print(color(
            ...
            "Use `cybernetics gateway restart` from a shell outside the gateway.",
            Colors.RED,
        ))
        return 1
...
def cron_command(args):
    ...
    print("Usage: cybernetics cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
