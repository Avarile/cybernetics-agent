...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or cybernetics -c.",
    ...
    "Ctrl+Z suspends Cybernetics to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "cybernetics -c resumes your most recent CLI session. cybernetics -c \"project name\" resumes by title.",
    "cybernetics -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "cybernetics -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "cybernetics chat -t web,terminal enables only specific toolsets for a focused session.",
    "cybernetics chat -s github-pr-workflow preloads a skill at launch.",
    "cybernetics chat -q \"query\" runs a single non-interactive query and exits.",
    "cybernetics chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "cybernetics chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "cybernetics --yolo bypasses all dangerous command approval prompts for the entire session.",
    "cybernetics chat --source telegram tags the session for filtering in cybernetics sessions list.",
    "cybernetics -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "cybernetics doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "cybernetics dump outputs a compact setup summary — great for bug reports.",
    "cybernetics config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "cybernetics config edit opens config.yaml in your default editor.",
    "cybernetics config check scans for missing or stale configuration options.",
    "cybernetics sessions browse opens an interactive session picker with search.",
    "cybernetics sessions stats shows session counts by platform and database size.",
    "cybernetics sessions prune --older-than 30 cleans up old sessions.",
    "cybernetics skills search react --source skills-sh searches the skills.sh public directory.",
    "cybernetics skills check scans installed hub skills for upstream updates.",
    "cybernetics skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "cybernetics skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "cybernetics mcp add github --command npx adds MCP servers from the command line.",
    "cybernetics mcp serve runs Cybernetics itself as an MCP server for other agents.",
    "cybernetics auth add lets you add multiple API keys for credential pool rotation.",
    "cybernetics completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "cybernetics logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "cybernetics backup creates a zip backup of your entire Cybernetics home directory.",
    "cybernetics profile create coder creates an isolated profile that becomes its own command.",
    "cybernetics profile create work --clone copies your current config and keys to a new profile.",
    "cybernetics update syncs new bundled skills to ALL profiles automatically.",
    "cybernetics gateway install sets up Cybernetics as a system service (systemd/launchd).",
    "cybernetics memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "cybernetics webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: cybernetics tools disables unused tools, cybernetics skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "cybernetics models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as cybernetics -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Cybernetics tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'cybernetics profile create coder' creates the 'coder' command.",
    "cybernetics profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Cybernetics prints a resume command with session ID and stats.",
    "cybernetics sessions export backup.jsonl exports all sessions for backup or analysis.",
    "cybernetics -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "cybernetics skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: cybernetics cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Cybernetics runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "cybernetics gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Cybernetics via a local macOS server.",
    ...
    "Cybernetics loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "cybernetics mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "cybernetics mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "cybernetics mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — cybernetics tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.cybernetics/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "cybernetics chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "cybernetics plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "cybernetics acp runs Cybernetics as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "cybernetics auth add nous or cybernetics auth add openai-codex sets up OAuth-based providers.",
    ...
    "cybernetics status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.cybernetics/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Cybernetics your own.",
    ...
    "Before a gateway session reset, Cybernetics auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.cybernetics/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.cybernetics/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'cybernetics profile create ls' would be rejected.",
    "cybernetics profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "cybernetics claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via cybernetics tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.cybernetics/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.cybernetics/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Cybernetics auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Cybernetics config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Cybernetics is working.',
    ...
    '/reload reloads ~/.cybernetics/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.cybernetics/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.cybernetics/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'cybernetics curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "cybernetics curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'cybernetics curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'cybernetics auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'cybernetics dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'cybernetics dashboard embeds the full Cybernetics TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.cybernetics/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.cybernetics/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.cybernetics/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.cybernetics/skills/ so drop-in skills appear without restarting the session.',
    ...
    'cybernetics -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'cybernetics chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'cybernetics chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'cybernetics chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "cybernetics chat --source tool tags programmatic chats so they don't clutter cybernetics sessions list.",
    'cybernetics dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'cybernetics sessions rename <ID> "new title" renames any past session; cybernetics sessions delete <ID> removes one.',
    'cybernetics import restores a session export or profile archive produced by sessions export or profile export.',
    'cybernetics fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'cybernetics pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'cybernetics setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'cybernetics status --deep runs the full health sweep across every component; plain cybernetics status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Cybernetics block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from cybernetics sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.cybernetics/dashboard-plugins/.',
]
