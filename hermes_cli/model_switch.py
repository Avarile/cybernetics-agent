...
_HERMES_MODEL_WARNING = (
    "Nous Research Cybernetics 3 & 4 models are NOT agentic and are not designed "
    "for use with Cybernetics Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research Cybernetics 3 / Cybernetics 4 chat families.
...
def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous Cybernetics 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous Cybernetics 3/4 chat model."""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``cybernetics config set model.aliases.xxx``)
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'cybernetics model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'cybernetics doctor' — config issues detected:"
...
def list_authenticated_providers(
    ...
    *,
    ...
    # Build curated model lists keyed by cybernetics provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://hermes-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Cybernetics release. Falls back to the in-repo
    ...
    # --- 1. Check Cybernetics-mapped providers ---
    from hermes_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hermes_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # /model picker sees the SAME list `cybernetics model` would build, with
    ...
    # --- 2. Check Cybernetics-only providers (nous, openai-codex, copilot, opencode-go) ---
    from hermes_cli.providers import HERMES_OVERLAYS
    from hermes_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Cybernetics provider ID.
    # HERMES_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Cybernetics IDs ("copilot").
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # Resolve Cybernetics slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # `cybernetics model` picker deliberately shows ONLY the curated agentic
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Cybernetics's canonical write key (matches
            ...
            # Cybernetics writes ``models:`` as a dict keyed by model id
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Cybernetics's own writer uses; a hyphen variant is accepted
            ...
            # active model. Cybernetics's own writer (main.py::_save_custom_provider)
