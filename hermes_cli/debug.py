...
_REDACTION_BANNER = (
    "[cybernetics debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    ...
    ran ``cybernetics debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``cybernetics debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``cybernetics debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``cybernetics debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `cybernetics debug share` "
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``cybernetics debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``cybernetics dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``cybernetics dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``cybernetics debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "cybernetics debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `cybernetics debug share --local` to print the report instead.\n")
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: cybernetics debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'cybernetics debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``cybernetics debug`` call.
    ...
    # best-effort — any failure is swallowed so ``cybernetics debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: cybernetics debug <command>")
        print()
        print("Commands:")
