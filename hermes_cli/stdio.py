...
on Japanese locales, etc.).  Cybernetics's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Cybernetics is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Cybernetics-managed Git install directories so
    ...
    # install without Cybernetics-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Cybernetics's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Cybernetics's
    ``cybernetics config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Cybernetics (User env var in Windows Settings, or
    export in a PowerShell profile) and Cybernetics picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Cybernetics-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    ...
    Any subprocess Cybernetics spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Cybernetics-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Cybernetics venv Scripts directory — host of the hermes.exe shim itself,
