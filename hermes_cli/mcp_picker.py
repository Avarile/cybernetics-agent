"""MCP picker — interactive `cybernetics mcp picker` (also the default `cybernetics mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``cybernetics mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `cybernetics plugin` picker UX: arrow keys to navigate, ENTER on a row
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Cybernetics session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `cybernetics mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: cybernetics mcp install <name>    Picker: cybernetics mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Cybernetics is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Cybernetics — run `cybernetics update` "
...
def show_catalog() -> None:
    """`cybernetics mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`cybernetics mcp picker` (and default `cybernetics mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`cybernetics mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `cybernetics mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
