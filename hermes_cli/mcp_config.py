"""
MCP Server Management CLI — ``cybernetics mcp`` subcommand.

Implements ``cybernetics mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.cybernetics/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.cybernetics/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``cybernetics mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  cybernetics mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  cybernetics mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  cybernetics mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: cybernetics mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `cybernetics mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  cybernetics mcp add <name> --url <endpoint>')
        _info('  cybernetics mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `cybernetics mcp remove` + `cybernetics mcp add` to reconfigure auth.")
        return
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `cybernetics mcp login " + name + "`.")
            return
        if tools:
...
# ─── cybernetics mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'cybernetics mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``cybernetics mcp`` subcommands."""
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `cybernetics plugin`.
        from hermes_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("cybernetics mcp                                    Open the catalog picker (default)")
        _info("cybernetics mcp catalog                            List Nous-approved MCPs")
        _info("cybernetics mcp install <name>                     Install a catalog MCP")
        _info("cybernetics mcp serve                              Run as MCP server")
        _info("cybernetics mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("cybernetics mcp add <name> --command <cmd>         Add a stdio server")
        _info("cybernetics mcp add <name> --preset <preset>       Add from a known preset")
        _info("cybernetics mcp remove <name>                      Remove a server")
        _info("cybernetics mcp list                               List configured servers")
        _info("cybernetics mcp test <name>                        Test connection")
        _info("cybernetics mcp configure <name>                   Toggle tools")
        _info("cybernetics mcp login <name>                       Re-authenticate OAuth")
        print()
