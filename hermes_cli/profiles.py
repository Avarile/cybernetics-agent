...
Usage::
    ...
    cybernetics profile create coder          # fresh profile + bundled skills
    cybernetics profile create coder --clone  # also copy config, .env, SOUL.md, skills
    cybernetics profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    cybernetics -p coder chat                 # or via flag
    cybernetics profile use coder             # set as sticky default
    cybernetics profile delete coder          # remove profile + alias + service
"""
...
#   backups             — `cybernetics backup` archives
...
# Marker file written by `cybernetics profile create --no-skills`.  When present in
...
# `cybernetics skills install` or drop SKILL.md files into the profile's skills/.
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the cybernetics root, NOT to the current HERMES_HOME
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, cybernetics subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _HERMES_SUBCOMMANDS:
        return f"'{canon}' conflicts with a cybernetics subcommand"
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``cybernetics -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``cybernetics profile alias
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``cybernetics profile list`` for an unrelated profile.
    """
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``cybernetics update`` skips re-seeding this profile's
    ...
    # Keys pages, `cybernetics -p <name> auth add`) had no file until first
    ...
    # Write the opt-out marker so seed_profile_skills() and `cybernetics update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`cybernetics profile create --no-skills`).\n"
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `cybernetics profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `cybernetics -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Profiles that opted out of bundled skills (via ``cybernetics profile create
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            ...
            "To remove everything, use: cybernetics uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: cybernetics profile create {canon}"
        )
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.cybernetics itself — its parent is ~/ and its
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: cybernetics profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.cybernetics itself — disallow
    ...
    if canon == "default":
        raise ValueError(
            ...
            "Specify a different name: cybernetics profile import <archive> --name <name>"
        )
...
def resolve_profile_env(profile_name: str) -> str:
    ...
    Called early in the CLI entry point, before any cybernetics modules
