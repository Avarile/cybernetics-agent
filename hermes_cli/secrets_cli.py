"""CLI handlers for ``cybernetics secrets bitwarden ...``.
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "Secrets will be pulled at the start of every Cybernetics process."
    )
    console.print(
        "  Status:  [cyan]cybernetics secrets bitwarden status[/cyan]\n"
        "  Refresh: [cyan]cybernetics secrets bitwarden sync[/cyan]\n"
        "  Disable: [cyan]cybernetics secrets bitwarden disable[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]cybernetics secrets bitwarden setup[/cyan] to enable.")
        return 0
    if not token_set:
        console.print(
            f"\n  [yellow]Enabled but {token_env} is not set — Cybernetics will skip BSM "
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not bw_cfg.get("enabled"):
        console.print(
            ...
            "`cybernetics secrets bitwarden setup` first.[/yellow]"
        )
        return 1
    ...
    if res.returncode != 0:
        ...
        if "invalid_client" in lowered or "400 bad request" in lowered:
            console.print(
                ...
                "[cyan]cybernetics secrets bitwarden setup[/cyan] and pick EU or "
