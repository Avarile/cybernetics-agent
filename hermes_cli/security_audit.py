"""On-demand supply-chain audit for Cybernetics Agent installs.

Scans three surfaces a Cybernetics user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Cybernetics venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.cybernetics/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.cybernetics/plugins``.
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `cybernetics security audit`."""
