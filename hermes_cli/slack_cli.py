"""``cybernetics slack ...`` CLI subcommands.

Today only ``cybernetics slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ cybernetics slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(bot_name: str, bot_description: str) -> dict:
    ...
    it stays in sync with the rest of Cybernetics. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Cybernetics deployment — users can tweak them in the Slack UI after
    pasting.
    """
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Cybernetics agent on Slack")[:140],
        ...
        "features": {
            ...
            "assistant_view": {
                "assistant_description": "Chat with Cybernetics in threads and DMs.",
            },
        },
        "oauth_config": {
            "scopes": {
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Cybernetics")
      --description DESC  Override the bot description
      --slashes-only  Emit only the ``features.slash_commands`` array (for
                      merging into an existing manifest manually)
    """
    name = getattr(args, "name", None) or "Cybernetics"
    description = getattr(args, "description", None) or "Your Cybernetics agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("HERMES_HOME") or str(Path.home() / ".cybernetics")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Cybernetics app\n"
