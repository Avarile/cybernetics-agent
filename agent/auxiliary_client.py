...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.cybernetics/auth.json active provider)
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Cybernetics triggers summarization.  Higher values delay
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "CyberneticsAgent",
}
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Cybernetics Agent)",
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.cybernetics/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``cybernetics auth
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Cybernetics auth store.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `cybernetics setup` or `cybernetics model` and enter a valid http(s) base URL."
        ) from exc
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``cybernetics config aux reset``)."""
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `cybernetics model` but the
    #    old OPENAI_BASE_URL lingers in ~/.cybernetics/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: cybernetics model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.cybernetics/.env",
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: cybernetics auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: cybernetics model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: cybernetics model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: cybernetics doctor for "
                "diagnostics)"
            )
            return None, None
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `cybernetics model`."
                )
