...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.cybernetics/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Cybernetics ↔ models.dev
# ---------------------------------------------------------------------------

# Cybernetics provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Cybernetics (built lazily)
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``cybernetics config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Cybernetics provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Cybernetics agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Cybernetics provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Cybernetics ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Cybernetics or models.dev provider ID.  Tries exact match then
