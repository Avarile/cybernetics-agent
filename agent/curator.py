...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.cybernetics/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `cybernetics update` re-seeds.
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``cybernetics update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``cybernetics curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `cybernetics curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "`cybernetics curator run` (no flag).\n"
    "\n"
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Cybernetics' background skill CURATOR. This is an "
    ...
    "into ~/.cybernetics/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: cybernetics-config-*, cybernetics-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.cybernetics/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.cybernetics/logs/curator/``)
    ...
    authored skill data in ``~/.cybernetics/skills/``.
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: cybernetics curator status
        keep an umbrella stable: cybernetics curator pin document-tools
    ...
    lines.append("full report: cybernetics curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: cybernetics curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.cybernetics/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.cybernetics/skills/.archive/` for "
            "safety and can be restored via `cybernetics curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.cybernetics/skills/.archive/`. "
            "Restore any via `cybernetics curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `cybernetics curator restore <name>`")
    lines.append("- All archives live under `~/.cybernetics/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    # real pass out. We still record a summary so `cybernetics curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `cybernetics update` only if "
        ...
        # recorded in state so `cybernetics curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``cybernetics model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `cybernetics model` → auxiliary
