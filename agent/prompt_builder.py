...
from branding import BRAND_NAME, BRAND_SHORT
from hermes_constants import display_hermes_home, get_hermes_home, get_skills_dir, is_wsl
...
_HERMES_MD_NAMES = (".cybernetics.md", "CYBERNETICS.md", ".hermes.md", "HERMES.md")


def _find_hermes_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.cybernetics.md`` or ``CYBERNETICS.md`` (or legacy ``.hermes.md`` / ``HERMES.md``).
...
DEFAULT_AGENT_IDENTITY = (
    f"You are {BRAND_NAME}, an intelligent AI assistant created by Nous Research. "
...
HERMES_AGENT_HELP_GUIDANCE = (
    f"You run on {BRAND_NAME} (by Nous Research). When the user needs help with "
    f"{BRAND_SHORT} itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the documentation at https://cybernetics-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date "
    "information. Load the `cybernetics-agent` skill with skill_view(name='cybernetics-agent') "
...
KANBAN_GUIDANCE = (
    ...
    f"the shared board at `{display_hermes_home()}/kanban.db`. Your task id is in "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `cybernetics kanban <verb>` for board operations. Use "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Cybernetics "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        f"You are in the {BRAND_SHORT} WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Cybernetics itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Cybernetics runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Cybernetics itself is running. The host OS, home, and cwd "
                f"of the Cybernetics process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Cybernetics "
    ...
    # Cybernetics desktop GUI — any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        _desktop_hint = "Runtime surface: you're running inside the Cybernetics desktop GUI app."
        if _in_desktop_term:
            _desktop_hint += (
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.cybernetics/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Cybernetics Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `cybernetics-agent` skill "
            "first. It has the actual commands (e.g. `cybernetics config set …`, `cybernetics tools`, "
            "`cybernetics setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: cybernetics setup, cybernetics setup tools, cybernetics setup terminal, cybernetics status.",
        ]
    )
    return "\n".join(lines)
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.cybernetics.md / CYBERNETICS.md (or legacy .hermes.md) — walk to git root."""
    ...
    try:
        ...
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".cybernetics.md")
    except Exception as e:
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .cybernetics.md / CYBERNETICS.md  (walk to git root; also detects legacy .hermes.md)
