...
def _hermes_home_path() -> Path:
    ...
    try:
        ...
        return Path(os.path.expanduser("~/.cybernetics"))


def _hermes_root_path() -> Path:
    """Resolve the Cybernetics root dir (always the parent of any profile, never per-profile)."""
    try:
...
def is_write_denied(path: str) -> bool:
    ...
    # Cybernetics control-plane files: block both the ACTIVE profile's view
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Cybernetics path.

    Three categories are blocked:

      * Internal Cybernetics cache files under ``HERMES_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under HERMES_HOME and the global Cybernetics
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.cybernetics/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Cybernetics root so credential stores at <root>/auth.json etc. are also
    ...
    for hd in hermes_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Cybernetics cache file "
    ...
    for hd in hermes_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Cybernetics credential store "
    ...
    for hd in hermes_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Cybernetics MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Cybernetics MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
#
# Cybernetics profiles are separate HERMES_HOME dirs under
...
#
# Reference: May 2026 incident where a cybernetics-security profile session
# edited skills under both ``~/.cybernetics/profiles/cybernetics-security/skills/``
# AND ``~/.cybernetics/skills/`` (the default profile's skills) without realizing
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Cybernetics scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Cybernetics entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Cybernetics profile {info['target_profile']!r}, but the "
...
#
#   <HERMES_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.cybernetics/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.cybernetics/…`` (or legacy ``.cybernetics``)
# segment and warns regardless of which Cybernetics profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.cybernetics/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.cybernetics`` (or legacy ``.cybernetics``) part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.cybernetics/…`` (or the
    legacy ``.cybernetics`` form) and returns the index where the inner
    Cybernetics-state portion starts. Returns ``None`` for paths that do not
    contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .cybernetics / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] in (".cybernetics", ".hermes"):
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Cybernetics state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.cybernetics``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.cybernetics`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Cybernetics resolver to
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Cybernetics process never "
...
# full ``…/sandboxes/<backend>/<task>/home/.cybernetics/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.cybernetics/…``.  The root:root
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Cybernetics state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Cybernetics "
