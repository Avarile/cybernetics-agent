...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Cybernetics starts and finds ``~/.openclaw/``.

    Points users at ``cybernetics claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``cybernetics claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Cybernetics, run "
        "`cybernetics claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `cybernetics claw cleanup` (renames it to ~/.openclaw.pre-migration — "
