...
Plugins built on Cybernetics Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Cybernetics
...
embedded slugs, no shorthands. This mirrors Cybernetics' main config
...
handles every provider, fallback chain, and per-task override Cybernetics
supports.
"""
