...
Every Cybernetics request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Cybernetics Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=cybernetics-agent",
        "client=cybernetics-client-v<__version__>",
    ]
...
def _hermes_version() -> str:
    """Return the current Cybernetics release version, e.g. ``"0.13.0"``.
...
def hermes_client_tag() -> str:
    ...
    Format: ``client=cybernetics-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=cybernetics-client-v{_hermes_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=cybernetics-agent", hermes_client_tag()]
