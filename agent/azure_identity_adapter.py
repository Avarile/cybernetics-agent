...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Cybernetics does
...
class EntraIdentityConfig:
    ...
    Captures the Cybernetics-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Cybernetics-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.cybernetics/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Cybernetics processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``cybernetics doctor`` /
    ``cybernetics auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``cybernetics doctor`` and the wizard preflight — never
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `cybernetics doctor` or `az login` to recover.",
                exc,
            )
