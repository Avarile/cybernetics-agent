...
* First-use consent is gated by the allowlist under
  ``~/.cybernetics/shell-hooks-allowlist.json``.  Non-TTY callers must pass
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``cybernetics hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Cybernetics wire-shape dict.

    For ``pre_tool_call`` the Claude-Code-style ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Cybernetics
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Cybernetics is about to register a shell hook that will run a\n"
...
# ---------------------------------------------------------------------------
# Introspection (used by `cybernetics hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``cybernetics hooks test`` and ``cybernetics hooks doctor``.
    ...
    produce — otherwise scripts tested via ``cybernetics hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Cybernetics-wire-shape response."""
