...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``cybernetics model`` / ``cybernetics auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Cybernetics process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``cybernetics`` invocation that defaults to the active provider
        (e.g. setup wizard, ``cybernetics auth status``) would land on
        ...
        try:
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    self._persist()
                    return None
            # For openai-codex: same race as xAI/nous — another Cybernetics process
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Cybernetics profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `cybernetics model` / `cybernetics auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `cybernetics model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``cybernetics auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `cybernetics auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `cybernetics auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Cybernetics PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `cybernetics setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.cybernetics/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `cybernetics setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `cybernetics auth add nous --label <name>`).  Fall back to the
        ...
        # Cybernetics auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.cybernetics/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `cybernetics auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `cybernetics auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Cybernetics auth store.  Without this gate the removal is instantly
        ...
        # Cybernetics owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `cybernetics auth openai-codex`.
        ...
        # When the user logs in via ``cybernetics model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``cybernetics auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.cybernetics/.env over os.environ — the user's config file is the
    # authoritative source for Cybernetics credentials. Stale env vars from parent
    ...
    # Honour user suppression — `cybernetics auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.cybernetics/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.cybernetics/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # Cybernetics PKCE is Hermes-owned/persistable while present, but it is
