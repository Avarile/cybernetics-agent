...
description: "Run Cybernetics Agent directly on an Android phone with Termux"
---

# Cybernetics on Android with Termux

This is the tested path for running Cybernetics Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Cybernetics from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Cybernetics now ships a Termux-aware installer path:
...
- links `cybernetics` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `cybernetics` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/cybernetics" "$PREFIX/bin/cybernetics"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `cybernetics` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.cybernetics/.env`.
...
### `cybernetics doctor` says ripgrep or Node is missing
