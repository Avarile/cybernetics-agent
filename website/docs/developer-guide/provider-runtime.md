...
description: "How Cybernetics resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Cybernetics has a shared provider runtime resolver used across:
...
- `cybernetics_cli/runtime_provider.py` — credential resolution, `_resolve_custom_runtime()`
- `cybernetics_cli/auth.py` — provider registry, `resolve_provider()`
- `cybernetics_cli/model_switch.py` — shared `/model` switch pipeline (CLI + gateway)
...
That ordering matters because Cybernetics treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `cybernetics model`.
...
This resolver is the main reason Cybernetics can share auth/runtime logic between:
...
Cybernetics contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY` and `OPENAI_API_KEY`).
...
Cybernetics also distinguishes between:
...
When provider resolution selects `anthropic`, Cybernetics uses:
...
- Cybernetics preflights Anthropic credential refresh before native Messages API calls
- Cybernetics still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Cybernetics resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `cybernetics model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Cybernetics supports a configured fallback provider chain — a list of `(provider, model)` entries tried in order when the primary model encounters errors. The legacy single-pair `fallback_model` dict is still accepted for back-compat (and migrated on first write).
...
- `tests/cybernetics_cli/test_fallback_cmd.py` — the `/fallback` CLI command
