...
sidebar_position: 1
title: "Architecture"
description: "Cybernetics Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Cybernetics Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ cybernetics_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # CyberneticsCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── cybernetics_state.py           # SQLite session/state database with FTS5
├── cybernetics_constants.py       # HERMES_HOME, profile-aware paths
...
│
├── cybernetics_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `cybernetics` subcommands (large file)
...
│   ├── skills_config.py      # cybernetics skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # cybernetics tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # cybernetics gateway start/stop
│
...
```text
User input → CyberneticsCLI.process_input()
...
Three discovery sources: `~/.cybernetics/plugins/` (user), `.cybernetics/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `cybernetics plugins` or `config.yaml`.

→ [Plugin Guide](/guides/build-a-cybernetics-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Cybernetics as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`cybernetics -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
