...
description: "Build wrapper CLIs that extend the Cybernetics TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Cybernetics exposes protected extension hooks on `CyberneticsCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Cybernetics."""

from cli import CyberneticsCLI
from prompt_toolkit.layout import FormattedTextControl, Window
from prompt_toolkit.filters import Condition


class MyCLI(CyberneticsCLI):
...
Called after Cybernetics registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
