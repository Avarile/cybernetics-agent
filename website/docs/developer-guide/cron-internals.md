...
description: "How Cybernetics stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `cybernetics_cli/cron.py` | CLI `cybernetics cron` subcommands |
...
Jobs are stored in `~/.cybernetics/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `cybernetics cron` commands are run or during active CLI sessions.
...
```python
# ~/.cybernetics/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.cybernetics/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `cybernetics cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `cybernetics cron` CLI provides direct job management:

```bash
cybernetics cron list                    # Show all jobs
cybernetics cron create                  # Interactive job creation (alias: add)
cybernetics cron edit <job_id>           # Edit job configuration
cybernetics cron pause <job_id>          # Pause a running job
cybernetics cron resume <job_id>         # Resume a paused job
cybernetics cron run <job_id>            # Trigger immediate execution
cybernetics cron remove <job_id>         # Delete a job
```
