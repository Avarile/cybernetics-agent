...
description: "How to add a new inference provider to Cybernetics Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Cybernetics can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `cybernetics model` menu entries
...
1. `cybernetics_cli/auth.py` decides how credentials are found.
2. `cybernetics_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `cybernetics_cli/models.py` and `cybernetics_cli/main.py` make the provider show up in the CLI. (`cybernetics_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
1. `cybernetics_cli/auth.py`
2. `cybernetics_cli/models.py`
3. `cybernetics_cli/runtime_provider.py`
4. `cybernetics_cli/main.py`
...
:::tip
`cybernetics_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `cybernetics setup`.
:::
...
7. `cybernetics model` menu includes the provider
8. `cybernetics setup` wizard delegates to `main.py` automatically
...
- Provider-specific `cybernetics model` menu entries with bespoke auth flows
...
- `PROVIDER_REGISTRY` in `cybernetics_cli/auth.py`
- `_PROVIDER_LABELS` in `cybernetics_cli/models.py`
- `_PROVIDER_ALIASES` in both `cybernetics_cli/auth.py` and `cybernetics_cli/models.py`
- CLI `--provider` choices in `cybernetics_cli/main.py`
...
## Step 2: Add auth metadata in `cybernetics_cli/auth.py`
...
- What env vars should Cybernetics check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `cybernetics_cli/models.py`
...
## Step 4: Resolve runtime data in `cybernetics_cli/runtime_provider.py`
...
Be careful with API-key precedence. Cybernetics already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `cybernetics_cli/main.py`

A provider is not discoverable until it shows up in the interactive `cybernetics model` flow.

Update these in `cybernetics_cli/main.py`:
...
:::tip
`cybernetics_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `cybernetics model` and `cybernetics setup` automatically.
:::
...
When you add a native provider, double-check that Cybernetics is only sending fields that provider actually understands.
...
- `tests/cybernetics_cli/test_runtime_provider_resolution.py`
- `tests/cli/test_cli_provider_resolution.py`
- `tests/cybernetics_cli/test_model_switch_custom_providers.py` (and adjacent `tests/cybernetics_cli/test_model_switch_*.py`)
- `tests/cybernetics_cli/test_setup_model_provider.py`
...
python -m pytest tests/cybernetics_cli/test_runtime_provider_resolution.py tests/cli/test_cli_provider_resolution.py tests/cybernetics_cli/test_setup_model_provider.py tests/run_agent/test_provider_parity.py -n0 -q
```
...
```bash
source venv/bin/activate
python -m cybernetics_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m cybernetics_cli.main model
python -m cybernetics_cli.main setup
```
...
- [ ] `ProviderConfig` added in `cybernetics_cli/auth.py`
- [ ] aliases added in `cybernetics_cli/auth.py` and `cybernetics_cli/models.py`
- [ ] model catalog added in `cybernetics_cli/models.py`
- [ ] runtime branch added in `cybernetics_cli/runtime_provider.py`
- [ ] CLI wiring added in `cybernetics_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `cybernetics model` but not `cybernetics setup`
