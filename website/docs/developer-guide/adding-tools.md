...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Cybernetics Agent — schemas, handlers, registration, and toolsets"
---
...
This page is for adding a **built-in Cybernetics tool** to the repository itself.
If you want a personal, project-local, or otherwise custom tool without
modifying Cybernetics core, use the plugin route instead:

- [Plugins](/user-guide/features/plugins)
- [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin)
...
If your tool requires an API key, add it to `cybernetics_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `cybernetics_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `cybernetics chat -q "Use the weather tool for London"`
