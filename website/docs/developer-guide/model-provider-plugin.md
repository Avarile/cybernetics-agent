...
description: "How to build a model provider (inference backend) plugin for Cybernetics Agent"
---

# Building a Model Provider Plugin

Model provider plugins declare an inference backend — an OpenAI-compatible endpoint, an Anthropic Messages server, a Codex-style Responses API, or a Bedrock-native surface — that Cybernetics can route `AIAgent` calls through. Every built-in provider (OpenRouter, Anthropic, GMI, DeepSeek, Nvidia, …) ships as one of these plugins. Third parties can add their own by dropping a directory under `$HERMES_HOME/plugins/model-providers/` with zero changes to the repo.
...
1. **Bundled plugins** — `<repo>/plugins/model-providers/<name>/` — ship with Cybernetics
...
The only required file is `__init__.py`. `plugin.yaml` is used by `cybernetics plugins` for introspection and by the general PluginManager to route the plugin to the right loader; without it, the general loader falls back to a source-text heuristic.
...
|---|---|---|
| Credential resolution | `cybernetics_cli/auth.py` | `PROVIDER_REGISTRY["acme-inference"]` populated from profile |
| `--provider` CLI flag | `cybernetics_cli/main.py` | Accepts `acme-inference` |
| `cybernetics model` picker | `cybernetics_cli/models.py` | Appears in `CANONICAL_PROVIDERS`, model list fetched from `{base_url}/models` |
| `cybernetics doctor` | `cybernetics_cli/doctor.py` | Health check for `ACME_API_KEY` + `{base_url}/models` probe |
| `cybernetics setup` | `cybernetics_cli/config.py` | `ACME_API_KEY` appears in `OPTIONAL_ENV_VARS` and the setup wizard |
...
| Runtime resolution | `cybernetics_cli/runtime_provider.py` | Returns correct `base_url`, `api_key`, `api_mode` |
...
| `display_name` | str | Human label shown in `cybernetics model` picker |
...
Say you want to point `gmi` at your private staging endpoint for testing. Create `~/.cybernetics/plugins/model-providers/gmi/__init__.py`:
...
Four values are recognized. Cybernetics picks one based on:
...
`auth_type` gates which codepaths treat your provider as a "simple api-key provider" — if it's not `api_key`, the PluginManager still records the manifest but Cybernetics' CLI-level automation (doctor checks, `--provider` flag, setup wizard delegation) may skip over it.
...
```bash
export HERMES_HOME=/tmp/cybernetics-plugin-test
...
cybernetics -z "hello" --provider my-provider -m some-model
```

## General PluginManager integration

The general `PluginManager` (the thing `cybernetics plugins` operates on) **sees** model-provider plugins but does not import them — `providers/__init__.py` owns their lifecycle. The manager records the manifest for introspection and categorizes by `kind: model-provider`. When you drop an unlabeled user plugin into `$HERMES_HOME/plugins/` that happens to call `register_provider` with a `ProviderProfile`, the manager auto-coerces it to `kind: model-provider` via a source-text heuristic — so the plugin still routes correctly even without `plugin.yaml`.

## Distribute via pip

Like any Cybernetics plugin, model providers can ship as a pip package. Add an entry point to your `pyproject.toml`:

```toml
[project.entry-points."cybernetics_agent.plugins"]
acme-inference = "acme_cybernetics_plugin:register"
```

…where `acme_cybernetics_plugin:register` is a function that calls `register_provider(profile)`. The general PluginManager picks up entry-point plugins during `discover_and_load()`. For `kind: model-provider` pip plugins, you still need to declare the kind in your manifest (or rely on the source-text heuristic).

See [Building a Cybernetics Plugin](/guides/build-a-cybernetics-plugin#distribute-via-pip) for the full entry-points setup.
...
- [Building a Cybernetics Plugin](/guides/build-a-cybernetics-plugin) — general plugin authoring
