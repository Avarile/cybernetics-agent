...
The ACP adapter wraps Cybernetics' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
cybernetics acp / cybernetics-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.cybernetics/.env
  -> configure stderr logging
  -> construct CyberneticsACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'cybernetics-agent[acp]==<version>' cybernetics-acp`, pointed at the `cybernetics-agent` PyPI release.
...
- `allow_once` -> Cybernetics `once`
- `allow_always` -> Cybernetics `always`
- reject options -> Cybernetics `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Cybernetics tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["cybernetics-acp"])
...
Instead it reuses Cybernetics' runtime resolver:

- `acp_adapter/auth.py`
- `cybernetics_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Cybernetics provider/credentials. It also always advertises a terminal setup auth method (`cybernetics-setup`, args `--setup`) so first-run registry clients can open Cybernetics' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.cybernetics/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `cybernetics-acp` toolset definition
- `cybernetics_cli/main.py` — `cybernetics acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `cybernetics-acp` script
