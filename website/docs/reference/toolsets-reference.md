...
description: "Reference for Cybernetics core, composite, platform, and dynamic toolsets"
---
...
- **Platform** — A complete tool configuration for a specific deployment context (e.g., `cybernetics-cli` is the default for interactive CLI sessions)
...
```bash
cybernetics chat --toolsets web,file,terminal
cybernetics chat --toolsets debugging        # composite — expands to file + terminal + web
cybernetics chat --toolsets all              # everything
```
...
```yaml
toolsets:
  - cybernetics-cli          # default for CLI
  # - cybernetics-telegram   # override for Telegram gateway
```

### Interactive management

```bash
cybernetics tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Cybernetics tools programmatically. |
| `cronjob` | `cronjob` | Schedule and manage recurring tasks. |
| `debugging` | composite (`file` + `terminal` + `web`) | Debug bundle — file, process/terminal, web extract/search. |
| `delegation` | `delegate_task` | Spawn isolated subagent instances for parallel work. |
| `discord` | `discord` | Core Discord text/embed/DM actions (gateway-only). Active on the `cybernetics-discord` toolset. |
| `discord_admin` | `discord_admin` | Discord moderation (bans, role changes, channel management). Active on the `cybernetics-discord` toolset; requires the bot to hold the relevant Discord permissions. |
...
| `feishu_drive` | `feishu_drive_add_comment`, `feishu_drive_list_comments`, `feishu_drive_list_comment_replies`, `feishu_drive_reply_comment` | Feishu/Lark drive comment operations. Scoped to the comment agent; not exposed on `cybernetics-cli` or other messaging toolsets. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `cybernetics tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
| `yuanbao` | `yb_query_group_info`, `yb_query_group_members`, `yb_search_sticker`, `yb_send_dm`, `yb_send_sticker` | Yuanbao DM/group actions and sticker search. Registered only on `cybernetics-yuanbao`. |

## Platform Toolsets

Platform toolsets define the complete tool configuration for a deployment target. Most messaging platforms use the same set as `cybernetics-cli`:

| Toolset | Differences from `cybernetics-cli` |
|---------|-------------------------------|
| `cybernetics-cli` | Full toolset — the default for interactive CLI sessions. Includes file, terminal, web, browser, memory, skills, vision, image_gen, todo, tts, delegation, code_execution, cronjob, session_search, clarify, and `safe` (read-only) bundles plus the standard messaging tools. |
| `cybernetics-acp` | Drops `clarify`, `cronjob`, `image_generate`, `send_message`, `text_to_speech`, and all four Home Assistant tools. Focused on coding tasks in IDE context. |
| `cybernetics-api-server` | Drops `clarify`, `send_message`, and `text_to_speech`. Keeps everything else — suitable for programmatic access where user interaction isn't possible. |
| `cybernetics-cron` | Same as `cybernetics-cli`. |
| `cybernetics-telegram` | Same as `cybernetics-cli`. |
| `cybernetics-discord` | Adds `discord` and `discord_admin` on top of `cybernetics-cli`. |
| `cybernetics-slack` | Same as `cybernetics-cli`. |
| `cybernetics-whatsapp` | Same as `cybernetics-cli`. |
| `cybernetics-signal` | Same as `cybernetics-cli`. |
| `cybernetics-matrix` | Same as `cybernetics-cli`. |
| `cybernetics-mattermost` | Same as `cybernetics-cli`. |
| `cybernetics-email` | Same as `cybernetics-cli`. |
| `cybernetics-sms` | Same as `cybernetics-cli`. |
| `cybernetics-bluebubbles` | Same as `cybernetics-cli`. |
| `cybernetics-dingtalk` | Same as `cybernetics-cli`. |
| `cybernetics-feishu` | Adds the five `feishu_doc_*` / `feishu_drive_*` tools (only used by the document-comment handler, not the regular chat adapter). |
| `cybernetics-qqbot` | Same as `cybernetics-cli`. |
| `cybernetics-wecom` | Same as `cybernetics-cli`. |
| `cybernetics-wecom-callback` | Same as `cybernetics-cli`. |
| `cybernetics-weixin` | Same as `cybernetics-cli`. |
| `cybernetics-yuanbao` | Adds the five `yb_*` tools (DM/group/sticker) on top of `cybernetics-cli`. |
| `cybernetics-homeassistant` | Same as `cybernetics-cli` (the Home Assistant tools are already present by default and activate when `HASS_TOKEN` is set). |
| `cybernetics-webhook` | Same as `cybernetics-cli`. |
| `cybernetics-gateway` | Internal gateway orchestrator toolset — union of every `cybernetics-<platform>` toolset; used when the gateway needs to accept any message source. |
...
## Relationship to `cybernetics tools`

The `cybernetics tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
