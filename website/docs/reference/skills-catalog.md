...
description: "Catalog of bundled skills that ship with Cybernetics Agent"
---

# Bundled Skills Catalog

Cybernetics ships with a large built-in skill library copied into `~/.cybernetics/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.

Cybernetics also syncs bundled skills on `cybernetics update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.cybernetics/skills/` tree, it is still shipped with Cybernetics; restore it with `cybernetics skills reset <name> --restore`.
...
| [`cybernetics-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-cybernetics-agent) | Configure, extend, or contribute to Cybernetics Agent. | `autonomous-ai-agents/cybernetics-agent` |
...
| [`kanban-worker`](/docs/user-guide/skills/bundled/devops/devops-kanban-worker) | Pitfalls, examples, and edge cases for Cybernetics Kanban workers. The lifecycle itself is auto-injected into every worker's system prompt as KANBAN_GUIDANCE (from agent/prompt_builder.py); this skill is what you load when you want deeper det... | `devops/kanban-worker` |
...
| [`teams-meeting-pipeline`](/docs/user-guide/skills/bundled/productivity/productivity-teams-meeting-pipeline) | Operate the Teams meeting summary pipeline via Cybernetics CLI — summarize meetings, inspect pipeline status, replay jobs, manage Microsoft Graph subscriptions. | `productivity/teams-meeting-pipeline` |
...
| Skill | Description | Path |
|-------|-------------|------|
| [`cybernetics-agent-skill-authoring`](/docs/user-guide/skills/bundled/software-development/software-development-cybernetics-agent-skill-authoring) | Author in-repo SKILL.md: frontmatter, validator, structure. | `software-development/cybernetics-agent-skill-authoring` |
...
| [`plan`](/docs/user-guide/skills/bundled/software-development/software-development-plan) | Plan mode: write an actionable markdown plan to .cybernetics/plans/, no execution. Bite-sized tasks, exact paths, complete code. | `software-development/plan` |
