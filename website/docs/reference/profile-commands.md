...
This page covers all commands related to [Cybernetics profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `cybernetics profile` without a subcommand shows help.
...
## `cybernetics profile use`

```bash
cybernetics profile use <name>
```

Sets `<name>` as the active profile. All subsequent `cybernetics` commands (without `-p`) will use this profile.
...
cybernetics profile use default
```

## `cybernetics profile create`

```bash
cybernetics profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `cybernetics profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `cybernetics update` runs won't re-seed the bundled set, and refuses to combine with `--clone` / `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.cybernetics`), use `cybernetics skills opt-out` / `cybernetics skills opt-in`. |
...
cybernetics profile create mybot

# Clone config only from current profile
cybernetics profile create work --clone

# Clone everything from current profile
cybernetics profile create backup --clone-all

# Clone config from a specific profile
cybernetics profile create work2 --clone --clone-from work
```

## `cybernetics profile describe`

```bash
cybernetics profile describe [<name>] [options]
```
...
cybernetics profile describe researcher

# Set it explicitly
cybernetics profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
cybernetics profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
cybernetics profile describe --all --auto
```

## `cybernetics profile delete`

```bash
cybernetics profile delete <name> [options]
```
...
```bash
cybernetics profile delete mybot
cybernetics profile delete mybot --yes
```
...
## `cybernetics profile show`

```bash
cybernetics profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Cybernetics home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ cybernetics profile show work
Profile: work
Path:    ~/.cybernetics/profiles/work
...
## `cybernetics profile alias`

```bash
cybernetics profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Cybernetics installation.
...
```bash
cybernetics profile alias work
# Creates/updates ~/.local/bin/work

cybernetics profile alias work --name mywork
# Creates ~/.local/bin/mywork

cybernetics profile alias work --remove
# Removes the wrapper script
```

## `cybernetics profile rename`

```bash
cybernetics profile rename <old-name> <new-name>
```
...
```bash
cybernetics profile rename mybot assistant
# ~/.cybernetics/profiles/mybot → ~/.cybernetics/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `cybernetics profile export`

```bash
cybernetics profile export <name> [options]
```
...
```bash
cybernetics profile export work
# Creates work.tar.gz in the current directory

cybernetics profile export work -o ./work-2026-03-29.tar.gz
```

## `cybernetics profile import`

```bash
cybernetics profile import <archive> [options]
```
...
```bash
cybernetics profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

cybernetics profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`cybernetics profile export` / `import` are still the right commands for
...
### `cybernetics profile install`

```bash
cybernetics profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `cybernetics -p telemetry`). |
...
cybernetics profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
cybernetics profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
cybernetics profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
cybernetics profile install ./telemetry/
```

### `cybernetics profile update`

```bash
cybernetics profile update <name> [--force-config] [--yes]
```
...
### `cybernetics profile info`

```bash
cybernetics profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Cybernetics version, author, env var requirements, the source URL/path, and
...
`cybernetics profile list` also shows the distribution name and version in a
`Distribution` column, and `cybernetics profile show <name>` / `delete <name>`
...
cybernetics profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
cybernetics profile install https://github.com/your-org/internal-assistant.git
```
...
`cybernetics_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Cybernetics version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Cybernetics can clone from.
3. Tell recipients to run `cybernetics profile install <your-repo-url>`.
...
## `cybernetics -p` / `cybernetics --profile`

```bash
cybernetics -p <name> <command> [options]
cybernetics --profile <name> <command> [options]
```

Global flag to run any Cybernetics command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
cybernetics -p work chat -q "Check the server status"
cybernetics --profile dev gateway start
cybernetics -p personal skills list
cybernetics -p work config edit
```

## `cybernetics completion`

```bash
cybernetics completion <shell>
```
...
- `cybernetics profile <TAB>` — subcommands (list, use, create, etc.)
- `cybernetics profile use <TAB>` — profile names
- `cybernetics -p <TAB>` — profile names
