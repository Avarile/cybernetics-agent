...
sidebar_position: 3
title: "Desktop App"
description: "The native Cybernetics desktop app — a polished experience for chatting with Cybernetics, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Cybernetics desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Cybernetics Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `cybernetics` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Cybernetics has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`cybernetics`) and **[TUI](./tui.md)** (`cybernetics --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`cybernetics dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Cybernetics Desktop](../getting-started/installation.md).

If you already have Cybernetics installed, simply run
...
- **The same conversation history** as every other Cybernetics surface — sessions started here resume in the CLI/TUI and vice versa.
...
Chatting against a Cybernetics instance on another machine instead of the bundled local backend? See [Connecting to a remote backend](#connecting-to-a-remote-backend) below — and for the full picture of how the remote-hosted dashboard connection works (the auth gate, the `/api/ws` chat socket, and WebSocket close-code triage), see [Web Dashboard → Connecting Cybernetics Desktop to a remote backend](./features/web-dashboard.md#connecting-cybernetics-desktop-to-a-remote-backend).

### File browser

Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `cybernetics desktop --cwd <path>` (or the `HERMES_DESKTOP_CWD` environment variable).

### Voice

Talk to Cybernetics and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `cybernetics model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Cybernetics management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#cybernetics-cron).
- **Profiles** — switch between [Cybernetics profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://cybernetics-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Cybernetics agent, your config, and your chats stay. (Same as `cybernetics uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `cybernetics uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `cybernetics uninstall --full`.)
...
You can do the same from the terminal — `cybernetics uninstall --gui` for the GUI alone, or `cybernetics uninstall` / `cybernetics uninstall --full` for the agent too.

:::note
Running `cybernetics uninstall --gui` from a **source checkout** (a `cybernetics desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `cybernetics desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `cybernetics desktop`

To launch via the CLI, simply run `cybernetics desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `cybernetics update`)                      |
...
| `--cybernetics-root PATH` | Override the Cybernetics source root the app uses (sets `HERMES_DESKTOP_HERMES_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `cybernetics` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships only the Electron shell. On first launch it installs the Cybernetics Agent runtime into `HERMES_HOME` (`~/.cybernetics`, or `%LOCALAPPDATA%\cybernetics` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. The React renderer talks to a `cybernetics dashboard` backend over the standard gateway APIs and reuses the agent rather than reimplementing it. Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Cybernetics backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `cybernetics dashboard` process
"Remote backend" means a **`cybernetics dashboard`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that dashboard is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `cybernetics dashboard` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the dashboard bound to a reachable address. The credentials live in `~/.cybernetics/.env` (the secrets file, mode 0600):
...
Keep that `cybernetics dashboard` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.
...
Running the dashboard as a systemd service? Give the unit `EnvironmentFile=%h/.cybernetics/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/cybernetics` work if you front it with a reverse proxy)
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.cybernetics/.env` and that the dashboard process actually loaded them.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Cybernetics Desktop to a remote backend](./features/web-dashboard.md#connecting-cybernetics-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Cybernetics Desktop](../reference/environment-variables.md#web-dashboard--cybernetics-desktop).
...
rm "$HOME/.cybernetics/cybernetics-agent/.cybernetics-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.cybernetics/cybernetics-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.cybernetics
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.cybernetics/cybernetics-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
