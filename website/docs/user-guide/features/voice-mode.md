...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Cybernetics Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Cybernetics Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Cybernetics](/guides/use-voice-mode-with-cybernetics).

## Prerequisites

Before using voice features, make sure you have:

1. **Cybernetics Agent installed** — `pip install cybernetics-agent` (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `cybernetics model` or set your preferred provider credentials in `~/.cybernetics/.env`
3. **A working base setup** — run `cybernetics` to verify the agent responds to text before enabling voice

:::tip
The `~/.cybernetics/` directory and default `config.yaml` are created automatically the first time you run `cybernetics`. You only need to create `~/.cybernetics/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `cybernetics setup --portal` wires both up at once.
:::
...
pip install "cybernetics-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "cybernetics-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "cybernetics-agent[tts-premium]"
...
Voice mode is available in both the **classic CLI** (`cybernetics chat`) and the **TUI** (`cybernetics --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.cybernetics/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
cybernetics                # Start the interactive CLI
```
...
1. Start the CLI with `cybernetics` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.cybernetics/config.yaml` (default: `ctrl+b`).
:::
...
```bash
cybernetics gateway        # Start the gateway (connects to configured platforms)
cybernetics gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.cybernetics/.env`:
```bash
...
```bash
cybernetics gateway        # Start with existing configuration
```
...
If you are running Cybernetics inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.cybernetics/.env`
...
- Review gateway logs: `tail -f ~/.cybernetics/logs/gateway.log`
