...
Cybernetics Agent can drive your Mac's desktop — clicking, typing, scrolling,
...
Use `cybernetics computer-use status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `cybernetics tools`, pick `🖱️ Computer Use (macOS)` → `cua-driver (background)`.
...
4. Start a session with the toolset enabled:
   ...
   ```
   or add `computer_use` to your enabled toolsets in `~/.cybernetics/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Cybernetics refreshes the binary in two
places so you don't get stuck on a stale release:

- **`cybernetics update`** — when you update Cybernetics itself, if `cua-driver` is
...
- **`cybernetics computer-use install --upgrade`** — manual force-refresh.
...
`cybernetics computer-use status` shows the installed version next to the
binary path.
...
Cybernetics applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.cybernetics/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Cybernetics applies four layers of optimisation:
...
`cybernetics computer-use install` to fetch the cua-driver binary, or run
`cybernetics tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/NousResearch/cybernetics-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
