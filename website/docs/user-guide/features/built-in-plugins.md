...
description: "Plugins shipped with Cybernetics Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Cybernetics ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.cybernetics/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin) to write your own.
...
2. **User** — `~/.cybernetics/plugins/<name>/`
3. **Project** — `./.cybernetics/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `cybernetics_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `cybernetics memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `cybernetics plugins list` and the interactive `cybernetics plugins` UI), but none load until you explicitly enable them:

```bash
cybernetics plugins enable disk-cleanup
```

Or via `~/.cybernetics/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Cybernetics. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
cybernetics plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `cybernetics plugins enable <name>`.
...
| `cybernetics-achievements` | dashboard tab | Steam-style collectible badges generated from your real Cybernetics session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `cybernetics memory` and `cybernetics plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HERMES_HOME` or `/tmp/cybernetics-*`, track it silently as `test` / `temp` / `cron-output`. |
...
**Safety** — cleanup only ever touches paths under `HERMES_HOME` or `/tmp/cybernetics-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `cybernetics plugins enable disk-cleanup` (or check the box in `cybernetics plugins`).

**Disabling again:** `cybernetics plugins disable disk-cleanup`.
...
**Enabling:** `cybernetics plugins enable security-guidance` (or check the box in `cybernetics plugins`).

**Disabling again:** `cybernetics plugins disable security-guidance`.
...
Traces Cybernetics turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Cybernetics' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `cybernetics logs`.
...
```bash
cybernetics tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Cybernetics and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
cybernetics plugins enable observability/langfuse
```

Then put the credentials in `~/.cybernetics/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Cybernetics turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Cybernetics session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `cybernetics chat` session lives under one Langfuse session.

**Verify:**

```bash
cybernetics plugins list                 # observability/langfuse should show "enabled"
cybernetics chat -q "hello"              # check the Langfuse UI for a "Cybernetics turn" trace
```
...
Cybernetics-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Cybernetics-prefixed wins when both are set.
...
**Disabling:** `cybernetics plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.cybernetics/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
cybernetics plugins enable google_meet
...
**Disabling:** `cybernetics plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.cybernetics/cache/google_meet/` until you remove them.

### cybernetics-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Cybernetics session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Cybernetics feature changes.

**How it works:**

- Scans your entire `~/.cybernetics/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$HERMES_HOME/plugins/cybernetics-achievements/state.json`
...
| Secret | Hidden until Cybernetics detects the first related signal in your history |

**API** — routes mount under `/api/plugins/cybernetics-achievements/`:
...
**State files** — live under `$HERMES_HOME/plugins/cybernetics-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Cybernetics updates. |
...
**Enabling:** Nothing to enable — `cybernetics-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `cybernetics dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/cybernetics-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.cybernetics/plugins/cybernetics-achievements/` that ships no dashboard. The plugin's state files under `$HERMES_HOME/plugins/cybernetics-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Cybernetics plugin — see [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.cybernetics/plugins/<name>/`
- Manifest source is reported as `bundled` in `cybernetics plugins list`
