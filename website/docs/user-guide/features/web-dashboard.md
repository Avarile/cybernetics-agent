...
The web dashboard is a browser-based UI for managing your Cybernetics Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `cybernetics setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
cybernetics dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
cybernetics dashboard --host 0.0.0.0

# Start without opening browser
cybernetics dashboard --no-open
```
...
processes (manage them via `cybernetics -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `cybernetics-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'cybernetics-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install cybernetics-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `cybernetics dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `cybernetics dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- Node.js (same requirement as `cybernetics --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'cybernetics-agent[web,pty]'`, or `[all]` covers both)
...
```ini
[Service]
EnvironmentFile=%h/.cybernetics/.env
ExecStart=/path/to/venv/bin/python -m cybernetics_cli.main dashboard \
    --host 0.0.0.0 --port 9119 --no-open
```

with `~/.cybernetics/.env` containing:
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `cybernetics config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Cybernetics instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `cybernetics profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.cybernetics/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `cybernetics skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
block in `config.yaml` that `cybernetics mcp` reads from.
...
`cybernetics mcp catalog` / `cybernetics mcp install` use.
...
Connect Cybernetics to any messaging platform from the browser — full parity with
`cybernetics setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.cybernetics/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Cybernetics versions, CPU core count + utilization, memory, disk usage of the Cybernetics home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Cybernetics version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git or pip install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `cybernetics update` in the background. On Docker/Nix/Homebrew installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
- **Nous Portal** — login status, the active inference provider, and the Tool Gateway routing table (which tools run via the Portal vs. locally), with a link to manage your subscription. Read-only mirror of `cybernetics portal`.
- **Skill curator** — the background skill-maintenance status (active / paused, interval, last run) with pause/resume and a run-now button. Mirrors `cybernetics curator`.
...
This re-reads `~/.cybernetics/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
| `GET /api/cybernetics/update/check` | Report update availability (commits behind, install method) without applying. For git/pip installs that are behind, also returns a `commits` list (`sha`, `summary`, `author`, `at`) of what's changed. `?force=1` busts the 6h cache |
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Cybernetics Agent engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `cybernetics dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `cybernetics dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `cybernetics dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.
...
- **CLI — `cybernetics dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `cybernetics setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.cybernetics/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  cybernetics dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.cybernetics/.env
  ```
...
|---------|-----------|--------|----------------|
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `cybernetics dashboard register` |

Per the Cybernetics Agent convention (`~/.cybernetics/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Bundled providers reported these issues:
  • nous: HERMES_DASHBOARD_OAUTH_CLIENT_ID is not set (and
    ...
    deploys a Cybernetics Agent instance — set it to your provisioned
...
From a logged-in Cybernetics install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `cybernetics dashboard register` uses your existing Nous login to provision an OAuth client and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.cybernetics/.env` for you:

```bash
cybernetics setup            # if you're not already logged into Nous Portal
cybernetics dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.cybernetics/.env
```
...
```bash
cybernetics dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
`GET /api/auth/me` then returns the verified session (`provider: nous`). For an internet-facing host, register with `--redirect-uri https://cybernetics.example.com/auth/callback` and set `HERMES_DASHBOARD_PUBLIC_URL` so the OAuth callback resolves to your public URL (see [Public URL override](#public-url-override)).
...
**1. Set credentials in `~/.cybernetics/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
dashboard:
  oauth:
    ...
    self_hosted:
      issuer: https://auth.example.com/application/o/cybernetics/   # required
      client_id: cybernetics-dashboard                              # required
...
**1. Run Keycloak with a pre-configured realm.** Save this realm export as `realm-cybernetics.json` — it defines a `cybernetics` realm, a **public PKCE client** (`cybernetics-dashboard`), and a test user, all imported on boot so there's nothing to click in the admin UI:
...
```json
{
  ...
  "clients": [
    {
      "clientId": "cybernetics-dashboard",
      "name": "Cybernetics Agent Dashboard",
      "enabled": true,
      "publicClient": true,
...
docker run --rm -p 8080:8080 \
  ...
  -v "$PWD/realm-cybernetics.json:/opt/keycloak/data/import/realm-cybernetics.json:ro" \
  quay.io/keycloak/keycloak:26.0 \
  start-dev --import-realm
```

Once it's up, the realm advertises standard OIDC discovery at
`http://localhost:8080/realms/cybernetics/.well-known/openid-configuration` (issuer
`http://localhost:8080/realms/cybernetics`). The admin console is at
...
```bash
export HERMES_DASHBOARD_OIDC_ISSUER="http://localhost:8080/realms/cybernetics"
export HERMES_DASHBOARD_OIDC_CLIENT_ID="cybernetics-dashboard"
...
> admin console (Clients → cybernetics-dashboard → Settings). The same pattern works
...
```yaml
dashboard:
  public_url: "https://dashboard.example.com/cybernetics"
```
...
5. Server exchanges the code for an access token at `POST /api/oauth/token`, verifies the JWT signature against the Portal's JWKS (`/.well-known/jwks.json`), and sets the `cybernetics_session_at` cookie.
...
| Name | Lifetime | Notes |
|------|----------|-------|
| `cybernetics_session_at` | Token TTL (15 min) | HttpOnly, SameSite=Lax, Secure-when-HTTPS |
| `cybernetics_session_pkce` | 10 min | HttpOnly; holds the PKCE verifier + provider hint during the round trip |
| `cybernetics_session_rt` | unused in v1 | Reserved for forward-compat; not written when `refresh_token` is empty |
...
```python
# ~/.cybernetics/plugins/dashboard-auth-myidp/__init__.py
from cybernetics_cli.dashboard_auth import DashboardAuthProvider, Session, LoginStart
...
Cybernetics Desktop can drive a Cybernetics backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateway → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Cybernetics Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`cybernetics dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
```bash
# 1. Set the dashboard login credentials in ~/.cybernetics/.env (secrets file, 0600).
cat >> ~/.cybernetics/.env <<'EOF'
HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
cybernetics dashboard --no-open --host 0.0.0.0 --port 9119
```
...
If you run the dashboard as a systemd service, `~/.cybernetics/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.cybernetics/.env`, so the credentials are in the environment at boot.
...
### In Cybernetics Desktop

**Settings → Gateway → Remote gateway:**

- **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/cybernetics` are supported if you front it with a reverse proxy)
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `cybernetics_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `cybernetics update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `cybernetics dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Cybernetics Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Cybernetics Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
