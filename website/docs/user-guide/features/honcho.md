...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Cybernetics's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Cybernetics instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
cybernetics memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `cybernetics` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Cybernetics syncs them back at session init.
...
The `cybernetics honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `cybernetics memory setup honcho` (or run `cybernetics memory setup` and pick it from the list); the `cybernetics honcho` subcommand then appears on the next invocation.

```bash
cybernetics memory setup honcho    # Configure Honcho directly (works before activation)
cybernetics honcho status          # Connection status, config, and key settings
cybernetics honcho setup           # Redirects to `cybernetics memory setup` (post-activation alias)
cybernetics honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
cybernetics honcho peer            # Show or update peer names + dialectic reasoning level
cybernetics honcho mode            # Show or set recall mode (hybrid/context/tools)
cybernetics honcho tokens          # Show or set token budget for context and dialectic
cybernetics honcho identity        # Seed or show the AI peer's Honcho identity
cybernetics honcho sync            # Sync Honcho config to all existing profiles
cybernetics honcho peers           # Show peer identities across all profiles
cybernetics honcho sessions        # List known Honcho session mappings
cybernetics honcho map             # Map current directory to a Honcho session name
cybernetics honcho enable          # Enable Honcho for the active profile
cybernetics honcho disable         # Disable Honcho for the active profile
cybernetics honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `cybernetics honcho`

If you previously used the standalone `cybernetics honcho setup`:
...
No re-login or re-setup needed. Run `cybernetics memory setup` and select "honcho" — the wizard detects your existing config.
