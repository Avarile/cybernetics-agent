...
When Cybernetics Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.cybernetics/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.cybernetics/config.yaml` (switchable per session
via `/personality`).
...
If you use Cybernetics' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.cybernetics/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Cybernetics Agent's deliverable
