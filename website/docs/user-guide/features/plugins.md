...
title: "Plugins"
description: "Extend Cybernetics with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Cybernetics has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Cybernetics
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.cybernetics/plugins/` with a `plugin.yaml` and Python code:

```
~/.cybernetics/plugins/my-plugin/
...
Start Cybernetics — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.cybernetics/plugins/hello-world/plugin.yaml`**
...
**`~/.cybernetics/plugins/hello-world/__init__.py`**

```python
"""Minimal Cybernetics plugin — registers a tool and a hook."""
...
Drop both files into `~/.cybernetics/plugins/hello-world/`, restart Cybernetics, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.cybernetics/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Cybernetics.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `cybernetics <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `cybernetics plugins install` |
| Distribute via pip | `[project.entry-points."cybernetics_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Cybernetics — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.cybernetics/plugins/` | Personal plugins |
| Project | `.cybernetics/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `cybernetics_agent.plugins` entry_points | Distributed packages |
| Nix | `services.cybernetics-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Cybernetics also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.cybernetics/plugins/model-providers/<name>/` and `~/.cybernetics/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `cybernetics plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.cybernetics/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
cybernetics plugins                    # interactive toggle (space to check/uncheck)
cybernetics plugins enable <name>      # add to allow-list
cybernetics plugins disable <name>     # remove from allow-list + add to disabled
```

After `cybernetics plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Cybernetics' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.cybernetics/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.cybernetics/plugins/`.

### Migration for existing users

When you upgrade to a version of Cybernetics that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.cybernetics/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Cybernetics has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.cybernetics/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Cybernetics also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Cybernetics Plugin](/guides/build-a-cybernetics-plugin) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `cybernetics <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Cybernetics auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `cybernetics skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.cybernetics/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `cybernetics plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
cybernetics plugins                               # unified interactive UI
cybernetics plugins list                          # table: enabled / disabled / not enabled
cybernetics plugins install user/repo             # install from Git, then prompt Enable? [y/N]
cybernetics plugins install user/repo --enable    # install AND enable (no prompt)
cybernetics plugins install user/repo --no-enable # install but leave disabled (no prompt)
cybernetics plugins update my-plugin              # pull latest
cybernetics plugins remove my-plugin              # uninstall
cybernetics plugins enable my-plugin              # add to allow-list
cybernetics plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `cybernetics plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `cybernetics plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/guides/build-a-cybernetics-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
