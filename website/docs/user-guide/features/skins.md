...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Cybernetics CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Cybernetics CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.cybernetics/skins/mytheme.yaml
```

Or set the default skin in `~/.cybernetics/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Cybernetics — gold and kawaii | `Cybernetics Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome — clean grayscale | `Cybernetics Agent` | All grays — no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue — developer-focused | `Cybernetics Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner — uses default faces. |
| `daylight` | Light theme for bright terminals with dark text and cool blue accents | `Cybernetics Agent` | Designed for white or bright terminals. Dark slate text with blue borders, pale status surfaces, and a light completion menu that stays readable in light terminal profiles. |
| `warm-lightmode` | Warm brown/gold text for light terminal backgrounds | `Cybernetics Agent` | Warm parchment tones for light terminals. Dark brown text with saddle-brown accents, cream-colored status surfaces. An earthy alternative to the cooler daylight theme. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Cybernetics Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Cybernetics Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` ⚕ Cybernetics ` |
...
Create YAML files under `~/.cybernetics/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.cybernetics/skins/mytheme.yaml
...
## Cybernetics Mod — Visual Skin Editor
...
- Opens any skin into a visual editor with all Cybernetics skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.cybernetics/skins/`
- Activates a skin by updating `~/.cybernetics/config.yaml`
...
6. Click **Save** to write the skin YAML to `~/.cybernetics/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Cybernetics Mod respects the `HERMES_HOME` environment variable, so it works with [profiles](/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `cybernetics_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.cybernetics/skins/` take precedence over built-in skins with the same name.
