...
**Tool Search** is Cybernetics' opt-in progressive-disclosure layer for that
...
:::info Built-in Cybernetics tools never defer
The tools that make up Cybernetics' core capability set (`terminal`,
...
When the model invokes `tool_call`, Cybernetics **unwraps the bridge** and
...
- **No JS sandbox.** Cybernetics uses the simpler "structured tools" mode
