...
chat completions — use your Cybernetics-managed provider subscription as their
...
| Use case | "Use Cybernetics as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Cybernetics stores
the refresh token in `~/.cybernetics/auth.json` — the same place all Cybernetics
...
interface in `cybernetics_cli/proxy/adapters/`.
...
If you see `not logged in`, run `cybernetics portal`. If you see
...
`cybernetics_cli/proxy/adapters/<provider>.py` and registering it in
