...
Cybernetics runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Cybernetics
...
1. Cybernetics captures a baseline of current diagnostics for the file.
...
…). Cybernetics auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
`node_modules` tree — Cybernetics installs both packages together when you
run `cybernetics lsp install typescript` or auto-install fires on first
use.

## CLI

```
cybernetics lsp status          # service state + per-server install status
cybernetics lsp list            # registry, optionally --installed-only
cybernetics lsp install <id>    # eagerly install one server
cybernetics lsp install-all     # try every server with a known recipe
cybernetics lsp restart         # tear down running clients
cybernetics lsp which <id>      # print resolved binary path
```

`cybernetics lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, Cybernetics installs binaries into
...
shared location — the staging dir is fully Cybernetics-owned and is
...
Servers are kept alive for the life of the Cybernetics process. There's
...
**`cybernetics lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`cybernetics lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `cybernetics lsp status`**
...
When `cybernetics lsp status` shows a `Backend warnings` section, install
...
`~/.cybernetics/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.cybernetics/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `cybernetics lsp restart` to clear the set;
