...
```bash
SCHEMA=$(cybernetics kanban create "Design auth schema" \
    --assignee backend-dev --tenant auth-project --priority 2 \
    --body "Design the user/session/token schema for the auth module." \
    --json | jq -r .id)

API=$(cybernetics kanban create "Implement auth API endpoints" \
    --assignee backend-dev --tenant auth-project --priority 2 \
    --parent $SCHEMA \
    --body "POST /register, POST /login, POST /refresh, POST /logout." \
    --json | jq -r .id)

cybernetics kanban create "Write auth integration tests" \
...
```bash
cybernetics kanban show $SCHEMA
cybernetics kanban runs $SCHEMA
...
for lang in Spanish French German; do
    cybernetics kanban create "Translate homepage to $lang" \
        --assignee translator --tenant content-ops
done
for i in 1 2 3 4 5; do
    cybernetics kanban create "Transcribe Q3 customer call #$i" \
        --assignee transcriber --tenant content-ops
done
for sku in 1001 1002 1003 1004; do
    cybernetics kanban create "Generate product description: SKU-$sku" \
...
```bash
cybernetics kanban unblock $IMPL
...
```bash
cybernetics kanban create "Deploy to staging (missing creds)" \
    --assignee deploy-bot --tenant ops \
    --max-retries 3
```

The dispatcher tries to spawn the worker. Spawn fails (`RuntimeError: AWS_ACCESS_KEY_ID not set`). The dispatcher releases the claim, increments a failure counter, and tries again next tick. Because this example sets `--max-retries 3`, the circuit trips after three consecutive failures: the task goes to `blocked` with outcome `gave_up`. If you omit the flag, Cybernetics uses `kanban.failure_limit` (default: 2). No more retries until a human unblocks it.
...
```bash
cybernetics kanban runs t_ef5d
# #   OUTCOME        PROFILE        ELAPSED  STARTED
...
The bulk-close guard exists because this data is per-run. `cybernetics kanban complete a b c --summary X` (you, from the CLI) is refused — copy-pasting the same summary to three tasks is almost always wrong. Bulk close without the handoff flags still works for the common "I finished a pile of admin tasks" case. The tool surface doesn't expose a bulk variant at all; `kanban_complete` is always single-task-at-a-time for the same reason.
...
- `cybernetics kanban --help` — every subcommand, every flag.
- `cybernetics kanban watch --kinds completed,gave_up,timed_out` — live stream terminal events across the whole board.
- `cybernetics kanban notify-subscribe <task> --platform telegram --chat-id <id>` — get a gateway ping when a specific task finishes.
