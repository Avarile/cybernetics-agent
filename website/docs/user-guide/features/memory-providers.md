...
Cybernetics Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
cybernetics memory setup      # interactive picker + configuration
cybernetics memory status     # check what's active
cybernetics memory off        # disable external provider
```

You can also select the active memory provider via `cybernetics plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.cybernetics/config.yaml`:
...
When a memory provider is active, Cybernetics automatically:
...
**Setup Wizard:**
```bash
cybernetics memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `cybernetics honcho setup` command still works (it now redirects to `cybernetics memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$HERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HERMES_HOME/honcho.json` > `~/.cybernetics/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/NousResearch/cybernetics-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
:::tip Migrating from `cybernetics honcho`
If you previously used `cybernetics honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Cybernetics profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Cybernetics profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Cybernetics profile. Host key `cybernetics` → default; `cybernetics.<profile>` for others. |
...
```bash
cybernetics profile create coder --clone
```

`--clone` creates a `cybernetics.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
cybernetics honcho sync
```

Scans every Cybernetics profile, creates host blocks for any profile without one, inherits settings from the default `cybernetics` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/NousResearch/cybernetics-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
cybernetics memory setup    # select "openviking"
# Or manually:
cybernetics config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.cybernetics/.env
```
...
**Setup:**
```bash
cybernetics memory setup    # select "mem0"
# Or manually:
cybernetics config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.cybernetics/.env
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `cybernetics-user` | User identifier |
| `agent_id` | `cybernetics` | Agent identifier |
...
**Setup:**
```bash
cybernetics memory setup    # select "hindsight"
# Or manually:
cybernetics config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.cybernetics/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p cybernetics ui start`
...
| `bank_id` | `cybernetics` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Cybernetics Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/NousResearch/cybernetics-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
cybernetics memory setup    # select "holographic"
# Or manually:
cybernetics config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.cybernetics-memory-store`
...
**Setup:**
```bash
cybernetics memory setup    # select "retaindb"
# Or manually:
cybernetics config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.cybernetics/.env
```
...
# Then configure Cybernetics
cybernetics memory setup    # select "byterover"
# Or manually:
cybernetics config set memory.provider byterover
```
...
**Setup:**
```bash
cybernetics memory setup    # select "supermemory"
# Or manually:
cybernetics config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.cybernetics/.env
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `cybernetics` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `cybernetics-{identity}` → `cybernetics-coder`) to isolate memories per Cybernetics profile
...
| **Requires** | `pip install cybernetics-memori` + `cybernetics-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install cybernetics-memori
cybernetics-memori install
cybernetics config set memory.provider memori
cybernetics memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `cybernetics-memori` | Tool-aware memory + structured recall |
