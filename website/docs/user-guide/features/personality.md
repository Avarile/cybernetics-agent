...
description: "Customize Cybernetics Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Cybernetics Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Cybernetics is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Cybernetics now seeds a default `SOUL.md` automatically in:

```text
~/.cybernetics/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Cybernetics with a custom home directory, it will use:
...
- Cybernetics creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Cybernetics loads `SOUL.md` only from `HERMES_HOME`
- Cybernetics does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Cybernetics falls back to a built-in default identity
...
If Cybernetics loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Cybernetics instance itself.

That also makes it easier to teach users:
- "Edit `~/.cybernetics/SOUL.md` to change Cybernetics' default personality."
...
- how Cybernetics should handle uncertainty, disagreement, or ambiguity
...
## What Cybernetics injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Cybernetics falls back to a built-in default identity ("You are Cybernetics Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Cybernetics ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Cybernetics, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Cybernetics its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.cybernetics/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.cybernetics/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-cybernetics)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Cybernetics speaks
- `display.skin` and `/skin` affect how Cybernetics looks in the terminal
