...
sidebar_position: 13
title: "Webhooks"
description: "Receive events from GitHub, GitLab, and other services to trigger Cybernetics agent runs"
---

# Webhooks

Receive events from external services (GitHub, GitLab, JIRA, Stripe, etc.) and trigger Cybernetics agent runs automatically. The webhook adapter runs an HTTP server that accepts POST requests, validates HMAC signatures, transforms payloads into agent prompts, and routes responses back to the source or to another configured platform.
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Cybernetics Agent — Webhooks Tutorial"
...
1. Enable via `cybernetics gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `cybernetics webhook subscribe`
...
Add the `github-pr` route to your `~/.cybernetics/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Cybernetics processes the event, and posts a review comment on the PR.
...
```bash
cybernetics webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `cybernetics webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.
...
```bash
cybernetics webhook list
```

### Remove a subscription

```bash
cybernetics webhook remove github-issues
```

### Test a subscription

```bash
cybernetics webhook test github-issues
cybernetics webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.cybernetics/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `cybernetics webhook subscribe` command.
...
- Run the gateway in foreground to see logs: `cybernetics gateway run`
