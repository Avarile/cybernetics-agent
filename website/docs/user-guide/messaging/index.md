...
description: "Chat with Cybernetics from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Cybernetics from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Cybernetics](/guides/use-voice-mode-with-cybernetics).
...
```mermaid
flowchart TB
    subgraph Gateway["Cybernetics Gateway"]
...
```bash
cybernetics gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
cybernetics gateway              # Run in foreground
cybernetics gateway setup        # Configure messaging platforms interactively
cybernetics gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo cybernetics gateway install --system   # Linux only: install a boot-time system service
cybernetics gateway start        # Start the default service
cybernetics gateway stop         # Stop the default service
cybernetics gateway status       # Check default service status
cybernetics gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Cybernetics Agent to the latest version |
...
Configure per-platform overrides in `~/.cybernetics/gateway.json`:
...
cybernetics pairing approve telegram XKGH5N7P

# Other pairing commands:
cybernetics pairing list          # View pending + approved users
cybernetics pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Cybernetics appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.cybernetics/config.yaml`:
...
Cybernetics confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.cybernetics/config.yaml`:
...
```bash
cybernetics gateway install               # Install as user service
cybernetics gateway start                 # Start the service
cybernetics gateway stop                  # Stop the service
cybernetics gateway status                # Check status
journalctl --user -u cybernetics-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo cybernetics gateway install --system
sudo cybernetics gateway start --system
sudo cybernetics gateway status --system
journalctl -u cybernetics-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `cybernetics update`. When `cybernetics update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart cybernetics-gateway` command (it never blocks on an interactive password prompt).
...
```bash
cybernetics gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `cybernetics update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo cybernetics update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/cybernetics-gateway`:

```
cybernetics ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed cybernetics-gateway*, /usr/bin/systemctl --no-ask-password start cybernetics-gateway*, /usr/bin/systemctl --no-ask-password restart cybernetics-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Cybernetics will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Cybernetics installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.cybernetics` uses `cybernetics-gateway`; other installations use `cybernetics-gateway-<hash>`. The `cybernetics gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::

### macOS (launchd)

```bash
cybernetics gateway install               # Install as launchd agent
cybernetics gateway start                 # Start the service
cybernetics gateway stop                  # Stop the service
cybernetics gateway status                # Check status
tail -f ~/.cybernetics/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.cybernetics.gateway.plist`. It includes three environment variables:
...
- **HERMES_HOME** — scopes the gateway to your Cybernetics installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `cybernetics gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERMES_HOME` directory gets its own launchd label. The default `~/.cybernetics` uses `ai.cybernetics.gateway`; other installations use `ai.cybernetics.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `cybernetics-cli` | Full access |
| Telegram | `cybernetics-telegram` | Full tools including terminal |
| Discord | `cybernetics-discord` | Full tools including terminal |
| WhatsApp | `cybernetics-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `cybernetics-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `cybernetics-slack` | Full tools including terminal |
| Google Chat | `cybernetics-google_chat` | Full tools including terminal |
| Signal | `cybernetics-signal` | Full tools including terminal |
| SMS | `cybernetics-sms` | Full tools including terminal |
| Email | `cybernetics-email` | Full tools including terminal |
| Home Assistant | `cybernetics-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `cybernetics-mattermost` | Full tools including terminal |
| Matrix | `cybernetics-matrix` | Full tools including terminal |
| DingTalk | `cybernetics-dingtalk` | Full tools including terminal |
| Feishu/Lark | `cybernetics-feishu` | Full tools including terminal |
| WeCom | `cybernetics-wecom` | Full tools including terminal |
| WeCom Callback | `cybernetics-wecom-callback` | Full tools including terminal |
| Weixin | `cybernetics-weixin` | Full tools including terminal |
| BlueBubbles | `cybernetics-bluebubbles` | Full tools including terminal |
| QQBot | `cybernetics-qqbot` | Full tools including terminal |
| Yuanbao | `cybernetics-yuanbao` | Full tools including terminal |
| Microsoft Teams | `cybernetics-teams` | Full tools including terminal |
| API Server | `cybernetics-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `cybernetics-webhook` | Full tools including terminal |
...
1. **Gateway log** (`~/.cybernetics/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
