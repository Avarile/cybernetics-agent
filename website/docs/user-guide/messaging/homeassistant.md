...
description: Control your smart home with Cybernetics Agent via Home Assistant integration.
...
Cybernetics Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Cybernetics Agent"
5. Copy the token
...
Cybernetics Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.cybernetics/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Cybernetics Agent".
...
The adapter reads credentials from `~/.cybernetics/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Cybernetics profile
...
running Cybernetics — `curl -H "Authorization: Bearer <token>" <url>/api/` should
