...
Connect Cybernetics to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Cybernetics — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Cybernetics runs it in a
...
cybernetics photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Cybernetics Agent` project on your account.
...
Runtime credentials are written to `~/.cybernetics/.env`
...
(device token, dashboard project id) lives in `~/.cybernetics/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Cybernetics
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Cybernetics replies with a pairing code. Approve it with:

```bash
cybernetics pairing approve photon <CODE>
```

Use `cybernetics pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.cybernetics/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.cybernetics/.env`):
...
By default Cybernetics responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Cybernetics` and
`@Cybernetics agent` variants. For a custom agent name, set regex patterns:
...
```bash
cybernetics gateway start --platform photon
```
...
Send an iMessage to your assigned number and Cybernetics will reply.

## Status & troubleshooting

```bash
cybernetics photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Cybernetics uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run cybernetics photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `cybernetics photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `cybernetics photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `cybernetics photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Cybernetics sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Cybernetics wake words  | JSON list / comma / newline regex patterns for group mentions |
