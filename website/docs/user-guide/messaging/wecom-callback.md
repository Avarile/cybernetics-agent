...
Connect Cybernetics to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Cybernetics supports two WeCom integration modes:
...
> Run `cybernetics gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Cybernetics decrypts the message, queues it for the agent
...
(Use `cybernetics gateway start` only after `cybernetics gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Cybernetics and the token the
...
in `~/.cybernetics/.env` values around `=` will also break signature checks. After
fixing, restart `cybernetics gateway run`.
...
Check `cybernetics gateway run` logs for the bound host/port. If the adapter bound to
