...
Cybernetics Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.cybernetics/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Cybernetics stores session history so it can resume conversations, but it does not
...
Cybernetics explicitly injects for that turn.
...
For example, if a user sends an image and asks Cybernetics to make a meme from it,
Cybernetics may inspect that image once with vision and run an image-processing
...
`cybernetics sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`cybernetics` or `cybernetics chat`) |
...
cybernetics -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
cybernetics -c "my project"   # → resumes "my project #3"
```
...
cybernetics --resume 20250305_091523_a1b2c3d4
cybernetics -r 20250305_091523_a1b2c3d4

# Resume by title
cybernetics --resume "refactoring auth"

# Or with the chat subcommand
cybernetics chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `cybernetics sessions list`.

### Conversation Recap on Resume

When you resume a session, Cybernetics displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Cybernetics session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.cybernetics/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `cybernetics -r "<title>"` from the shell) and pick up where the platform left off.
...
Cybernetics automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `cybernetics sessions list` or `cybernetics sessions browse`.
...
```bash
cybernetics sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Cybernetics creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`cybernetics -c "my project"`), it automatically picks the most recent session in the lineage.
...
Cybernetics provides a full set of session management commands via `cybernetics sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
cybernetics sessions list

# Filter by platform
cybernetics sessions list --source telegram

# Show more sessions
cybernetics sessions list --limit 50
```
...
cybernetics sessions export backup.jsonl

# Export sessions from a specific platform
cybernetics sessions export telegram-history.jsonl --source telegram

# Export a single session
cybernetics sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
cybernetics sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
cybernetics sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
cybernetics sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
cybernetics sessions prune

# Custom age threshold
cybernetics sessions prune --older-than 30

# Only prune sessions from a specific platform
cybernetics sessions prune --source telegram --older-than 60

# Skip confirmation
cybernetics sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`cybernetics insights`](/reference/cli-commands#cybernetics-insights).
...
When Cybernetics cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Cybernetics uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Cybernetics in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.cybernetics/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.cybernetics/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.cybernetics/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |
...
`*.jsonl` files in `~/.cybernetics/sessions/`. They are no longer written or
read by Cybernetics. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Cybernetics process in the same `HERMES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.cybernetics/config.yaml`:
...
cybernetics sessions delete <session_id>
