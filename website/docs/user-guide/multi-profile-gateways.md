...
If you only run one Cybernetics agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Cybernetics agents that should all
...
(`ai.cybernetics.gateway-<name>.plist`) or systemd user service
(`cybernetics-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
cybernetics profile create coder
cybernetics profile create personal-bot
cybernetics profile create research
...
`~/.local/bin/cybernetics-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: cybernetics-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    cybernetics gateway "$action"
  else
    cybernetics -p "$profile" gateway "$action"
  fi
}
...
```bash
cybernetics-gateways start      # start every configured profile
cybernetics-gateways stop       # stop every configured profile
cybernetics-gateways restart    # restart all
cybernetics-gateways status     # status across all
cybernetics-gateways list       # delegates to `cybernetics gateway list`
```

:::tip
The `default` profile is targeted with `cybernetics gateway <action>` (no `-p`),
not `cybernetics -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `cybernetics -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.cybernetics.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/cybernetics-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.cybernetics.gateway.plist` /
`cybernetics-gateway.service`.
...
```bash
# Default profile
tail -f ~/.cybernetics/logs/gateway.log
tail -f ~/.cybernetics/logs/gateway.error.log

# Named profile
tail -f ~/.cybernetics/profiles/<name>/logs/gateway.log
tail -f ~/.cybernetics/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.cybernetics/logs/gateway.log ~/.cybernetics/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
cybernetics logs -f                  # follow default profile
cybernetics -p coder logs -f         # follow one profile
cybernetics logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
cybernetics profile list             # profiles + model + gateway state
cybernetics-gateways status          # full status across every profile
launchctl list | grep cybernetics    # macOS — PIDs and labels
systemctl --user list-units 'cybernetics-gateway-*'   # Linux — units
```
...
```
~/.cybernetics/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.cybernetics/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
cybernetics config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.cybernetics/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=cybernetics --why="gateways running" \
  sleep infinity &
...
`cybernetics-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.cybernetics/.env ~/.cybernetics/profiles/*/.env
```

## Updating the code

`cybernetics update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `cybernetics gateway start` after a previous `cybernetics gateway stop`. The
...
```bash
ps -ef | grep "cybernetics_cli.*-p <profile>"
cat ~/.cybernetics/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.cybernetics.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.cybernetics.gateway-<profile>.plist

# Linux
systemctl --user restart cybernetics-gateway-<profile>.service
```

### Health check

```bash
cybernetics doctor                  # default profile
cybernetics -p <profile> doctor     # one profile
```
