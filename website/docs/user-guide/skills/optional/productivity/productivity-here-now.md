...
| | |
|---|---|
| Source | Optional — install with `cybernetics skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client cybernetics
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client cybernetics
```
...
| `--client {name}`      | Agent name for attribution (e.g. `cybernetics`)    |
