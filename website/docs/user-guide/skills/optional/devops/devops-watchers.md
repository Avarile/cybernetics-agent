...
| | |
|---|---|
| Source | Optional — install with `cybernetics skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `~/.cybernetics/.env` to avoid the 60 req/hr anonymous rate limit):
