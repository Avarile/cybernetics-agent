...
description: "Configure and use Honcho memory with Cybernetics -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Cybernetics -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `cybernetics skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Cybernetics

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Cybernetics profile its own peer identity while sharing a unified view of the user.
...
```bash
cybernetics honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Cybernetics profile's AI peer
...
Honcho models conversations as interactions between **peers**. Cybernetics creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Cybernetics instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Cybernetics run |
| `global` | Single session across all directories |

Manual override: `cybernetics honcho map my-project-name`
...
Each Cybernetics profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block cybernetics.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `cybernetics.coder` host block in `honcho.json`
...
```bash
cybernetics honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Cybernetics when Honcho memory is active.
...
Run `cybernetics honcho setup`. Ensure `memory.provider: honcho` is in `~/.cybernetics/config.yaml`.

### Memory not persisting across sessions
Check `cybernetics honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `cybernetics profile create <name> --clone`. For existing profiles: `cybernetics honcho sync`.
...
| Command | Description |
|---------|-------------|
| `cybernetics honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `cybernetics honcho status` | Show resolved config, connection test, peer info for active profile |
| `cybernetics honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `cybernetics honcho disable` | Disable Honcho for the active profile |
| `cybernetics honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `cybernetics honcho peers` | Show peer identities across all profiles |
| `cybernetics honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `cybernetics honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `cybernetics honcho sessions` | List known directory-to-session-name mappings |
| `cybernetics honcho map <name>` | Map current working directory to a Honcho session name |
| `cybernetics honcho identity` | Seed AI peer identity or show both peer representations |
| `cybernetics honcho sync` | Create host blocks for all Cybernetics profiles that don't have one yet |
| `cybernetics honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Cybernetics + Honcho |
| `cybernetics memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `cybernetics memory status` | Show active memory provider and config |
| `cybernetics memory off` | Disable external memory provider |
