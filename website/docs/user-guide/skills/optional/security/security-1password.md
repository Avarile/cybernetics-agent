...
| | |
|---|---|
| Source | Optional — install with `cybernetics skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Cybernetics Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Cybernetics terminal calls (desktop app flow only)
...
### Service Account (recommended for Cybernetics)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.cybernetics/.env` (the skill will prompt for this on first load).
...
## Cybernetics Execution Pattern (desktop app flow)

Cybernetics terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/cybernetics-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/cybernetics-op.sock"
