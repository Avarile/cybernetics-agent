...
| | |
|---|---|
| Source | Optional — install with `cybernetics skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Cybernetics or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Cybernetics. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.cybernetics/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.cybernetics/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Cybernetics integration, either:

- configure the server in `~/.cybernetics/config.yaml` using the `native-mcp` skill, or
...
### Cybernetics cannot see the deployed server

The server-building part may be correct while the Cybernetics config is not. Load the `native-mcp` skill and configure the server in `~/.cybernetics/config.yaml`, then restart Cybernetics.
