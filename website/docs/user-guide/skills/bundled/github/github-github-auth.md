...
- Give it a name like "cybernetics-agent"
- Select scopes:
...
- Give it a title like "cybernetics-agent-&lt;machine-name>"
...
elif [ -f ~/.cybernetics/.env ] && grep -q "^GITHUB_TOKEN=" ~/.cybernetics/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.cybernetics/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
