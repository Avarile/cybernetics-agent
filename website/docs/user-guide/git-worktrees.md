...
title: "Git Worktrees"
description: "Run multiple Cybernetics agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Cybernetics Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Cybernetics so each session has a clean, isolated working directory.

## Why Use Worktrees with Cybernetics?

Cybernetics treats the **current working directory** as the project root:

- CLI: the directory where you run `cybernetics` or `cybernetics chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.cybernetics/config.yaml`
...
git worktree add ../repo-feature feature/cybernetics-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/cybernetics-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Cybernetics there:

```bash
cd ../repo-feature

# Start Cybernetics in the worktree
cybernetics
```
...
git worktree add ../repo-experiment-a feature/cybernetics-a
git worktree add ../repo-experiment-b feature/cybernetics-b
```
...
Each Cybernetics process:

- Works on its own branch (`feature/cybernetics-a` vs `feature/cybernetics-b`).
...
- Cybernetics checkpoint data under `~/.cybernetics/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Cybernetics experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/cybernetics-checkpoints-docs`, `feature/cybernetics-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Cybernetics from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `cybernetics -w` (Automatic Worktree Mode)

Cybernetics has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `cybernetics/cybernetics-<hash>`).
...
```bash
cybernetics -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `cybernetics -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Cybernetics session its own clean checkout.
