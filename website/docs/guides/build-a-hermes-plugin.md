...
title: "Build a Cybernetics Plugin"
description: "Step-by-step guide to building a complete Cybernetics plugin with tools, hooks, data files, and skills"
---

# Build a Cybernetics Plugin

This guide walks through building a complete Cybernetics plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Cybernetics has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.cybernetics/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `cybernetics skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.cybernetics/plugins/calculator
cd ~/.cybernetics/plugins/calculator
```
...
This tells Cybernetics: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Cybernetics may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `cybernetics my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Cybernetics continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 cybernetics plugins list
```
...
The same logs are always written to `~/.cybernetics/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
cybernetics logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `cybernetics plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.cybernetics/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.cybernetics/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
```
~/.cybernetics/plugins/calculator/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.cybernetics/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.cybernetics/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `cybernetics plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Cybernetics into installing arbitrary packages — only specs Cybernetics itself ships are eligible |
...
auditable in `~/.cybernetics/logs/agent.log`. Plugins load after built-in
...
- **The system prompt is Cybernetics's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `cybernetics <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for cybernetics my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: cybernetics my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for cybernetics my-plugin."""
...
After registration, users can run `cybernetics my-plugin status`, `cybernetics my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `cybernetics name` in a terminal |
...
Cybernetics has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.cybernetics/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `cybernetics gateway status` without SDK instantiation.
...
Cybernetics also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Cybernetics without any Python plugin. Declare them in `~/.cybernetics/config.yaml`:
...
Cybernetics connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.cybernetics/hooks/<name>/`:
...
cybernetics skills search my-workflow --source myorg/skills-repo
cybernetics skills install myorg/skills-repo/my-workflow
```
...
```toml
# pyproject.toml
[project.entry-points."cybernetics_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install cybernetics-plugin-calculator
# Plugin auto-discovered on next cybernetics startup
```
...
```python
# Wrong — will break if Cybernetics passes extra context
def handler(args):
    ...
