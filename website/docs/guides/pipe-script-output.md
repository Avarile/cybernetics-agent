...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `cybernetics send`."
---

# Pipe Script Output to Messaging Platforms

`cybernetics send` is a small, scriptable CLI that pushes a message to any
messaging platform Cybernetics is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | cybernetics send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `cybernetics
...
cybernetics send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
cybernetics send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
cybernetics send --list

# Filter by platform
cybernetics send --list telegram
```
...
Any platform Cybernetics ships adapters for works as a target:
...
`cybernetics send` resolves the message body in this order:

1. **Positional argument** — `cybernetics send --to telegram "hi"`
2. **`--file PATH`** — `cybernetics send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | cybernetics send --to telegram`

When stdin is a TTY (no pipe), Cybernetics does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  cybernetics send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `cybernetics send` reuses your Cybernetics config, the same script works on
any host where Cybernetics is installed — no need to export bot tokens into
...
`cybernetics send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  cybernetics send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | cybernetics send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.cybernetics/bin/cybernetics send \
...
```bash
./train.py --epochs 200 && \
  cybernetics send --to telegram "training done" || \
  cybernetics send --to telegram "training failed (exit $?)"
```
...
cybernetics send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(cybernetics send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `cybernetics send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `cybernetics send` calls
the platform's REST endpoint directly using credentials from
`~/.cybernetics/.env` and `~/.cybernetics/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `cybernetics gateway start` and retry.
...
# Machine-readable
cybernetics send --list --json
```

The listing is built from `~/.cybernetics/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`cybernetics gateway
...
| Approach | Multi-platform | Reuses Cybernetics creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `cybernetics send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`cybernetics send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `cybernetics send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `cybernetics send` shares with cron delivery.
