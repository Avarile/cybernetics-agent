...
Copy-paste blueprints for common automation patterns. Each blueprint uses Cybernetics's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`cybernetics webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `cybernetics webhook subscribe`) |
...
```bash
cybernetics webhook subscribe github-pr-review \
  --events "pull_request" \
...
3. For each merged PR, check if it modified:
   ...
   - CLI commands (cybernetics_cli/commands.py, cybernetics_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (cybernetics_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
cybernetics cron create "0 6 * * *" \
  "Run a dependency security audit on the cybernetics-agent project.

1. cd ~/.cybernetics/cybernetics-agent && source .venv/bin/activate
...
```bash
cybernetics webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
cybernetics webhook subscribe alert-triage \
...
```python title="~/.cybernetics/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Cybernetics-Monitor/1.0"})
...
```bash
cybernetics cron create "every 30m" \
  ...
  --script ~/.cybernetics/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
cybernetics cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Cybernetics Agent development." \
...
```bash
cybernetics webhook subscribe github-issues \
  --events "issues" \
...
```bash
cybernetics webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
cybernetics webhook subscribe stripe-payments \
...
```bash
cybernetics cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the cybernetics-agent codebase.
...
```bash
cybernetics cron create "0 10 * * 3" \
