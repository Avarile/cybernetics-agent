...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Cybernetics Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Cybernetics instantly when PRs are opened or updated.
:::
...
- **Cybernetics Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  cybernetics gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.cybernetics/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Cybernetics can access GitHub. Start a chat:
...
Still in the chat, ask Cybernetics to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NousResearch/cybernetics-agent
```
...
A skill gives Cybernetics consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.cybernetics/skills/code-review
```

Create `~/.cybernetics/skills/code-review/SKILL.md`:
...
Verify it loaded — start `cybernetics` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Cybernetics your team's standards:
...
```bash
cybernetics cron run pr-review
```
...
2. Teach Cybernetics your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
cybernetics gateway status    # Is the gateway running?
cybernetics cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/guides/build-a-cybernetics-plugin)** — wrap the review logic into a shareable plugin
