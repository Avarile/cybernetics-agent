...
description: "Diagnose and fix common Cybernetics cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`cybernetics gateway` for foreground, or `cybernetics gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `cybernetics cron tick`.
...
```bash
date
cybernetics cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.cybernetics/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.cybernetics/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.cybernetics/.env` |
...
| `local` | Write access to `~/.cybernetics/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `cybernetics cron list` for updated `last_error` field (if available).
...
```bash
cybernetics skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `cybernetics skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `cybernetics skills list`.
...
2. `~/.cybernetics/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `cybernetics cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Cybernetics config directory). Verify:
```bash
ls ~/.cybernetics/scripts/your-script.py   # Must exist
cybernetics cron edit <job_id> --script ~/.cybernetics/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `cybernetics skills install <skill-name>`.
...
```bash
ps aux | grep cybernetics
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.cybernetics/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.cybernetics/cron/jobs.json
chmod 600 ~/.cybernetics/cron/jobs.json   # Your user should own it
```
...
```bash
cybernetics cron list                    # Show all jobs, states, next_run times
cybernetics cron run <job_id>            # Schedule for next tick (for testing)
cybernetics cron edit <job_id>           # Fix configuration issues
cybernetics logs                         # View recent Cybernetics logs
cybernetics skills list                  # Verify installed skills
```
...
1. Run the job with `cybernetics cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.cybernetics/logs/agent.log` for scheduler messages and `~/.cybernetics/logs/errors.log` for warnings
