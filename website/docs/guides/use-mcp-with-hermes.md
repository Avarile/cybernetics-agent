---
sidebar_position: 6
title: "Use MCP with Cybernetics"
description: "A practical guide to connecting MCP servers to Cybernetics Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Cybernetics

This guide shows how to actually use MCP with Cybernetics Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Cybernetics tool
- you want Cybernetics to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Cybernetics to internal APIs, databases, or company systems without modifying Cybernetics core

Do not use MCP when:
- a built-in Cybernetics tool already solves the job well
...
- Cybernetics remains the agent
- MCP servers contribute tools
- Cybernetics discovers those tools at startup or reload time
...
If you installed Cybernetics with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Cybernetics banner/status should show MCP integration when configured
- ask Cybernetics what tools it has available
...
## WSL2: bridge Cybernetics in WSL to Windows Chrome

This is the practical setup when:

- Cybernetics runs inside WSL2
...
In this setup, Cybernetics does **not** connect to Chrome directly. Instead:

- Cybernetics runs in WSL
- Cybernetics starts a local stdio MCP server
...
```text
Cybernetics (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Cybernetics stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Cybernetics core browser transport
...
```bash
cybernetics mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
cybernetics mcp test chrome-devtools-win
```

Then start a fresh Cybernetics session or run:
...
Once loaded, Cybernetics can use the MCP-prefixed browser tools directly. For example:
...
If Cybernetics runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Cybernetics from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Cybernetics from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Cybernetics:
...
2. Cybernetics-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Cybernetics will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Cybernetics to reason over a bounded workspace.
...
Now Cybernetics can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Cybernetics core.
...
Because Cybernetics now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
