...
This tutorial walks you through setting up a Telegram bot powered by Cybernetics Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Cybernetics Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.cybernetics/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Cybernetics itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Cybernetics Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_cybernetics_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Cybernetics Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.cybernetics/.env`:
...
```
[Gateway] Starting Cybernetics Gateway...
...
# View live logs
journalctl --user -u cybernetics-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `cybernetics gateway install` to update the plist.
:::
...
```bash
cybernetics gateway stop && cybernetics gateway start
```
...
cybernetics pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.cybernetics/.env`:
...
Control how much detail the bot shows when using tools. In `~/.cybernetics/config.yaml`:
...
Customize how the bot communicates by editing `~/.cybernetics/SOUL.md`:

For a full guide, see [Use SOUL.md with Cybernetics](/guides/use-soul-with-cybernetics).
...
```bash
# From the CLI
cybernetics cron list          # View all scheduled jobs
cybernetics cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u cybernetics-gateway` (Linux) or `~/.cybernetics/logs/gateway.log` (macOS) |
| Cron job output | `~/.cybernetics/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.cybernetics/cron/jobs.json` |
| Pairing data | `~/.cybernetics/pairing/` |
| Session history | `~/.cybernetics/sessions/` |
