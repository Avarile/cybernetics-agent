...
3. Sets `model.provider: nous` in `~/.cybernetics/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Cybernetics is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
cybernetics setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: manual paste (for Cloud Shell, Codespaces, EC2 Instance Connect)
cybernetics auth add nous --type oauth --manual-paste
# Then re-run `cybernetics setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Cybernetics Agent release notes" and summarize the top 3 hits.
```

You should see Cybernetics call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`cybernetics setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `cybernetics tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Cybernetics runs the Portal login inline (without changing your inference provider or your other tools).

Verify your mix with:

```bash
cybernetics portal tools
```
...
```bash
cybernetics setup voice
# → pick "Nous Subscription" for TTS
# → pick a speech-to-text backend (local faster-whisper is free, no setup)
```

Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Cybernetics will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
cybernetics cron create "every day at 9am" \
...
If you use [Cybernetics profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.cybernetics/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `cybernetics portal info` shows "not logged in" after `cybernetics setup --portal`
...
```bash
cybernetics config set model.provider nous
```
...
Re-verify with `cybernetics portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Cybernetics doesn't replay it endlessly. Just log in again:

```bash
cybernetics auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/NousResearch/cybernetics-agent/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`cybernetics portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Cybernetics profiles where you're using the wrong one (check `cybernetics profile current`)

### Want to revoke and start clean

```bash
cybernetics auth remove nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Cybernetics tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Cybernetics configurations
