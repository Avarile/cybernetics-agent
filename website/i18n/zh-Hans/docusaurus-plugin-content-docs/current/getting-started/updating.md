...
sidebar_position: 3
title: "更新与卸载"
description: "如何将 Cybernetics Agent 更新至最新版本或将其卸载"
---
...
```bash
cybernetics update --check    # 查看 PyPI 上是否有更新的版本
cybernetics update            # 执行 pip install --upgrade cybernetics-agent
```

或手动执行：

```bash
pip install --upgrade cybernetics-agent    # 或：uv pip install --upgrade cybernetics-agent
```

:::tip
`cybernetics update` 会自动检测新的配置选项并提示你添加。如果跳过了该提示，可手动运行 `cybernetics config check` 查看缺失的选项，再运行 `cybernetics config migrate` 以交互方式添加。
:::

### 更新过程（Git 安装方式）

运行 `cybernetics update` 时，将依次执行以下步骤：

1. **配对数据快照** — 保存一份轻量级的更新前状态快照（涵盖 `~/.cybernetics/pairing/`、飞书评论规则及其他运行时修改的状态文件）。可通过 [快照与回滚](../user-guide/checkpoints-and-rollback.md) 中描述的快照恢复流程进行恢复，或从 Cybernetics 写入 `~/.cybernetics/` 目录旁的最新快速快照 zip 文件中提取。
2. **Git pull** — 从 `main` 分支拉取最新代码并更新子模块
3. **依赖安装** — 运行 `uv pip install -e ".[all]"` 以获取新增或变更的依赖项
4. **配置迁移** — 检测自当前版本以来新增的配置选项并提示设置
5. **Gateway 自动重启** — 更新完成后刷新正在运行的 gateway，使新代码立即生效。由服务管理的 gateway（Linux 上的 systemd、macOS 上的 launchd）通过服务管理器重启；手动启动的 gateway 在 Cybernetics 能将运行中的 PID 映射回某个 profile 时会自动重新启动。

### 仅预览：`cybernetics update --check`

想在拉取前确认是否有更新？运行 `cybernetics update --check` — 对于 Git 安装方式，它会获取并与 `origin/main` 比较提交；对于 pip 安装方式，它会查询 PyPI 上的最新版本。不修改任何文件，不重启 gateway。适合在以"是否有更新"为条件的脚本和 cron 任务中使用。
...
### Windows：另一个 `cybernetics.exe` 正在运行

在 Windows 上，如果 `cybernetics update` 检测到另一个 `cybernetics.exe` 进程持有 venv 入口点可执行文件的句柄，它将拒绝运行 — 最常见的情况是 Cybernetics Desktop 应用启动的后端进程、另一个终端中打开的 `cybernetics` REPL，或正在运行的 gateway：
...
`cybernetics update` 处理主要的更新流程，但快速验证可确认一切正常落地：

1. `git status --short` — 若工作树出现意外的脏状态，请在继续前检查
2. `cybernetics doctor` — 检查配置、依赖项和服务健康状态
3. `cybernetics --version` — 确认版本已按预期更新
4. 如果使用 gateway：`cybernetics gateway status`
5. 如果 `doctor` 报告 npm audit 问题：在标记的目录中运行 `npm audit fix`

:::warning 更新后工作树出现脏状态
如果 `cybernetics update` 后 `git status --short` 显示意外变更，请在继续前停下来检查。这通常意味着本地修改被重新应用到了更新后的代码之上，或依赖步骤刷新了锁文件。
:::

### 终端在更新中途断开连接

`cybernetics update` 针对意外终端断开进行了保护：

- 更新会忽略 `SIGHUP`，因此关闭 SSH 会话或终端窗口不再会在安装中途终止它。`pip` 和 `git` 子进程继承此保护，因此 Python 环境不会因连接断开而处于半安装状态。
- 更新运行期间，所有输出会同步镜像到 `~/.cybernetics/logs/update.log`。如果终端消失，重新连接后检查日志，确认更新是否完成以及 gateway 重启是否成功：
...
你不再需要将 `cybernetics update` 包裹在 `screen` 或 `tmux` 中来应对终端断开。
...
与 [GitHub releases 页面](https://github.com/NousResearch/cybernetics-agent/releases) 上的最新版本进行比较。
...
:::warning
如果新增了配置选项，回滚可能导致配置不兼容。回滚后运行 `cybernetics config check`，如果遇到错误，请从 `config.yaml` 中删除无法识别的选项。
:::
...
卸载程序会提供选项，让你保留配置文件（`~/.cybernetics/`）以便将来重新安装。

### pip 安装方式

```bash
pip uninstall cybernetics-agent
rm -rf ~/.cybernetics            # 可选 — 如计划重新安装则保留
```
