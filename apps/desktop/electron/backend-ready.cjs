const _READY_RE = /^CYBERNETICS_DASHBOARD_READY port=(\d+)/m

/**
 * Watch a child process's stdout for the `CYBERNETICS_DASHBOARD_READY port=<N>`
...
function waitForDashboardPort(child, timeoutMs = 45_000) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Cybernetics backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Cybernetics backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
