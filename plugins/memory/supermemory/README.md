...
|-----|---------|-------------|
| `container_tag` | `cybernetics` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `cybernetics-{identity}` → `cybernetics-coder`). |
...
All Supermemory API calls send `x-sm-source: cybernetics`, and document writes stamp
`metadata.sm_source: cybernetics`. This is a **functional routing key, not telemetry**:
it groups Hermes-written memories into a dedicated "Cybernetics" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Cybernetics profile:

```json
{
  "container_tag": "cybernetics-{identity}"
}
```

For a profile named `coder`, this resolves to `cybernetics-coder`. The default profile resolves to `cybernetics-default`. Without `{identity}`, all profiles share the same container.
