...
Resolution order for config file:
  1. $HERMES_HOME/honcho.json  (instance-local, enables isolated Cybernetics instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Cybernetics profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Cybernetics profile.

    Resolution order:
      1. HERMES_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``cybernetics.<profile>``
      3. Fallback: ``"cybernetics"`` (default profile)
    """
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      ...
      2. ~/.cybernetics/honcho.json          (default profile — shared host blocks live here)
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Cybernetics system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.cybernetics
    ...
    def from_global_config(
        ...
        When host is None, derives it from the active Cybernetics profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.cybernetics block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Cybernetics session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — Cybernetics session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Cybernetics session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'cybernetics honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run cybernetics honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `cybernetics honcho setup` to configure)."
            )
