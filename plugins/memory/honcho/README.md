...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/cybernetics>
...
```bash
cybernetics memory setup honcho   # configure Honcho directly (works on a fresh install)
cybernetics memory setup          # generic picker, choose Honcho from the list
```

Or manually:
```bash
cybernetics config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.cybernetics/.env
```

> `cybernetics honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `cybernetics memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Cybernetics instances) |
| 2 | `~/.cybernetics/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Cybernetics profile: `cybernetics` (default) or `cybernetics_<profile>`.
...
**Deployment shapes** (`cybernetics memory setup honcho` asks one prompt to set these):

- **Single-operator** — `pinUserPeer: true`. All gateway users → `peerName`. Recommended for personal use where you connect Cybernetics to your own Telegram/Discord/etc.
...
| 4 | `per-session` strategy | Cybernetics session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`cybernetics-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`cybernetics`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-cybernetics-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening cybernetics in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Cybernetics session ID (timestamp + hex). Every `cybernetics` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Cybernetics profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`cybernetics`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Cybernetics profile: `cybernetics` (default) or `cybernetics_<profile>` (e.g. `cybernetics -p coder` -> host key `cybernetics_coder`). Older `cybernetics.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `cybernetics memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `cybernetics honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `cybernetics memory setup`) |
| `cybernetics honcho status` | Show resolved config for active profile |
| `cybernetics honcho enable` / `disable` | Toggle Honcho for active profile |
| `cybernetics honcho mode <mode>` | Change recall or observation mode |
| `cybernetics honcho peer --user <name>` | Update user peer name |
| `cybernetics honcho peer --ai <name>` | Update AI peer name |
| `cybernetics honcho tokens --context <N>` | Set context token budget |
| `cybernetics honcho tokens --dialectic <N>` | Set dialectic max chars |
| `cybernetics honcho map <name>` | Map current directory to a session name |
| `cybernetics honcho sync` | Create host blocks for all Cybernetics profiles |
