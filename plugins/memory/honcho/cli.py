...
Handles: cybernetics honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "cybernetics" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Cybernetics profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'cybernetics honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'cybernetics honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `cybernetics update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Cybernetics profile."""
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Cybernetics persistent cross-session memory.")
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'cybernetics honcho setup' again once you have a key.\n")
            return
    ...
    current_ai = hermes_host.get("aiPeer") or cfg.get("aiPeer", "cybernetics")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        hermes_host["aiPeer"] = new_ai

    current_workspace = hermes_host.get("workspace") or cfg.get("workspace", "cybernetics")
    ...
    if new_shape == "single":
        ...
        print("  Multi-user mode: each runtime ID → own peer. Use 'cybernetics honcho status' to inspect.")
    ...
    try:
        ...
        print("  Run: cybernetics config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    cybernetics honcho status     -- show full config")
    print("    cybernetics honcho mode       -- change recall/observation mode")
    print("    cybernetics honcho tokens     -- tune context and dialectic budgets")
    print("    cybernetics honcho peer       -- update peer names")
    print("    cybernetics honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Cybernetics profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: cybernetics honcho setup\n")
        return
    ...
    if not cfg:
        ...
        try:
            ...
            if _env_cfg.api_key or _env_cfg.base_url:
                ...
                print("  Run 'cybernetics honcho setup' to configure.\n")
                return
        except Exception:
            print(f"  No Honcho config found at {active_path}")
            print("  Run 'cybernetics honcho setup' to configure.\n")
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: cybernetics honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        ...
        print(f"  AI peer:     {ai}")
        print("    Cybernetics' identity in Honcho. Seed with 'cybernetics honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: cybernetics honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: cybernetics honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ...
        print("    AI-to-AI inference. Cybernetics asks Honcho's AI peer a question")
        ...
        print("\n  Set with: cybernetics honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'cybernetics honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'cybernetics honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    cybernetics honcho identity --show        — show both peer representations")
        print("    cybernetics honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Cybernetics + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Cybernetics\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Cybernetics persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  cybernetics honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'cybernetics honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'cybernetics honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  cybernetics honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'cybernetics'")
        print("  with Honcho configured and no prior session history.")
        print("  (Cybernetics calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    cybernetics honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'cybernetics honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Cybernetics, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'cybernetics honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    cybernetics honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  cybernetics honcho identity <path/to/SOUL.md>")
    ...
    print("    Cybernetics:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Cybernetics:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Cybernetics:   Honcho observes every message and updates representations")
    ...
    print("    Cybernetics:   per-session by default — each run gets its own session")
    print("              Map a custom name:  cybernetics honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. cybernetics honcho setup              — configure API key (required)")
        print("  2. cybernetics honcho migrate            — re-run this walkthrough")
    else:
        print("  1. cybernetics honcho status             — verify Honcho connection")
        print("  2. cybernetics                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. cybernetics honcho identity --show    — verify AI peer representation")
        print("  4. cybernetics honcho tokens             — tune context and dialectic budgets")
        print("  5. cybernetics honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'cybernetics memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``cybernetics honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``cybernetics honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to cybernetics memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Cybernetics Honcho",
    )
