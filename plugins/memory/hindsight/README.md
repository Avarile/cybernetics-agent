...
Daemon startup logs: `~/.cybernetics/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p cybernetics ui start
```
...
Config file: `~/.cybernetics/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `cybernetics` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `cybernetics-{profile}` isolates memory per active Cybernetics profile. Empty placeholders collapse cleanly (e.g. `cybernetics-{user}` with no user becomes `cybernetics`). |
...
The LLM API key is stored in `~/.cybernetics/.env` as `HINDSIGHT_LLM_API_KEY`.
