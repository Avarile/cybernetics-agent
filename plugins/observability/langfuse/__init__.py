"""langfuse — Cybernetics plugin for Langfuse observability.

Traces Cybernetics conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Cybernetics plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``cybernetics plugins enable
observability/langfuse`` or ``cybernetics tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``cybernetics tools`` or ~/.cybernetics/.env):
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Cybernetics plugin system —
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Cybernetics turn",
                tags=["cybernetics", "langfuse"],
            ):
                root_ctx = client.start_as_current_observation(
                    trace_context=trace_ctx,
                    name="Cybernetics turn",
                    as_type="chain",
                    input=trace_input,
                    metadata=metadata,
                    end_on_exit=False,
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Cybernetics branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Cybernetics also has a turn-scoped
    ...
    # messages is a list only for legacy Cybernetics branches that fired
    # pre_llm_call with API messages directly. Current Cybernetics fires
...
def register(ctx) -> None:
    ...
    # Cybernetics versions.  pre_api_request / post_api_request fire per API
