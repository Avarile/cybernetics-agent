...
Optional Cybernetics observability plugin that maps Cybernetics observer hooks to
...
not replace Cybernetics Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Cybernetics emit NeMo Relay lifecycle events for
the work Cybernetics already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Cybernetics Agent can:

- Preserve Cybernetics execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
cybernetics plugins enable observability/nemo_relay
```
...
```bash
env HERMES_HOME=/tmp/cybernetics-nemo-relay-test \
  cybernetics plugins enable observability/nemo_relay
```
...
`HERMES_HOME` is the Cybernetics profile/config home used by both
`cybernetics plugins enable ...` and the later `cybernetics chat ...` run. If unset,
...
```bash
export HERMES_HOME=/tmp/cybernetics-nemo-relay-test
cybernetics plugins enable observability/nemo_relay
cybernetics chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `cybernetics` command you run is built from the
...
uv run cybernetics plugins enable observability/nemo_relay
uv run cybernetics chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
`HERMES_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Cybernetics does not
...
exposes `llm.execute(...)` / `tools.execute(...)`, Cybernetics routes LLM and tool
...
For the full generic Cybernetics middleware contract, see
...
export HERMES_HOME=/tmp/cybernetics-nemo-relay-docs/hermes-home
mkdir -p "$HERMES_HOME"
...
This run starts a parent Cybernetics session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/cybernetics-nemo-relay-docs/subagent/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/cybernetics-nemo-relay-docs/subagent/atif
export HERMES_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='Cybernetics Agent E2E'
...
{"kind":"mark","name":"cybernetics.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Cybernetics Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Cybernetics dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/cybernetics-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/cybernetics-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/cybernetics-nemo-relay-docs/workdir/beta.txt
cd /tmp/cybernetics-nemo-relay-docs/workdir

export HERMES_NEMO_RELAY_ATOF_ENABLED=1
export HERMES_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/cybernetics-nemo-relay-docs/parallel/atof
export HERMES_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HERMES_NEMO_RELAY_ATOF_MODE=overwrite
export HERMES_NEMO_RELAY_ATIF_ENABLED=1
export HERMES_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/cybernetics-nemo-relay-docs/parallel/atif
...
- Cybernetics sessions map to `agent` scopes.
- Cybernetics API request hooks map to `llm` scope start/end events.
- Cybernetics tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Cybernetics execution middleware to hand
...
Enable it for Cybernetics:

```bash
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/cybernetics-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Cybernetics routes execution
...
Hermes provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Cybernetics provider adapter next_call(...)

Hermes tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Cybernetics tool dispatcher next_call(...)
```
...
middleware for a local Cybernetics run. This path requires a NeMo Relay runtime that
...
```bash
export HERMES_HOME=/tmp/cybernetics-middleware-test/hermes-home
mkdir -p "$HERMES_HOME" /tmp/cybernetics-middleware-test/nemo-relay
...
cat > /tmp/cybernetics-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/cybernetics-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/cybernetics-middleware-test/atif"
...
export HERMES_NEMO_RELAY_PLUGINS_TOML=/tmp/cybernetics-middleware-test/nemo-relay/plugins.toml
