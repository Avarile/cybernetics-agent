"""`cybernetics meet node ...` subcommand tree.

Wired into the existing ``cybernetics meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``cybernetics meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="cybernetics-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``cybernetics meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             cybernetics meet node approve <name> ws://<host>:{args.port} {token}")
        try:
