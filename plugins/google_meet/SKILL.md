...
cybernetics meet install                 # pip deps + Chromium (transcribe only)
cybernetics meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
cybernetics meet auth                    # optional; skips guest-lobby wait
cybernetics meet setup                   # preflight checks
```

`cybernetics meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.cybernetics/.env
```
...
pip install playwright websockets && python -m playwright install chromium
cybernetics plugins enable google_meet
cybernetics meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
cybernetics meet node approve my-mac ws://<mac-ip>:18789 <token>
cybernetics meet node ping my-mac                   # confirm reachable
```

Run `cybernetics meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Cybernetics agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `cybernetics meet auth` avoids this.
...
| `realtimeDevice` | Audio device name the bot is feeding ((e.g. `cybernetics_meet_src`)). |
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `cybernetics meet node approve`.
