...
Let the cybernetics agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where cybernetics runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `cybernetics meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `cybernetics meet` CLI |
...
| `cli.py` | `cybernetics meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `cybernetics meet node {run,list,approve,remove,status,ping}` |
...
cybernetics meet install                                      # pip + Chromium
cybernetics meet setup                                        # preflight
cybernetics meet auth                                         # optional
cybernetics meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
cybernetics meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.cybernetics/.env
cybernetics meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
cybernetics meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
cybernetics meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.cybernetics/.env
cybernetics meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, cybernetics will **not** switch your system audio input automatically — the
...
cybernetics plugins enable google_meet
cybernetics meet node run --display-name my-mac --host 0.0.0.0 --port 18789
...
cybernetics meet node ping my-mac
