...
# and exposes ``window.__cyberneticsMeetDrain()`` to pull new entries. This
# mirrors the OpenUtter caption scraping approach.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__cyberneticsMeetInstalled) return;
  window.__cyberneticsMeetInstalled = true;
  window.__cyberneticsMeetQueue = [];
  ...
  function pushEntry(speaker, text) {
    ...
    window.__cyberneticsMeetQueue.push({
      ts: Date.now(),
  ...
  window.__cyberneticsMeetDrain = () => {
    const out = window.__cyberneticsMeetQueue.slice();
    window.__cyberneticsMeetQueue = [];
    return out;
  };
})();
...
def _start_realtime_speaker(
    *,
    ...
    if platform_tag == "linux":
        ...
        sink = (bridge_info or {}).get("write_target") or "cybernetics_meet_sink"
        try:
            proc = _sp.Popen(
                [
                    "paplay",
...
def run_bot() -> int:  # noqa: C901 — orchestration, explicit branches
    ...
    guest_name = os.environ.get("HERMES_MEET_GUEST_NAME", "Cybernetics Agent")
    ...
    try:
        with sync_playwright() as pw:
            ...
            while not stop_flag["stop"]:
                ...
                try:
                    queued = page.evaluate("window.__cyberneticsMeetDrain && window.__cyberneticsMeetDrain()")
...
def _detect_admission(page) -> bool:
    ...
    (() => {
      ...
      if (window.__cyberneticsMeetInstalled) {
