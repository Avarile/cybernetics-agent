...
``{title, url, description, position}`` rows every other Cybernetics web
provider produces.
...
prefers Cybernetics-managed xAI Grok OAuth (via ``cybernetics auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.cybernetics/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``cybernetics tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `cybernetics auth` to sign in with "
