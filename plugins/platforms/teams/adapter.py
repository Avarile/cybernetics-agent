"""
Microsoft Teams platform adapter for Cybernetics Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Cybernetics"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        cybernetics_action = data.get("cybernetics_action", "")
        session_key = data.get("session_key", "")

        if not cybernetics_action or not session_key:
        ...
        choice = choice_map.get(cybernetics_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="cybernetics_approve",
                    data={**btn_data_base, "cybernetics_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="cybernetics_approve",
                    data={**btn_data_base, "cybernetics_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="cybernetics_approve",
                    data={**btn_data_base, "cybernetics_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="cybernetics_approve",
                    data={**btn_data_base, "cybernetics_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print()
    print_success("Teams configuration saved to ~/.cybernetics/.env")
