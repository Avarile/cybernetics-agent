"""
LINE Messaging API platform adapter for Cybernetics Agent.
...
to Cybernetics Agent. It deliberately ports the *strongest* idea from each into
...
def is_connected(config) -> bool:
    """Surface in ``cybernetics status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``cybernetics status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cybernetics setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.cybernetics/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.cybernetics/.env")
        return
