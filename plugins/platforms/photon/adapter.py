"""
Photon Spectrum (iMessage) platform adapter for Cybernetics Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Cybernetics dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `cybernetics photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `cybernetics setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Cybernetics defaults).
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Cybernetics checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `cybernetics photon setup`)"
            )
...
def register(ctx) -> None:
    """Called by the Cybernetics plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `cybernetics photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: cybernetics photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `cybernetics gateway setup` alongside every other
    ...
    # Register CLI subcommands — `cybernetics photon ...`
