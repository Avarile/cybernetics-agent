...
Credential storage mirrors every other Cybernetics channel:

    * runtime SDK creds  -> ``~/.cybernetics/.env``  (``PHOTON_PROJECT_ID`` =
      spectrumProjectId, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.cybernetics/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a cybernetics dependency
...
# registers Cybernetics as its own client_id.
...
# Default name of the project Cybernetics provisions for the operator.
DEFAULT_PROJECT_NAME = "Cybernetics Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of cybernetics-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.cybernetics/auth.json`` honouring the active Cybernetics profile."""
    try:
        from hermes_constants import get_hermes_home
        return Path(get_hermes_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.cybernetics")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.cybernetics/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.cybernetics/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.cybernetics/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `cybernetics photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `cybernetics photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `cybernetics photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `cybernetics photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `cybernetics photon setup`)"
