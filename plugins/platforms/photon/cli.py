"""
``cybernetics photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone `login` verb (matching how every other Cybernetics
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `cybernetics photon ...` subcommands."""
    ...
    p_setup.add_argument("--project-name", default=None,
                         help="Project name (default: 'Cybernetics Agent')")
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``cybernetics photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the "Cybernetics Agent" project.
    ...
    #    and persist both (runtime creds -> ~/.cybernetics/.env, ids -> auth.json).
    try:
    ...
    print("  Start the gateway:  cybernetics gateway start --platform photon")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `cybernetics photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`cybernetics photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.cybernetics/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `cybernetics photon telemetry on` / `cybernetics photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.cybernetics/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  cybernetics gateway restart")
    return 0
...
#
# `cybernetics gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``cybernetics photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `cybernetics gateway setup` wizard."""
