"""
IRC Platform Adapter for Cybernetics Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Cybernetics agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "cybernetics-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Cybernetics Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Cybernetics Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: cybernetics_, cybernetics_1, cybernetics_2...
...
def check_requirements() -> bool:
    ...
    # cybernetics setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `cybernetics gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Cybernetics to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    nickname = prompt(
        "Bot nickname (e.g. cybernetics-bot)",
    ...
    channel = prompt(
        "Channel to join (e.g. #cybernetics — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.cybernetics/.env")
    print_info("Restart the gateway for changes to take effect: cybernetics gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "cybernetics-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "cybernetics-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Cybernetics Agent (cron)")
