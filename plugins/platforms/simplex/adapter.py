"""SimpleX Chat platform adapter (Cybernetics plugin).
...
This adapter ships as a Cybernetics platform plugin under
``plugins/platforms/simplex/``. The Cybernetics plugin loader scans the
...
discoverable and ``cybernetics setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Cybernetics — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``cybernetics setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.cybernetics/.env`` via
    ``hermes_cli.config``.
    """
    print()
