...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.cybernetics/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Cybernetics gateway
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Cybernetics manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "cybernetics_voice",
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Cybernetics session")
        ...
        @tree.command(name="status", description="Show Cybernetics session status")
        ...
        @tree.command(name="stop", description="Stop the running Cybernetics agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.cybernetics/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Cybernetics Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Cybernetics gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Cybernetics session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Cybernetics in the thread",
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Cybernetics skill",
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Cybernetics session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Cybernetics: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Cybernetics"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Cybernetics: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        reason = "Cybernetics session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Cybernetics handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Cybernetics needs your input",
                description=body,
                color=discord.Color.orange(),
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``cybernetics update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``cybernetics update`` prompts.
...
# is not in this process (e.g. ``cybernetics cron`` running standalone) and no live
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cybernetics-agent[messaging]'",
