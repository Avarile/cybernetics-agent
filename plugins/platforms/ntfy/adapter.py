"""ntfy platform adapter (Cybernetics plugin).
...
a Cybernetics dependency.

This adapter ships as a Cybernetics platform plugin under
``plugins/platforms/ntfy/``. The Cybernetics plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "cybernetics-in"              # subscribe topic (incoming)
          publish_topic: "cybernetics-out"     # optional — defaults to topic
...
_ECHO_TAG = "cybernetics-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``cybernetics cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Cybernetics dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `cybernetics gateway status` without
