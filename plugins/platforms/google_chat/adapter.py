...
# ``model_tools`` import time, so ``cybernetics status``, ``cybernetics chat``, etc.
...
# ``cybernetics_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Cybernetics is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a cybernetics MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        try:
            ...
            _hermes_home = _Path.home() / ".cybernetics"
        ...
        # they don't sit in the chat forever as "Cybernetics is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HERMES_HOME", str(_Path.home() / ".cybernetics"))
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a cybernetics MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Cybernetics is thinking…' marker message.
        ...
        "Cybernetics is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Cybernetics is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Cybernetics is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Cybernetics is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Cybernetics is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``cybernetics setup``.
    ...
    prompt for env vars, persist them to ``~/.cybernetics/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. cybernetics-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.cybernetics/.env")
    print_info("Restart the gateway: cybernetics gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``cybernetics cron`` running as a
    separate process from ``cybernetics gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Cybernetics plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'cybernetics-agent[google_chat]'",
        ...
        # the "Cybernetics is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Cybernetics is thinking…' marker message "
