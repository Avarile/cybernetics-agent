# Cybernetics Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: cybernetics-achievements plugin
...
### `GET /api/plugins/cybernetics-achievements/achievements`
...
### `POST /api/plugins/cybernetics-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/cybernetics-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/cybernetics-achievements/overview`
...
- Persist plugin data under:
  - `~/.cybernetics/plugins/cybernetics-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.cybernetics/plugins/cybernetics-achievements/scan_checkpoint.json`
...
- `~/.cybernetics/plugins/cybernetics-achievements/`
