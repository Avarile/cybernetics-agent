...
Auto-tracks and cleans up ephemeral files created during Cybernetics Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HERMES_HOME` and `/tmp/cybernetics-*`.
...
- `is_safe_path()` rejects anything outside `HERMES_HOME` or `/tmp/cybernetics-*`
