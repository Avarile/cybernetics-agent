# OpenAI-Compatible API Server for Cybernetics Agent
...
SSE streaming. By exposing this endpoint, cybernetics-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  cybernetics-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.cybernetics/.env`
2. Run `cybernetics gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with cybernetics-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns cybernetics-agent as a model) |
...
Authorization: Bearer cybernetics-api-key-here
...
```json
{
  "object": "list",
  "data": [{
    "id": "cybernetics-agent",
    "object": "model",
    "created": 1710000000,
    "owned_by": "cybernetics-agent"
  }]
}
```
...
But cybernetics-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "cybernetics-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured cybernetics-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of cybernetics-agent's core prompt
...
Once implemented, cybernetics-agent works as a drop-in backend for:
