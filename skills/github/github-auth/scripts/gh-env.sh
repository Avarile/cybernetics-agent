#!/usr/bin/env bash
# GitHub environment detection helper for Cybernetics Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _cybernetics_env="${HERMES_HOME:-$HOME/.cybernetics}/.env"; [ -f "$_cybernetics_env" ] && grep -q "^GITHUB_TOKEN=" "$_cybernetics_env" 2>/dev/null; then
