...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _cybernetics_env="${HERMES_HOME:-$HOME/.cybernetics}/.env"; [ -f "$_cybernetics_env" ] && grep -q "^GITHUB_TOKEN=" "$_cybernetics_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_cybernetics_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
