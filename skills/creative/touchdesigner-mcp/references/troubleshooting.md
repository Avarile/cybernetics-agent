...
## 5. Cybernetics Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.cybernetics/config.yaml` when `HERMES_HOME` is unset)
...
Restart the Cybernetics session for changes to take effect. The MCP connection is
