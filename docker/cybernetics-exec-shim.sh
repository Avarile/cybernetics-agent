#!/bin/sh
# shellcheck shell=sh
# /opt/cybernetics/bin/cybernetics — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `cybernetics` user (UID 10000). When an operator runs `docker exec <c> cybernetics ...`
...
# user runs `docker exec <c> cybernetics login`, this writes
...
# Portal" on every incoming message — even though `docker exec <c> cybernetics
...
# Fix
# ---
# This shim sits at /opt/cybernetics/bin/cybernetics and is placed earliest on PATH.
# When invoked as root, it drops to the cybernetics user (via s6-setuidgid)
...
# `docker exec --user cybernetics`, kanban subagents, etc. — it short-circuits
...
# (/opt/cybernetics/.venv/bin/cybernetics), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set CYBERNETICS_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the cybernetics CLI. Default is to drop.

set -e

REAL=/opt/cybernetics/.venv/bin/cybernetics

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "cybernetics-shim: $REAL not found or not executable" >&2
    exit 127
fi
...
case "${CYBERNETICS_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the cybernetics user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "cybernetics-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "cybernetics-shim: re-run with --user cybernetics or set CYBERNETICS_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the cybernetics user's home before dropping privileges. Without
